...
let match_expr _loc es cases nbody : exp =
  ...
  | _ ->
	...
	[ None -> {:case| $(pat:tuple_com_unit _loc ps) -> $body |}
      ...
      {:exp| match $(tuple_com_unit _loc es) with [ $list:cases | _ -> $nbody ] |}] ;
...
let value_type _loc comps =
  ...
  match cs with
  [[] -> uid _loc n
  | _  -> `Of(_loc,uid _loc n, sta_of_list cs )] in
  ...
  let ts = StringMap.fold
	...
	[(ctyp_of_cons _loc (PlNames.comp comp) args)::l])
      comps [ {:or_ctyp| $(uid:PlNames.int_cons) of int |} ] in
  {:stru| type $(lid:PlNames.val_type) = [ $list:ts ] |};
...
let list_repr _loc comps =
  ...
  [{:case| $(uid:PlNames.comp PlNames.nil) -> [] |} :: cases] in
  let cases =
  ...
  {:stru| let rec $(lid:PlNames.list_of) = fun [ $list:cases ] |};
...
let int_repr _loc comps =
	...
	let cases =
	  ...
	  [{:case| $(uid:PlNames.comp PlNames.neg) x ->
	...
	{:stru| let rec $(lid:PlNames.int_of) = fun [ $list:cases ] |};
...
let value_repr _loc comps =
	let cases = StringMap.fold (fun comp n l ->
		...
		let pat = ep_of_cons _loc (PlNames.comp comp)
                    (List.map (lid _loc) args)
		and expr =
                  match concat_expr ~sep:"," _loc
                      (List.map
                         (fun a ->
			   {:exp| $(lid:PlNames.string_of) $lid:a |}) args) with
		  [ None -> {:exp| $str:comp |}
		  | Some e -> {:exp| $(str:comp ^ "(") ^ $e ^ ")" |}] in
		[{:case| $pat:pat -> $expr |} ::l])
            comps [ {:case| $(uid:PlNames.int_cons) i -> string_of_int i |} ] in
...
exception UnboundVar of string * FanLoc.t;
