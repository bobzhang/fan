...
(* 	  then {:exp| let $(tuple_patt _loc ps) = $(tuple_expr _loc es) in $body |}	 *)
(* 	  else {:exp| match $(tuple_expr _loc es) with [ $(pat:tuple_patt _loc ps) -> $body | _ -> $nbody ] |} *)
(* 	| Some t -> {:exp| match $(tuple_expr _loc es) with [ $(pat:tuple_patt _loc ps) when $t -> $body | _ -> $nbody ] |} ]) ]) *)
(*   | _ -> *)
(*       let cases = List.map (fun (ps,t,body) -> match t with *)
(* 	[ None -> {:case| $(pat:tuple_patt _loc ps) -> $body |} *)
(*         | Some t -> *)
(*             {:case| $(pat:tuple_patt _loc ps) when $t -> $body |}]) cases in *)
(*       {:exp| match $(tuple_expr _loc es) with [ $list:cases | _ -> $nbody ] |}] ; *)
...
(* 		let pat = patt_of_cons _loc (PlNames.comp comp) *)
