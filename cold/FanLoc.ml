...
let  of_lexbuf (lb) =
  ...
  let  start = (Lexing.lexeme_start_p lb ) and stop =
  (Lexing.lexeme_end_p lb )  in
  
  let  loc = {loc_start = start ; loc_end = stop ; loc_ghost = false  } in
  loc
...
let  strictly_before (x) (y) =
  ...
  (( (( (x.loc_end).pos_cnum ) < ( (y.loc_start).pos_cnum )) ) && (
    (( (x.loc_end).pos_fname ) = ( (y.loc_start).pos_fname )) )) in b
let  to_string (x) =
...
let  error_report ((loc , s )) = begin
  ...
  let  err_location =
  (String.sub s abs_start_off ( (( (abs_stop_off - abs_start_off) ) + 1) ) )
  in (prerr_endline ( (sprintf "err: ^%s^" err_location ) ) )
  end
