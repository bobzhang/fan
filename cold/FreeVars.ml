open FanUtil
class
   ['accu ]c_fold_pattern_vars (f) (init) =
    object
    inherit  Camlp4Ast.fold as super val   acc = init
    method   acc :'accu = acc
    method   patt =
...
let rec  fold_binding_vars (f) (bi) (acc) =
  ...
  | Ast.BiAnd(_,bi1,bi2) ->
    ...
    | Ast.BiEq(_,p,_) -> (fold_pattern_vars f p acc) | Ast.BiNil(_) -> acc
    | Ast.BiAnt(_,_) -> assert false)
class
   ['accu ]fold_free_vars ((f : ( string  -> ('accu  -> 'accu ) ) ))
    ?(env_init=SSet.empty) (free_init) = object
     (o)inherit  Camlp4Ast.fold as super val   free = (free_init :'accu  )
    val   env = (env_init : SSet.t  ) method   free = free
    ...
    method   module_expr =
      ...
      | Ast.MeStr(_,st) -> ((o#str_item st)#set_env env)
        | me -> (super#module_expr me)) end
let  free_vars (env_init) (e) =
  
  let  fold = ((new fold_free_vars) SSet.add ~env_init:env_init SSet.empty)
  in (fold#expr e)#free