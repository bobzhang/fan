...
let mapi_expr ~arity  ~names  simple_expr_of_ctyp i (y : ctyp) =
...
let tuple_expr_of_ctyp ~arity  ~names  ~mk_tuple  simple_expr_of_ctyp ty =
...
let rec normal_simple_expr_of_ctyp ~arity  ~names  ~mk_tuple  ~right_type_id 
  ...
  let open Transform in
    let open ErrorMonad in
      ...
      | Unhandled _t ->
          fail &
            (sprintf "normal_simple_expr_of_ctyp inner:{|%s|} outer:{|%s|}\n"
               "" "")
      ...
      | Unhandled _t0 ->
          fail &
            (sprintf "obj_simple_expr_of_ctyp inner:{|%s|} outer:{|%s|}\n" ""
               "")
let expr_of_ctyp ?cons_transform  ~arity  ~names  ~trail  ~mk_variant 
...
let fun_of_tydcl ~names  ~arity  ~left_type_variable  ~mk_record 
                    ...
                    | { label; is_mutable; ctyp } ->
                        {
                          info =
                            (mapi_expr ~arity ~names
                               (unwrap simple_expr_of_ctyp) i ctyp);
                          label;
                          is_mutable
                        }) cols in
             ...
             let process =
               (fun ctyp  ->
                  (simple_expr_of_ctyp ctyp) >>=
                    (fun expr  -> return & (eta_expand (expr +> names) arity)))
                 <|> expr_of_ctyp in
             let funct =
...
let binding_of_tydcl ?cons_transform  simple_expr_of_ctyp tydcl ~arity 
...
let obj_of_module_types ?cons_transform  ?module_name  ~arity  ~names  ~trail
