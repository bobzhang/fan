...
module Camlp4Bin(PreCast:Sig.PRECAST) =
  ...
  let (objext,libext) =
    if DynLoader.is_native then begin
      (".cmxs",".cmxs")
    end else begin
      (".cmo",".cma")
    end
  let rewrite_and_load (n) (x) =
    ...
    begin
      ...
      | _ ->
          ...
          (real_load ( begin try (find_in_path y)
            with
            | Not_found  ->   x end ))
      end;
  ...
  let rec parse_file (dyn_loader) (name) (pa) (getdir) =
    ...
    begin
      ...
      let phr = begin try
        (pa ?directive_handler:directive_handler loc cs)
        with
        | x ->   begin
                 (clear () );
                 (raise x)
