...
module Camlp4Bin =
 functor (PreCast : Sig.PRECAST) ->
  ...
  let print_version =
   fun ()  -> ( (eprintf "Camlp4 version %s@." FanConfig.version) ); (exit 0)
  let print_stdlib =
   fun ()  -> ( (printf "%s@." FanConfig.camlp4_standard_library) ); (exit 0)
  let usage =
   fun ini_sl ->
    fun ext_sl ->
     (
  ...
  let (task , do_task) =
   ...
   let do_task =
    fun usage -> (match t.contents with | Some (f) -> (f usage) | None -> ()) in
   (task , do_task)
  let input_file =
   fun x ->
    let dyn_loader = ((DynLoader.instance.contents) () ) in
