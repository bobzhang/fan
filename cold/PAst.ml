...
and ctyp =
  [ `TyNil of loc  | `TyAli of (loc * ctyp * ctyp )  | `TyAny of loc  |
    ...
    `TyOlb of (loc * string * ctyp )  | `TyPol of (loc * ctyp * ctyp )  |
    `TyTypePol of (loc * ctyp * ctyp )  | `TyQuo of (loc * string )  |
    `TyQuP of (loc * string )  | `TyQuM of (loc * string )  | `TyAnP of loc 
    | `TyAnM of loc  | `TyVrn of (loc * string )  | `TyRec of (loc * ctyp ) 
...
and patt =
  [ `PaNil of loc  | `PaId of (loc * ident )  |
    ...
    `PaAny of loc  | `PaApp of (loc * patt * patt )  |
    `PaArr of (loc * patt )  | `PaCom of (loc * patt * patt )  |
    `PaSem of (loc * patt * patt )  | `PaChr of (loc * string )  |
...
and expr =
  [ `ExNil of loc  | `ExId of (loc * ident )  |
    ...
    `ExAsf of loc  | `ExAsr of (loc * expr )  |
    `ExAss of (loc * expr * expr )  | `ExChr of (loc * string )  |
    `ExCoe of (loc * expr * ctyp * ctyp )  | `ExFlo of (loc * string )  |
    `ExFor of (loc * string * expr * expr * direction_flag * expr )  |
    `ExFun of (loc * match_case )  | `ExIfe of (loc * expr * expr * expr )  |
    `ExInt of (loc * string )  | `ExInt32 of (loc * string )  |
    `ExInt64 of (loc * string )  | `ExNativeInt of (loc * string )  |
    `ExLab of (loc * string * expr )  | `ExLaz of (loc * expr )  |
...
and sig_item =
  [ `SgNil of loc  | `SgCls of (loc * class_type )  |
    ...
    | `SgDir of (loc * string * expr )  | `SgExc of (loc * ctyp )  |
