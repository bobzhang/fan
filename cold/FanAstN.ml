...
type 'a meta_option = [ `None | `Some of 'a] 
type 'a meta_list = [ `LNil | `LCons of ('a* 'a meta_list)] 
...
type ctyp =
  ...
  | `Package of module_type] 
and patt =
  [ `Nil | `Id of ident | `Alias of (patt* alident) | `Any
...
and expr =
  [ `Nil | `Id of ident | `Dot of (expr* expr) | `App of (expr* expr)
...
and sig_item =
  ...
  | `Open of ident | `Type of ctyp | `Val of (alident* ctyp)] 
and with_constr =
