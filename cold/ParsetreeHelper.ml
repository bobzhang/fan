...
let mkpolytype (t) = begin match t.ptyp_desc with
  | Ptyp_poly(_,_) ->   t
...
let varify_constructors (var_names) =
  let rec loop (t) =
    let desc = begin match t.ptyp_desc with
      | Ptyp_any  ->   Ptyp_any
    ...
    let desc = begin match t.pfield_desc with
      | Pfield(n,typ) ->   Pfield ((n,( (loop typ) )))
      | Pfield_var  ->   Pfield_var end in
    {t with pfield_desc = desc} and loop_row_field (x) = begin match x with
    | Rtag(label,flag,lst) ->   Rtag ((label,flag,( (List.map loop lst) )))
    | Rinherit(t) ->   Rinherit ((loop t)) end in
  loop