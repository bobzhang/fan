...
module PExpr = struct
  ...
  let meta_string _loc i = `ExStr (_loc, (Ast.safe_string_escaped i))
  let meta_char _loc i = `ExChr (_loc, (Char.escaped i))
  let meta_unit _loc _ = (`ExId (_loc, (`IdUid (_loc, "()"))) : PAst.expr )
  let meta_bool _loc =
    function
    | true  -> `ExId (_loc, (`IdLid (_loc, "true")))
    | false  -> `ExId (_loc, (`IdLid (_loc, "false")))
  ...
  let mklist loc =
    let rec loop top =
      ...
      | e1::el ->
          let _loc =
            if top then loc else FanLoc.merge (PAst.loc_of_expr e1) loc in
          `ExApp
            (_loc,
...
module PPatt = struct
  ...
  let meta_string _loc i = `PaStr (_loc, (Ast.safe_string_escaped i))
  let meta_char _loc i = `PaChr (_loc, (Char.escaped i))
  let meta_unit _loc _ = (`PaId (_loc, (`IdUid (_loc, "()"))) : PAst.patt )
  let meta_bool _loc =
    function
    | true  -> `PaId (_loc, (`IdLid (_loc, "true")))
    | false  -> `PaId (_loc, (`IdLid (_loc, "false")))
  ...
  let mklist loc =
    let rec loop top =
      ...
      | e1::el ->
          let _loc =
            if top then loc else FanLoc.merge (PAst.loc_of_patt e1) loc in
          `PaApp
            (_loc,
