...
let grammar_module_name =
  let _loc = FanLoc.ghost in ref ( Ast.IdUid (_loc, "Stream") )
let gm () = grammar_module_name.contents
let strm_n = "__strm"
let peek_fun _loc =
  Ast.ExId (_loc, ( Ast.IdAcc (_loc, ( gm () ), ( Ast.IdLid (_loc, "peek") ))
    ))
let junk_fun _loc =
  Ast.ExId (_loc, ( Ast.IdAcc (_loc, ( gm () ), ( Ast.IdLid (_loc, "junk") ))
    ))
let is_raise =
  function
...
let is_raise_failure =
  ...
  | Ast.ExApp
      (_,Ast.ExId (_,Ast.IdLid (_,"raise")),Ast.ExId
       (_,Ast.IdAcc (_,m,Ast.IdUid (_,"Failure"))))
      when m = ( gm () ) -> true
  | _ -> false
let rec handle_failure e =
  ...
  | Ast.ExTry
      (_,_,Ast.McArr
       (_,Ast.PaId (_,Ast.IdAcc (_,m,Ast.IdUid (_,"Failure"))),Ast.ExNil _,e))
  ...
  | Ast.ExApp (_,Ast.ExId (_,Ast.IdLid (_,"raise")),e) -> (
      match e with
      | Ast.ExId (_,Ast.IdAcc (_,m,Ast.IdUid (_,"Failure"))) when
          m = ( gm () ) -> false
      | _ -> true )
  | Ast.ExApp (_,f,x) ->
...
let stream_pattern_component skont ckont =
  ...
  | SpTrm (_loc,p,None ) ->
      Ast.ExMat (_loc, (
        ...
        Ast.McOr (_loc, (
          Ast.McArr (_loc, (
            Ast.PaApp (_loc, ( Ast.PaId (_loc, ( Ast.IdUid (_loc, "Some") ))
              ), p)
            ), ( Ast.ExNil _loc ), (
            Ast.ExSeq (_loc, (
              Ast.ExSem (_loc, (
            ...
            (_,Ast.McArr
             (_,Ast.PaTyc
              (_,Ast.PaId (_,Ast.IdLid (_,v)),Ast.TyApp
               (_,Ast.TyId (_,Ast.IdAcc (_,m,Ast.IdLid (_,"t"))),Ast.TyAny _)),Ast.ExNil
              _,e))
        ...
        then e
          ...
          then e
            ...
            Ast.ExTry (_loc, e, (
              Ast.McArr (_loc, (
                Ast.PaId (_loc, (
                  Ast.IdAcc (_loc, ( gm () ), ( Ast.IdUid (_loc, "Failure")
                    ))
                  ))
      ...
      | Some ep ->
          Ast.ExLet (_loc, Ast.ReNil, (
            Ast.BiEq (_loc, ep, (
              Ast.ExApp (_loc, (
                Ast.ExId (_loc, (
                  Ast.IdAcc (_loc, ( gm () ), ( Ast.IdLid (_loc, "count") ))
                  ))
                ), ( Ast.ExId (_loc, ( Ast.IdLid (_loc, strm_n) )) ))
              ))
            ), e)
      ...
      let skont =
        let ekont err =
          ...
          Ast.ExApp (_loc, ( Ast.ExId (_loc, ( Ast.IdLid (_loc, "raise") ))
            ...
            Ast.ExApp (_loc, (
              Ast.ExId (_loc, (
                Ast.IdAcc (_loc, ( gm () ), ( Ast.IdUid (_loc, "Error") )) ))
              ), str)
            )) in
...
let rec cstream gloc =
  ...
  | SeTrm (_loc,e)::[] ->
      ...
      then
        Ast.ExApp (_loc, (
          Ast.ExId (_loc, (
            Ast.IdAcc (_loc, ( gm () ), ( Ast.IdLid (_loc, "ising") )) ))
          ), e)
      else
        Ast.ExApp (_loc, (
          Ast.ExId (_loc, (
            Ast.IdAcc (_loc, ( gm () ), ( Ast.IdLid (_loc, "lsing") )) ))
          ), ( slazy _loc e ))
  | SeTrm (_loc,e)::secl ->
      if not_computing e
      then
        Ast.ExApp (_loc, (
          Ast.ExApp (_loc, (
            Ast.ExId (_loc, (
              Ast.IdAcc (_loc, ( gm () ), ( Ast.IdLid (_loc, "icons") )) ))
            ), e)
          ), ( cstream gloc secl ))
      else
        Ast.ExApp (_loc, (
          Ast.ExApp (_loc, (
            Ast.ExId (_loc, (
              Ast.IdAcc (_loc, ( gm () ), ( Ast.IdLid (_loc, "lcons") )) ))
            ), ( slazy _loc e ))
          ), ( cstream gloc secl ))
  | SeNtr (_loc,e)::[] ->
      if not_computing e
      then e
        ...
        Ast.ExApp (_loc, (
          Ast.ExId (_loc, (
            Ast.IdAcc (_loc, ( gm () ), ( Ast.IdLid (_loc, "slazy") )) ))
          ), ( slazy _loc e ))
  | SeNtr (_loc,e)::secl ->
      if not_computing e
      then
        Ast.ExApp (_loc, (
          Ast.ExApp (_loc, (
            Ast.ExId (_loc, (
              Ast.IdAcc (_loc, ( gm () ), ( Ast.IdLid (_loc, "iapp") )) ))
            ), e)
          ), ( cstream gloc secl ))
      else
        Ast.ExApp (_loc, (
          Ast.ExApp (_loc, (
            Ast.ExId (_loc, (
              Ast.IdAcc (_loc, ( gm () ), ( Ast.IdLid (_loc, "lapp") )) ))
            ), ( slazy _loc e ))
          ), ( cstream gloc secl ))