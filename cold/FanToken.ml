...
let _=
  (Printexc.register_printer (
    ...
    (function
    | TokenError(e) -> Some ((string_of_error_msg e)) | _ -> None) ))
let  to_string =
  ...
  | (`KEYWORD s) -> (sprintf "`KEYWORD %S" s)
    ...
    | `NEWLINE -> (sprintf "`NEWLINE") | `EOI -> (sprintf "`EOI")
...
let  match_keyword (kwd) =
  ...
  (function
  | (`KEYWORD kwd') when (kwd = kwd') -> true | _ -> false)
let  extract_string =
...
let  keyword_conversion (tok) (is_kwd) =
  ...
  | (((`SYMBOL s) |(`LIDENT s)) |(`UIDENT s)) when (is_kwd s) -> `KEYWORD (s)
    | (`ESCAPED_IDENT s) -> `LIDENT (s) | _ -> tok)
...
let  check_unknown_keywords (tok) (loc) =
  ...
  (match tok with
  | (`SYMBOL s) -> (err ( Illegal_token (s) ) loc) | _ -> ())
module Filter  =
