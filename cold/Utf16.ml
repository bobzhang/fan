...
let next_in_stream bo s =
  let c1 = XStream.next s in
  let c2 = XStream.next s in
...
let compute_len opt_bo str pos bytes =
  let s = stream_from_char_stream opt_bo
    (XStream.from (fun i -> if i + pos >= bytes then None
                  else Some (str.[i + pos])))
  in 
  let l = ref 0 in
  XStream.iter (fun _ -> incr l) s ;
  !l

let blit_to_int opt_bo s spos a apos bytes =
  let s = stream_from_char_stream opt_bo
    (XStream.from (fun i -> if i+spos >= bytes then None
                  else Some (s.[i + spos]))) in
  let p = ref apos in
  try while true do a.(!p) <- XStream.next s ; incr p done; assert false
  with XStream.Failure -> ()
...
let from_utf16_channel ic opt_bo =
  from_utf16_stream ((XStream.of_channel ic)) opt_bo
