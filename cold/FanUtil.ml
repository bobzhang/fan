...
| Assert_failure(file , line , char ) ->
   (Some(sprintf "Assertion failed, file %S, line %d, char %d" file line char
          ))
...
else if (List.mem ( (String.get s i ) ) list ) then ( (loop s ( (i + 1) ) ) )
else (false) in loop
...
module Options :
 ...
 | Arg.Set_string(r) ->
    if (s = "") then
     (
     (match sl with | (s :: sl) -> ( (r := s) ); (Some(sl)) | [] -> (None))
     )
    else begin
     ( (r := s) ); (Some(sl))
    end
 ...
 let  fold (f) (init) =
 let  spec_list = (( init_spec_list.contents ) @ ( ext_spec_list.contents ))
 in
 let  specs =
