...
let  float_repres (f) =
  ...
  | FP_nan  -> "nan"
    ...
    | _ ->
      ...
      end in (valid_float_lexeme float_val))
...
let  stream_peek_nth (n) (strm) =
  ...
  | x::xs ->
    ...
    (loop ( (i - 1) ) xs)
    end | []  -> None) in (loop n ( (Stream.npeek n strm) ))
...
module Options  :
  ...
  struct type spec_list =  ( string * Arg.spec * string ) list   open Format
    ...
    let  remaining_args (argv) =
      ...
      begin
      (loop ( argv.(i)::l ) ( (i + 1) ))
      end in (List.rev ( (loop []  ( (( Arg.current.contents ) + 1) )) ))
