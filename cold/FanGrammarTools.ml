      ...
      fun ( (tok_match_pl,act,i) as accu ) ->
        ...
        | {
            ...
            Ast.ExLet (_loc, Ast.ReNil, (
              Ast.BiEq (_loc, ( Ast.PaId (_loc, ( Ast.IdLid (_loc, s) )) ), (
                Ast.ExApp (_loc, (
                  Ast.ExId (_loc, (
                    Ast.IdAcc (_loc, ( gm () ), (
                      Ast.IdLid (_loc, "string_of_token") ))
            ...
            let id = prefix ^ ( string_of_int i ) in ((
              Some ( (
                ...
                | Some (tok_pl,match_pl) -> ((
                    Ast.ExCom (_loc, (
                      Ast.ExId (_loc, ( Ast.IdLid (_loc, id) )) ), tok_pl)
  ...
  let e =
    ...
    let e2 =
      ...
      | Some (Ast.ExCom (_,t1,t2),Ast.PaCom (_,p1,p2)) ->
          Ast.ExMat (_loc, ( Ast.ExTup (_loc, ( Ast.ExCom (_loc, t1, t2) ))
            ), (
            Ast.McOr (_loc, (
              Ast.McArr (_loc, (
  ...
  | TXmeta (_loc,n,tl,e,t) ->
      ...
      Ast.ExApp (_loc, (
        Ast.ExApp (_loc, (
          Ast.ExApp (_loc, (
            Ast.ExId (_loc, (
              Ast.IdAcc (_loc, ( gm () ), ( Ast.IdUid (_loc, "Smeta") )) ))
            ), ( Ast.ExStr (_loc, n) ))
          ), el)
        ), (
        Ast.ExApp (_loc, (
            ...
            fun t ->
              fun txt ->
                ...
                Ast.ExApp (_loc, (
                  ...
                  ), txt) ) sl ( Ast.ExId (_loc, ( Ast.IdUid (_loc, "[]") ))
            ) in
        Ast.ExApp (_loc, (
      ...
      fun s ->
        fun e ->
          Ast.ExApp (_loc, (
            Ast.ExApp (_loc, ( Ast.ExId (_loc, ( Ast.IdUid (_loc, "::") )) ),
              ( make_expr n "" s.text ))
            ), e) ) sl ( Ast.ExId (_loc, ( Ast.IdUid (_loc, "[]") )) ) in
  (( n.expr ), sl)
let mk_name _loc i =
  {
    expr = ( Ast.ExId (_loc, i) );
    tvar = ( Ident.tvar_of_ident i );
    loc = _loc
  }
  ...
  | Some nl -> (
      ...
      let local_binding_of_name =
        ...
        | { expr = Ast.ExId (_,Ast.IdLid (_,i)); tvar = x; loc = _loc } ->
            Ast.BiEq (_loc, ( Ast.PaId (_loc, ( Ast.IdLid (_loc, i) )) ), (
              Ast.ExTyc (_loc, (
                Ast.ExApp (_loc, (
                  Ast.ExId (_loc, ( Ast.IdLid (_loc, "grammar_entry_create")
                    ))
                  ), ( Ast.ExStr (_loc, i) ))
                ), (
                Ast.TyApp (_loc, (
      ...
      let e =
        ...
        | x::xs ->
            ...
            Ast.ExLet (_loc, Ast.ReNil, (
              Ast.BiEq (_loc, (
                Ast.PaId (_loc, ( Ast.IdLid (_loc, "grammar_entry_create") ))
                ), entry_mk)
            ...
            let globals =
              List.fold_right (
                fun name ->
                  fun acc ->
                    Ast.BiAnd (_loc, acc, (
                      Ast.BiEq (_loc, ( Ast.PaAny _loc ), ( expr_of_name name
                        ))
                      )) ) xs (
      ...
      List.map (
        fun e ->
          ...
          Ast.ExApp (_loc, (
            Ast.ExApp (_loc, (
              Ast.ExId (_loc, (
                Ast.IdAcc (_loc, ( gm () ), ( Ast.IdLid (_loc, "extend") ))
                ))
              ), ent)
            ), (
            Ast.ExApp (_loc, (
    ...
    STapp (_loc, (
      STapp (_loc, (
        STtyp (
          Ast.TyApp (_loc, (
            Ast.TyId (_loc, (
              Ast.IdAcc (_loc, ( gm () ), ( Ast.IdLid (_loc, "fold") )) ))
            ), ( Ast.TyAny _loc ))
          )
        ), ( s.styp ))
      ), styp) in
    ...
    STapp (_loc, (
      STapp (_loc, (
        STtyp (
          Ast.TyApp (_loc, (
            Ast.TyId (_loc, (
              Ast.IdAcc (_loc, ( gm () ), ( Ast.IdLid (_loc, "foldsep") )) ))
            ), ( Ast.TyAny _loc ))
          )
        ), ( s.styp ))
      ), styp) in
