...
(Format.bprintf buf "%a@?" (
  fun (fmt) -> fun (p) -> (AstPrint.pattern fmt ( (Ast2pt.patt p ) ) ) ) patt
  ) in
...
(match s with
 | {text = TXtok(_loc , _ , _ , _ )
    ; _ } ->
    (FanLoc.raise _loc (
      (Stream.Error("Deprecated syntax, use a sub rule. " ^
...
(List.fold_left (
  fun (((tok_match_pl , act , i ) as accu)) ->
    ...
    | {pattern =
        Some(Ast.PaAli(_ , Ast.PaApp(_ , _ , Ast.PaTup(_ , Ast.PaAny(_) ) ) ,
    ...
    (Ast.ExMat(_loc , ( (Ast.ExTup(_loc , ( (Ast.ExCom(_loc , t1 , t2 )) ) ))
               ...
               (Ast.McOr(_loc , (
                         ...
                         (Ast.McArr(_loc , ( (Ast.PaAny(_loc)) ) , (
                                    (Ast.ExNil(_loc)) ) , ( (Ast.ExAsf(_loc))
                                    ) )) ) )) ) ))
 | Some(tok , match_ ) ->
    (Ast.ExMat(_loc , tok , (
               (Ast.McOr(_loc , (
                         (Ast.McArr(_loc , match_ , ( (Ast.ExNil(_loc)) ) ,
                                    e1 )) ) , (
                         (Ast.McArr(_loc , ( (Ast.PaAny(_loc)) ) , (
                                    (Ast.ExNil(_loc)) ) , ( (Ast.ExAsf(_loc))
                                    ) )) ) )) ) ))) in
...
make_expr_rules (_loc) (n) (rl) (tvar) =
(List.fold_left (
  fun (txt) ->
    fun ((sl , ac )) ->
      let  sl =
