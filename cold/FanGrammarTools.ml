...
let string_of_patt patt =
  ...
  let () =
    Format.bprintf buf "%a@?"
      (fun fmt -> fun p -> AstPrint.pattern fmt (Ast2pt.patt p)) patt in
...
let used_of_rule_list rl =
  List.fold_left
    (fun nl ->
       fun r -> List.fold_left (fun nl -> fun s -> s.used @ nl) nl r.prod) []
    rl
             ...
             ((Some
                 ((match tok_match_pl with
                   ...
                   | Some (tok_pl,match_pl) ->
                       ((Ast.ExCom
                           (_loc,(Ast.ExId (_loc,(Ast.IdLid (_loc,id)))),tok_pl)),(
                       Ast.PaCom (_loc,p,match_pl)))))),act,(succ i))
  ...
  let e =
    ...
    let e2 =
      ...
      | Some (Ast.ExCom (_,t1,t2),Ast.PaCom (_,p1,p2)) ->
          Ast.ExMat
            (_loc,(Ast.ExTup (_loc,(Ast.ExCom (_loc,t1,t2)))),(Ast.McOr
                                                                 (_loc,(
                                                                 Ast.McArr
                                                                   (_loc,(
                                                                   Ast.PaTup
      ...
      | Some (tok,match_) ->
          Ast.ExMat
            (_loc,tok,(Ast.McOr
                         ...
                         Ast.McArr
                           (_loc,(Ast.PaAny _loc),(Ast.ExNil _loc),(Ast.ExAsf
                                                                    _loc)))))) in
    Ast.ExFun
      (_loc,(Ast.McArr
               (_loc,(Ast.PaTyc
           ...
           | Some _ ->
               let p =
                 make_ctyp_patt s.styp tvar
                   (Ast.PaId
                      (_loc,(Ast.IdLid
                               (_loc,("__camlp4_" ^ (string_of_int i)))))) in
  ...
  let txt =
    ...
    then
      Ast.ExApp
        (_loc,(Ast.ExId
                 (_loc,(Ast.IdAcc
                          (_loc,(Ast.IdUid (_loc,"Obj")),(Ast.IdLid
                                                            (_loc,"magic")))))),(
        MetaAst.Expr.meta_expr _loc txt))
    else txt in
  Ast.ExApp
    (_loc,(Ast.ExId
             (_loc,(Ast.IdAcc (_loc,(gm ()),(Ast.IdLid (_loc,"mk_action")))))),txt)
...
let rec make_expr entry tvar =
  ...
  | TXlist (_loc,min,t,ts) ->
      ...
      (match (min,ts) with
       | (false ,None ) -> Ast.ExApp (_loc,(Ast.ExVrn (_loc,"Slist0")),txt)
       | (true ,None ) -> Ast.ExApp (_loc,(Ast.ExVrn (_loc,"Slist1")),txt)
       | (false ,Some s) ->
           let x = make_expr entry tvar s.text in
           Ast.ExApp
             (_loc,(Ast.ExVrn (_loc,"Slist0sep")),(Ast.ExTup
                                                     (_loc,(Ast.ExCom
                                                              (_loc,txt,x)))))
       | (true ,Some s) ->
           let x = make_expr entry tvar s.text in
           Ast.ExApp
             (_loc,(Ast.ExVrn (_loc,"Slist1sep")),(Ast.ExTup
                                                     (_loc,(Ast.ExCom
                                                              (_loc,txt,x))))))
  ...
  | TXnterm (_loc,n,lev) ->
      (match lev with
       | Some lab ->
           Ast.ExApp
             (_loc,(Ast.ExVrn (_loc,"Snterml")),(Ast.ExTup
                                                   (_loc,(Ast.ExCom
                                                            (_loc,(Ast.ExApp
                                                                    (_loc,(
                                                                    Ast.ExId
       ...
       | None  ->
           ...
           then Ast.ExVrn (_loc,"Sself")
             ...
             Ast.ExApp
               (_loc,(Ast.ExVrn (_loc,"Snterm")),(Ast.ExApp
                                                    (_loc,(Ast.ExId
                                                             (_loc,(Ast.IdAcc
                                                                    (_loc,(
                                                                    gm ()),(
  ...
  | TXkwd (_loc,kwd) ->
      Ast.ExApp (_loc,(Ast.ExVrn (_loc,"Skeyword")),(Ast.ExStr (_loc,kwd)))
  | TXtok (_loc,match_fun,attr,descr) ->
      Ast.ExApp
        (_loc,(Ast.ExVrn (_loc,"Stoken")),(Ast.ExTup
                                             (_loc,(Ast.ExCom
                                                      (_loc,match_fun,(
                                                      Ast.ExTup
                                                        (_loc,(Ast.ExCom
           ...
           List.fold_right
             (fun t ->
                fun txt ->
                  ...
                  Ast.ExApp
                    (_loc,(Ast.ExApp
                             (_loc,(Ast.ExId (_loc,(Ast.IdUid (_loc,"::")))),x)),txt))
             sl (Ast.ExId (_loc,(Ast.IdUid (_loc,"[]")))) in
         Ast.ExApp
           (_loc,(Ast.ExApp
                    (_loc,(Ast.ExId (_loc,(Ast.IdUid (_loc,"::")))),(
           ...
           let txt =
             ...
             Ast.ExApp
               (_loc,(Ast.ExApp
                        ...
                        Ast.ExTup
                          (_loc,(Ast.ExCom
                                   (_loc,lab,(Ast.ExCom (_loc,ass,e)))))))),txt) in
       ...
       let local_binding_of_name =
         (function
          | {expr = Ast.ExId (_,Ast.IdLid (_,i));tvar = x;loc = _loc} ->
              Ast.BiEq
                (_loc,(Ast.PaId (_loc,(Ast.IdLid (_loc,i)))),(Ast.ExTyc
                                                                (_loc,(
                                                                Ast.ExApp
                                                                  (_loc,(
                                                                  Ast.ExId
              ...
              let entry_mk =
                (match gram with
                 ...
                 | None  ->
                     Ast.ExId
                       (_loc,(Ast.IdAcc
                                (_loc,(gm ()),(Ast.IdLid (_loc,"mk")))))) in
              Ast.ExLet
                (_loc,Ast.ReNil,(Ast.BiEq
                                   (_loc,(Ast.PaId
                                            (_loc,(Ast.IdLid
                                                     (_loc,"grammar_entry_create")))),entry_mk)),(
              ...
              List.fold_right
                ((fun name ->
                    fun acc ->
                      Ast.BiAnd
                        (_loc,acc,(Ast.BiEq
                                     (_loc,(Ast.PaAny _loc),(expr_of_name
                                                               name)))))) xs
...
let text_of_functorial_extend _loc gram gl el =
  let args =
    ...
    | e::el ->
        Ast.ExSeq
          (_loc,(List.fold_left
                   ((fun acc -> fun x -> Ast.ExSem (_loc,acc,x))) e el)) in
  ...
  | None  ->
      ...
      let match_fun =
        ...
        then
          Ast.ExFun
            (_loc,(Ast.McArr
                     (_loc,p',(Ast.ExNil _loc),(Ast.ExId
                                                  (_loc,(Ast.IdUid
                                                           (_loc,"True")))))))
        else
          Ast.ExFun
  ...
  let e =
    Ast.ExApp
      (_loc,(Ast.ExApp
               (_loc,(Ast.ExId
                        (_loc,(Ast.IdAcc
                                 (_loc,(gm ()),(Ast.IdLid (_loc,foldfun)))))),f)),e) in
  let t =
    STapp
      (_loc,(STapp
               (_loc,(STtyp
