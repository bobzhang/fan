...
let rec ctyp =
  ...
  | TyOfAmp (_,_,_)|TyAmp (_,_,_)|TySta (_,_,_)|TyCom (_,_,_)|TyVrn
      ...
      _|TyTypePol (_,_,_)|TyObj (_,_,RvAnt _)|TyNil _|TyTup (_,_) ->
        ...
        | Ppat_variant (s,None ) ->
            let a =
              ...
              then mkpat loc ( Ppat_tuple al )
                ...
                match al with | a::[] -> a | _ -> mkpat loc ( Ppat_tuple al )
                ) in
...
let rec expr =
        ...
        | Pexp_variant (s,None ) ->
            ...
            let a =
              ...
              then mkexp loc ( Pexp_tuple al )
                ...
                match al with | a::[] -> a | _ -> mkexp loc ( Pexp_tuple al )
                ) in
  ...
  | Ast.ExFun (loc,Ast.McArr (_,PaLab (_,lab,po),w,e)) ->
      mkexp loc (
        Pexp_function (lab, None, [(( patt_of_lab loc lab po ), (
          when_expr e w ))]) )
  | Ast.ExFun (loc,Ast.McArr (_,PaOlbi (_,lab,p,e1),w,e2)) ->
      let lab = paolab lab p in
      mkexp loc (
        Pexp_function (( "?" ^ lab ), ( Some ( expr e1 ) ), [(( patt p ), (
          when_expr e2 w ))]) )
  | Ast.ExFun (loc,Ast.McArr (_,PaOlb (_,lab,p),w,e)) ->
      let lab = paolab lab p in
      mkexp loc (
        Pexp_function (( "?" ^ lab ), None, [(( patt_of_lab loc lab p ), (
          when_expr e w ))]) )
  | ExFun (loc,a) ->
  ...
  | Ast.BiEq
      ...
      let e =
        mkexp ( Pexp_constraint (( expr e ), ( Some ( ctyp ty ) ), None) ) in
  ...
  match w with
  | Ast.ExNil _ -> expr e
  | w -> mkexp ( loc_of_expr w ) ( Pexp_when (( expr w ), ( expr e )) ) and
  mklabexp x acc =
  match x with
  ...
  | Ast.RbEq (_,Ast.IdLid (sloc,s),e) -> (( with_loc s sloc ), ( expr e )) ::
      acc
  ...
  | SgTyp (loc,tdl) -> ( mksig loc ( Psig_type ( mktype_decl tdl [] ) ) ) ::
      l
  | SgVal (loc,n,t) -> (
  ...
  | StTyp (loc,tdl) -> ( mkstr loc ( Pstr_type ( mktype_decl tdl [] ) ) ) ::
      l
  | StVal (loc,rf,bi) -> (
  ...
  match c with
  | Ast.CgNil _ -> l
  | CgCtr (loc,t1,t2) -> ( mkctf loc ( Pctf_cstr (( ctyp t1 ), ( ctyp t2 )) )
      ) :: l
  ...
  match c with
  | CrNil _ -> l
  | CrCtr (loc,t1,t2) -> ( mkcf loc ( Pcf_constr (( ctyp t1 ), ( ctyp t2 )) )
      ) :: l
  ...
  | CrVal (loc,s,ov,mf,e) -> (
      mkcf loc (
        Pcf_val (( with_loc s loc ), ( mkmutable mf ), ( override_flag loc ov
          ), ( expr e )) )
      ) :: l
  | CrVir (loc,s,pf,t) -> (
      mkcf loc (
