...
let constructors_arity = fun ()  -> FanConfig.constructors_arity.contents
...
let mkvariant =
                       ...
                       | Ast.TyOf (loc, Ast.TyId (_, Ast.IdUid (sloc, s)), t) ->
                          (( (with_loc ( (conv_con s) ) sloc) ), (
                           ...
                           loc)
                       | Ast.TyCol
                          (loc, Ast.TyId (_, Ast.IdUid (sloc, s)),
                           Ast.TyArr (_, t, u)) ->
                          (( (with_loc ( (conv_con s) ) sloc) ), (
                           (List.map ctyp ( (list_of_ctyp t [] ) )) ), (
                           (Some (ctyp u)) ), loc)
                       | Ast.TyCol
                          (loc, Ast.TyId (_, Ast.IdUid (sloc, s)), t) ->
                          (( (with_loc ( (conv_con s) ) sloc) ), [] , (
                           (Some (ctyp t)) ), loc)
                       | _ -> assert false
...
let rec expr =
                                                        ...
                                                        | ExAss (loc, e, v) ->
                                                           let e =
                                                            (match e with
                                                             ...
                                                             | _ ->
                                                                (error loc
                                                                  "bad left part of assignment")) in
                                                           (mkexp loc e)
                                                        | ExAsr (loc, e) ->
                                                           (mkexp loc (
                                                             (Pexp_assert
                                                        ...
                                                        | ExLab (loc, _, _) ->
                                                           (error loc
                                                             "labeled expression not allowed here")
                                                        | ExLaz (loc, e) ->
                                                           (mkexp loc (
                                                             (Pexp_lazy
                                                               (expr e)) ))
                                                       ...
                                                       and module_type =
                                                        ...
                                                        | Ast.MtNil (loc) ->
                                                           (error loc
                                                             "abstract/nil module type not allowed here")
                                                        | Ast.MtId (loc, i) ->
                                                           (mkmty loc (
                                                             (Pmty_ident
                                                               (long_uident
                                                        ...
                                                        | Ast.MtQuo (loc, _) ->
                                                           (error loc
                                                             "module type variable not allowed here")
                                                        | Ast.MtSig (loc, sl) ->
                                                           (mkmty loc (
                                                             (Pmty_signature
                                                               (sig_item sl
                                                       ...
                                                       and class_type =
                                                        ...
                                                        | CtCon
                                                           (loc, _, _, _) ->
                                                           (error loc
                                                             "invalid virtual class inside a class type")
                                                        | ((((CtAnt (_, _)
                                                              | CtEq
                                                                 (_, _, _))
                                                             | CtCol
                                                       ...
                                                       and class_expr =
                                                        ...
                                                        | CeCon
                                                           (loc, _, _, _) ->
                                                           (error loc
                                                             "invalid virtual class inside a class expression")
                                                        | (((CeAnt (_, _)
                                                             | CeEq (_, _, _))
                                                            | CeAnd (_, _, _))
                                                           | CeNil (_)) ->
