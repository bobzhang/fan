...
let rec  patt =
  ...
  |
    ((((PaEq(_ , _ , _ ) |PaSem(_ , _ , _ )) |PaCom(_ , _ , _ )) |PaNil(_))
      as p) -> (error ( (loc_of_patt p ) ) "invalid pattern" ) and mklabpat =
...
let rec  expr =
  ...
  |
    ...
    let  e =
    (mkexp (
      Pexp_constraint ((( (expr e ) ) , ( Some ((ctyp ty )) ) , None  )) ) )
    in
  ...
  | w ->
    (mkexp ( (loc_of_expr w ) ) (
      Pexp_when ((( (expr w ) ) , ( (expr e ) ) )) ) )) and mklabexp (x)
  (acc) =
  ...
  |
    ((((CtAnt(_ , _ ) |CtEq(_ , _ , _ )) |CtCol(_ , _ , _ ))
       |CtAnd(_ , _ , _ )) |CtNil(_)) -> assert false and
  class_info_class_expr (ci) =
