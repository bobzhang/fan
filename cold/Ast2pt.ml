...
let long_uident_noloc ?(conv_con=(fun x -> x))  i = begin match (ident_tag i)
  with
...
let rec ctyp =
  ...
  | TyOfAmp (_,_,_)|TyAmp (_,_,_)|TySta (_,_,_)|TyCom (_,_,_)|TyVrn
    (_,_)|TyQuM (_,_)|TyQuP (_,_)|TyDcl (_,_,_,_,_)|TyAnP _|TyAnM _|TyTypePol
    (_,_,_)|TyObj (_,_,RvAnt _)|TyNil _|TyTup (_,_) ->   assert false) and
  row_field =
  (function
  | Ast.TyNil _ ->   []
  ...
  | t ->   [Rinherit ((ctyp t))]) and meth_list fl acc = begin match fl with
  | Ast.TyNil _ ->   acc
  ...
  | _ ->   assert false end and package_type_constraints wc acc = begin match
  wc with
  | Ast.WcNil _ ->   acc
...
let rec expr =
  ...
  | Ast.MtAnt (_,_) ->   assert false) and sig_item s l = begin match s with
  | Ast.SgNil _ ->   l
  | SgCls (loc,cd) ->
      (mksig loc (
        Psig_class
  ...
  | _ ->   assert false end and module_str_binding x acc = begin match x with
  | Ast.MbAnd (_,x,y) ->
