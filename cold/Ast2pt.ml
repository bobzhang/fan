     ...
     fun acc ->
      (match i with
       ...
       | Ast.IdApp (_, i1, i2) ->
          let i' =
           (Lapply
             (( (fst ( (self i1 None ) )) ), ( (fst ( (self i2 None ) )) ))) in
          let x =
           (match acc with
            | None -> i'
            | _ ->
...
let ident_noloc =
                    fun ?conv_lid ->
                     fun i -> (fst ( (ident_tag ?conv_lid:conv_lid i) ))
...
let rec ctyp =
           ...
           | Ast.TyObj (loc, fl, Ast.RvRowVar) ->
              (mktyp loc (
                (Ptyp_object
                  (meth_list fl ( [( (mkfield loc Pfield_var ) )] ))) ))
           | TyCls (loc, id) ->
           ...
           | TyPol (loc, t1, t2) ->
              (mktyp loc (
                (Ptyp_poly (( (ty_var_list_of_ctyp t1) ), ( (ctyp t2) ))) ))
           ...
           | Ast.TyVrnInfSup (loc, t, t') ->
              (mktyp loc (
                (Ptyp_variant
                  (( (row_field t) ), true , ( (Some (name_tags t')) ))) ))
           | TyAnt (loc, _) -> (error loc "antiquotation not allowed here")
           | (((((((((((((TyOfAmp (_, _, _) | TyAmp (_, _, _))
                         | TySta (_, _, _)) | TyCom (_, _, _))
                       | TyVrn (_, _)) | TyQuM (_, _)) | TyQuP (_, _))
                    | TyDcl (_, _, _, _, _)) | TyAnP (_)) | TyAnM (_))
                 | TyTypePol (_, _, _)) | TyObj (_, _, RvAnt (_)))
               | TyNil (_)) | TyTup (_, _)) ->
              assert false
          and row_field =
           function
           ...
           | Ast.TyOf (_, Ast.TyVrn (_, i), t) ->
              [(
               (Rtag
                 (i, false , ( (List.map ctyp ( (list_of_ctyp t [] ) )) )))
               )]
           | Ast.TyOr (_, t1, t2) ->
              (( (row_field t1) ) @ ( (row_field t2) ))
           | t -> [( (Rinherit (ctyp t)) )]
          and name_tags =
           function
           | Ast.TyApp (_, t1, t2) ->
              (( (name_tags t1) ) @ ( (name_tags t2) ))
           | Ast.TyVrn (_, s) -> [s]
           | _ -> assert false
          and meth_list =
           fun fl ->
          ...
          and package_type =
           (function
            ...
            | mt ->
               (error ( (loc_of_module_type mt) ) "unexpected package type") :
...
let rec type_decl =
                                                                   fun tl ->
                                                                    ...
                                                                    (error
                                                                    loc
                                                                    "only one manifest type allowed by definition")
                                                                    )
                                                                    else
                                                                    let m =
                                                                    (match
...
let type_decl =
 fun tl -> fun cl -> fun t -> fun loc -> (type_decl tl cl loc None  false  t)
                           ...
                           fun t ->
                            fun acc ->
                             (match t with
                              | Ast.TyCom (_, t1, t2) ->
                                 (class_parameters t1 (
                                   (class_parameters t2 acc) ))
                              | Ast.TyQuP (loc, s) ->
                                 (
                                  (( (with_loc s loc) ), (true , false )) ) ::
                                  acc 
                              | Ast.TyQuM (loc, s) ->
                                 (
                                  (( (with_loc s loc) ), (false , true )) ) ::
                                  acc 
                              | Ast.TyQuo (loc, s) ->
                                 (
                                  (( (with_loc s loc) ), (false , false )) ) ::
                                  acc 
                              | _ -> assert false)
                             ...
                             (mkpat loc (
                               (Ppat_or
                                 ((
                                  ...
                                  (
                                  (deep_mkrangepat loc (
                                    (Char.chr ( (( (Char.code c1) ) + 1) )) )
                                    c2) ))) ))

let rec patt =
                                                 function
                                                 | Ast.PaId
                                                    (loc, Ast.IdLid (sloc, s)) ->
                                                    (mkpat loc (
                                                      (Ppat_var
                                                        (with_loc s sloc)) ))
                                                 | Ast.PaId (loc, i) ->
                                                 ...
                                                 | PaAli (loc, p1, p2) ->
                                                    let (p, i) =
                                                     (match (p1, p2) with
                                                      ...
                                                      | _ ->
                                                         (error loc
                                                           "invalid alias pattern")) in
                                                    (mkpat loc (
                                                      (Ppat_alias
                                                        (( (patt p) ), i)) ))
                                                 | PaAnt (loc, _) ->
                                                    (error loc
                                                      "antiquotation not allowed here")
                                                 | PaAny (loc) ->
                                                    (mkpat loc Ppat_any )
                                                 | Ast.PaApp
                                                    (loc,
                                                 ...
                                                 | PaLab (loc, _, _) ->
                                                    (error loc
                                                      "labeled pattern not allowed here")
                                                 | (PaOlb (loc, _, _)
                                                    | PaOlbi (loc, _, _, _)) ->
                                                    (error loc
                                                      "labeled pattern not allowed here")
                                                 ...
                                                 | PaRng (loc, p1, p2) ->
                                                    (match (p1, p2) with
                                                     ...
                                                     | _ ->
                                                        (error loc
                                                          "range pattern allowed only for characters"))
                                                 | PaRec (loc, p) ->
                                                    let ps =
                                                     (list_of_patt p [] ) in
                                                    let is_wildcard =
...
let rec expr_fa =
 fun al ->
  function | ExApp (_, f, a) -> (expr_fa ( ( a ) :: al  ) f) | f -> (f, al)
...
let rec expr =
             ...
             (match (sep_expr_acc []  e) with
              | ((loc, ml, Ast.ExId (sloc, Ast.IdUid (_, s))) :: l) ->
                 ...
                 ((
                  (mkexp loc (
                    (Pexp_construct
                      (( (mkli sloc ( (conv_con s) ) ml) ), None , ca)) )) ),
                  l)
                 ...
                 (match e2 with
                  ...
                  | _ ->
                     (error ( (loc_of_expr e2) )
                       "lowercase identifier expected")) ) (loc, e) l) in
            e
         ...
         | ExArr (loc, e) ->
            (mkexp loc (
              (Pexp_array (List.map expr ( (list_of_expr e [] ) ))) ))
           ...
           (match x with
            ...
            | Ast.BiEq
               ...
               let e =
                (mkexp (
                  (Pexp_constraint
                    (( (expr e) ), ( (Some (ctyp ty)) ), None )) )) in
               let rec mk_newtypes =
                fun x ->
                 (match x with
                  | (newtype :: []) ->
                     (mkexp ( (Pexp_newtype (newtype, e)) ))
                  | (newtype :: newtypes) ->
                     (mkexp (
            ...
            | Ast.BiEq (_loc, p, Ast.ExTyc (_, e, Ast.TyPol (_, vs, ty))) ->
               (
                ((
                 (patt (
                   (Ast.PaTyc (_loc, p, ( (Ast.TyPol (_loc, vs, ty)) ))) ))
                 ), ( (expr e) )) ) :: acc 
        ...
        and module_type =
         ...
         | Ast.MtWit (loc, mt, wc) ->
            (mkmty loc (
              (Pmty_with (( (module_type mt) ), ( (mkwithc wc [] ) ))) ))
         | Ast.MtOf (loc, me) ->
         ...
         fun s ->
          fun l ->
           (match s with
            ...
            | Ast.SgExc
               (loc, Ast.TyOf (_, Ast.TyId (_, Ast.IdUid (_, s)), t)) ->
               ( (
                (mksig loc (
                  (Psig_exception
            ...
            | SgVal (loc, n, t) ->
               ( (
                (mksig loc (
                  (Psig_value
                    (( (with_loc n loc) ), ( (mkvalue_desc loc t [] ) ))) ))
                ) ) :: l 
        ...
        and class_expr =
         ...
         | CeFun (loc, p, ce) ->
            (mkcl loc (
              (Pcl_fun ("", None , ( (patt p) ), ( (class_expr ce) ))) ))
         | CeLet (loc, rf, bi, ce) ->
            (mkcl loc (
              (Pcl_let
                (( (mkrf rf) ), ( (binding bi [] ) ), ( (class_expr ce) )))
              ))
         | CeStr (loc, po, cfl) ->
            let p =
             (match po with | Ast.PaNil (_) -> (Ast.PaAny (loc)) | p -> p) in
