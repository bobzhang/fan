...
let rec  ctyp =
  ...
  |
    (((((((((((((TyOfAmp(_,_,_) |TyAmp(_,_,_)) |TySta(_,_,_)) |TyCom(_,_,_))
       ...
       |TyNil(_)) |TyTup(_,_)) -> assert false and row_field =
...
let rec  patt =
  ...
  | (PaApp(loc,_,_) as f) ->
    ...
    | Ppat_variant(s,None ) ->
      ...
      let  a =
      if (constructors_arity () ) then ( (mkpat loc ( Ppat_tuple (al) )) )
      else 
       (match al with
       | a::[]  -> a
       | _ -> (mkpat loc ( Ppat_tuple (al) ))) in
...
let rec  expr =
  ...
  | (ExApp(loc,_,_) as f) ->
    ...
    | Pexp_variant(s,None ) ->
      ...
      let  a =
      if (constructors_arity () ) then ( (mkexp loc ( Pexp_tuple (al) )) )
      else 
       (match al with
       | a::[]  -> a
       | _ -> (mkexp loc ( Pexp_tuple (al) ))) in
  ...
  | CrVvr(loc,s,mf,t) ->
    (mkcf loc (
      Pcf_valvirt ((( (with_loc s loc) ),( (mkmutable mf) ),( (ctyp t) ))) ))::l
