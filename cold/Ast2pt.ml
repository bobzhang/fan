...
let  ident_noloc ?conv_lid  (i) =
(fst ( (ident_tag ?conv_lid:conv_lid i ) ) )
...
| t ->
   if (m <> None ) then
    ...
    let  m = (match t with | Ast.TyNil(_) -> (None) | _ -> (Some(ctyp t )))
...
(id , (
 (pwith_type
   {ptype_params = params ; ptype_cstrs = []  ; ptype_kind = kind ;
    ptype_private = priv ; ptype_manifest = ( (Some(ct)) ) ; ptype_loc = loc
    ; ptype_variance = variance } ) ) )
...
| Ast.PaId(loc , i ) ->
   let  p =
   (Ppat_construct(( (long_uident ~conv_con:conv_con i ) ) , None  , (
   ...
   (match (patt f ).ppat_desc with
    | Ppat_construct(li , None , _ ) ->
       if (constructors_arity ()  ) then
        (
        (mkpat loc (
          (Ppat_construct(li , ( (Some(mkpat loc ( (Ppat_tuple(al)) ) )) ) ,
                          true  )) ) )
        )
       else
        let  a =
   ...
   (match (expr f ).pexp_desc with
    | Pexp_construct(li , None , _ ) ->
       ...
       if (constructors_arity ()  ) then
        (
        (mkexp loc (
          (Pexp_construct(li , ( (Some(mkexp loc ( (Pexp_tuple(al)) ) )) ) ,
                          true  )) ) )
        )
       else
        let  a =
...
| ExCoe(loc , e , t1 , t2 ) ->
   let  t1 = (match t1 with | Ast.TyNil(_) -> (None) | t -> (Some(ctyp t )))
   in
...
| Ast.ExId(loc , Ast.IdUid(_ , s ) ) ->
   (mkexp loc (
     (Pexp_construct(( (lident_with_loc ( (conv_con s ) ) loc ) ) , None  ,
                     true  )) ) )
| ExVrn(loc , s ) ->
...
| ((ExId(_ , _ ) | ExNil(_)) as e) ->
   (error ( (loc_of_expr e ) ) "invalid expr" ) and patt_of_lab (_loc) (lab)
=
function
| Ast.PaNil(_) ->
...
(match x with
 ...
 | Ast.BiEq(_loc , Ast.PaId(sloc , Ast.IdLid(_ , bind_name ) ) ,
...
(match x with
 ...
 | Ast.McNil(_) -> acc
 | _ -> assert false) and when_expr (e) (w) =
(match w with
 | Ast.ExNil(_) -> (expr e )
 | w ->
...
(match x with
 ...
 | _ -> assert false) and mktype_decl (x) (acc) =
(match x with
    ...
    (
     (( (with_loc c cloc ) ) , (
      (type_decl ( (List.fold_right optional_type_parameters tl []  ) ) cl td
        cloc ) ) ) ) :: acc 
 | _ -> assert false) and module_type =
function
...
(match s with
 ...
 | SgCls(loc , cd ) ->
    ( (
     (mksig loc (
       (Psig_class(List.map class_info_class_type (
                    (list_of_class_type cd []  ) ) )) ) ) ) ) :: l 
 | SgClt(loc , ctd ) ->
    ( (
     (mksig loc (
       (Psig_class_type(List.map class_info_class_type (
                         (list_of_class_type ctd []  ) ) )) ) ) ) ) :: l 
...
(match s with
 ...
 | StCls(loc , cd ) ->
    ( (
     (mkstr loc (
       (Pstr_class(List.map class_info_class_expr (
                    (list_of_class_expr cd []  ) ) )) ) ) ) ) :: l 
 | StClt(loc , ctd ) ->
    ( (
     (mkstr loc (
       (Pstr_class_type(List.map class_info_class_type (
                         (list_of_class_type ctd []  ) ) )) ) ) ) ) :: l 
 ...
 | Ast.StExc(loc , Ast.TyOf(_ , Ast.TyId(_ , Ast.IdUid(_ , s ) ) , t ) ,
             Ast.ONone ) ->
    ( (
     (mkstr loc (
       (Pstr_exception(( (with_loc ( (conv_con s ) ) loc ) ) , (
 ...
 | Ast.StExc(loc , Ast.TyOf(_ , Ast.TyId(_ , Ast.IdUid(_ , _ ) ) , _ ) ,
             Ast.OSome(_) ) ->
...
| CtSig(loc , t_o , ctfl ) ->
   ...
   (mkcty loc (
     (Pcty_signature({pcsig_self = ( (ctyp t ) ) ; pcsig_fields = cil ;
                      pcsig_loc = loc })) ) )
| CtCon(loc , _ , _ , _ ) ->
...
| (((CeAnt(_ , _ ) | CeEq(_ , _ , _ )) | CeAnd(_ , _ , _ )) | CeNil(_)) ->
   assert false and class_str_item (c) (l) =
(match c with
 | CrNil(_) -> l
 | CrCtr(loc , t1 , t2 ) ->
