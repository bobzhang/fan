...
let  mkdirection =
  ...
  (function
  | Ast.DiTo  -> Upto | Ast.DiDownto  -> Downto | _ -> assert false)
let  conv_con =
...
let  mkrf =
  ...
  (function
  | Ast.ReRecursive  -> Recursive | Ast.ReNil  -> Nonrecursive
    | _ -> assert false)
let  ident_tag ?(conv_lid=(fun (x) -> x))  (i) =
  ...
  | Ast.IdAcc(_,Ast.IdLid(_,"*predef*"),Ast.IdLid(_,"option")) ->
    ...
    | Ast.IdApp(_,i1,i2) ->
      ...
      (match (( (self i1 None ) ),( (self i2 None ) ),acc)
      with
...
let  long_uident_noloc ?(conv_con=(fun (x) -> x))  (i) =
  ...
  | (Ldot(i,s),`uident) -> (ldot i ( (conv_con s) ))
    | (Lident(s),`uident) -> (lident ( (conv_con s) )) | (i,`app) -> i
...
let rec  ctyp =
  ...
  | TyId(loc,i) ->
    ...
    | TyAli(loc,t1,t2) ->
      ...
      (match (t1,t2)
      with
      | (t,TyQuo(_,s)) -> (t,s) | (TyQuo(_,s),t) -> (t,s)
    ...
    |
      (((((((((((((TyOfAmp(_,_,_) |TyAmp(_,_,_)) |TySta(_,_,_))
                  |TyCom(_,_,_)) |TyVrn(_,_)) |TyQuM(_,_)) |TyQuP(_,_))
              |TyDcl(_,_,_,_,_)) |TyAnP(_)) |TyAnM(_)) |TyTypePol(_,_,_))
          |TyObj(_,_,RvAnt(_))) |TyNil(_)) |TyTup(_,_)) -> assert false) and
  row_field =
  
  (function
  | Ast.TyNil(_) -> [] | Ast.TyVrn(_,i) -> [Rtag ((i,true ,[] ))]
    ...
    | t -> [Rinherit ((ctyp t))]) and meth_list (fl) (acc) =
  ...
  | Ast.TyNil(_) -> acc
    ...
    | _ -> assert false) and package_type_constraints (wc) (acc) =
...
let  mkprivate =
  ...
  (function
  | Ast.PrPrivate  -> Private | Ast.PrNil  -> Public | _ -> assert false)
let  mktrecord =
...
let rec  list_of_meta_list =
  ...
  (function
  | Ast.LNil  -> [] | Ast.LCons(x,xs) -> x::(list_of_meta_list xs)
    | Ast.LAnt(_) -> assert false)
let  mkmutable =
  
  (function
  | Ast.MuMutable  -> Mutable | Ast.MuNil  -> Immutable | _ -> assert false)
let  paolab (lab) (p) =
  ...
  |
    ("",(Ast.PaId(_,Ast.IdLid(_,i))
          |Ast.PaTyc(_,Ast.PaId(_,Ast.IdLid(_,i)),_))) -> i
    | ("",p) -> (error ( (loc_of_patt p) ) "bad ast in label") | _ -> lab)
...
let rec  type_parameters (t) (acc) =
  ...
  | Ast.TyApp(_,t1,t2) -> (type_parameters t1 ( (type_parameters t2 acc) ))
    ...
    | Ast.TyQuo(_,s) -> (s,(false ,false ))::acc | _ -> assert false)
let rec  optional_type_parameters (t) (acc) =
  ...
  | Ast.TyApp(_,t1,t2) ->
    ...
    | Ast.TyAny(_loc) -> (None ,(false ,false ))::acc | _ -> assert false)
...
let rec  type_parameters_and_type_name (t) (acc) =
  ...
  | Ast.TyApp(_,t1,t2) ->
    ...
    | Ast.TyId(_,i) -> (( (ident i) ),acc) | _ -> assert false)
    ...
    | Ast.PaId(loc,i) ->
      ...
      Ppat_construct
        ((( (long_uident ~conv_con:conv_con i) ),None ,(
          (constructors_arity () ) ))) in (mkpat loc p)
    | PaAli(loc,p1,p2) ->
...
let  override_flag (loc) =
  ...
  (function
  | Ast.OvOverride  -> Override | Ast.OvNil  -> Fresh
...
let rec  expr =
  ...
  | ((ExAcc(loc,_,_) |Ast.ExId(loc,Ast.IdAcc(_,_,_))) as e) ->
        ...
        | ExSte(loc,e1,e2) ->
          Pexp_apply
            (((
              ...
              [("",( (expr e1) ));("",( (expr e2) ));("",( (expr v) ))] )))
        | _ -> (error loc "bad left part of assignment")) in (mkexp loc e)
    ...
    | ExCoe(loc,e,t1,t2) ->
      ...
      let  t1 = 
      (match t1 with
      | Ast.TyNil(_) -> None | t -> Some ((ctyp t))) in
    ...
    | ExObj(loc,po,cfl) ->
      ...
      let  p = 
      (match po with
      | Ast.PaNil(_) -> Ast.PaAny (loc) | p -> p) in
    ...
    | ExRec(loc,lel,eo) ->
      ...
      | Ast.RbNil(_) -> (error loc "empty record")
        | _ ->
          ...
          (match eo with
          | Ast.ExNil(_) -> None | e -> Some ((expr e))) in
  ...
  | Ast.BiAnd(_,x,y) -> (binding x ( (binding y acc) ))
    ...
    | Ast.BiEq(_loc,p,Ast.ExTyc(_,e,Ast.TyPol(_,vs,ty))) ->
      (( (patt ( Ast.PaTyc ((_loc,p,( Ast.TyPol ((_loc,vs,ty)) ))) )) ),(
       (expr e) ))::acc | Ast.BiEq(_,p,e) -> (( (patt p) ),( (expr e) ))::acc
    | Ast.BiNil(_) -> acc | _ -> assert false) and match_case (x) (acc) =
  
  (match x
  with
  | Ast.McOr(_,x,y) -> (match_case x ( (match_case y acc) ))
    | Ast.McArr(_,p,w,e) -> (( (patt p) ),( (when_expr e w) ))::acc
    | Ast.McNil(_) -> acc | _ -> assert false) and when_expr (e) (w) =
  ...
  | Ast.RbNil(_) -> acc
    ...
    | Ast.RbEq(_,Ast.IdLid(sloc,s),e) ->
      (( (with_loc s sloc) ),( (expr e) ))::acc | _ -> assert false) and
  mktype_decl (x) (acc) =
    ...
    | Ast.TyDcl(cloc,c,tl,td,cl) ->
      ...
      (( (with_loc c cloc) ),(
       (type_decl ( (List.fold_right optional_type_parameters tl [] ) ) cl td
         cloc) ))::acc | _ -> assert false) and module_type =
  
  (function
  | Ast.MtNil(loc) -> (error loc "abstract/nil module type not allowed here")
    ...
    | Ast.MtAnt(_,_) -> assert false) and sig_item (s) (l) =
  ...
  | Ast.SgNil(_) -> l
    ...
    | SgClt(loc,ctd) ->
      (mksig loc (
        ...
        ))::l | Ast.SgSem(_,sg1,sg2) -> (sig_item sg1 ( (sig_item sg2 l) ))
    | SgDir(_,_,_) -> l
    ...
    | Ast.SgAnt(loc,_) -> (error loc "antiquotation in sig_item")) and
  module_sig_binding (x) (acc) =
  
  (match x
  with
  | Ast.MbAnd(_,x,y) -> (module_sig_binding x ( (module_sig_binding y acc) ))
    | Ast.MbCol(loc,s,mt) -> (( (with_loc s loc) ),( (module_type mt) ))::acc
    | _ -> assert false) and module_str_binding (x) (acc) =
  ...
  | Ast.MbAnd(_,x,y) -> (module_str_binding x ( (module_str_binding y acc) ))
    ...
    | _ -> assert false) and module_expr =
  ...
  | Ast.StNil(_) -> l
    ...
    | StClt(loc,ctd) ->
      (mkstr loc (
        Pstr_class_type
          ((List.map class_info_class_type ( (list_of_class_type ctd [] ) )))
        ))::l | Ast.StSem(_,st1,st2) -> (str_item st1 ( (str_item st2 l) ))
    | StDir(_,_,_) -> l
    ...
    | Ast.StExc(loc,Ast.TyOf(_,Ast.TyId(_,Ast.IdUid(_,_)),_),Ast.OSome(_)) ->
      (error loc "type in exception alias") | StExc(_,_,_) -> assert false
    ...
    | Ast.StAnt(loc,_) -> (error loc "antiquotation in str_item")) and
  class_type =
  ...
  | CtCon(loc,ViNil ,id,tl) ->
    ...
    | CtSig(loc,t_o,ctfl) ->
      ...
      let  t = 
      (match t_o with
      | Ast.TyNil(_) -> Ast.TyAny (loc) | t -> t) in
    ...
    | ((((CtAnt(_,_) |CtEq(_,_,_)) |CtCol(_,_,_)) |CtAnd(_,_,_)) |CtNil(_))
      -> assert false) and class_info_class_expr (ci) =
  ...
  |
    ...
    | ct ->
      (error ( (loc_of_class_type ct) )
        "bad class/class type declaration/definition")) and class_sig_item
  (c) (l) =
  ...
  | Ast.CgNil(_) -> l
    ...
    | CgAnt(_,_) -> assert false) and class_expr =
  ...
  | CrNil(_) -> l
    ...
    | CrMth(loc,s,ov,pf,e,t) ->
      ...
      (match t
      with
      | Ast.TyNil(_) -> None | t -> Some ((mkpolytype ( (ctyp t) )))) in
    ...
    | CrVvr(loc,s,mf,t) ->
      (mkcf loc (
        ...
        ))::l | CrAnt(_,_) -> assert false)
let  sig_item (ast) = (sig_item ast [] )
let  str_item (ast) = (str_item ast [] )
let  directive =
  
  (function
  | Ast.ExNil(_) -> Pdir_none | ExStr(_,s) -> Pdir_string (s)
