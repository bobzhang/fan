...
let rec ctyp =
  ...
  | Ast.TyVrnEq (loc,t) -> mktyp loc (Ptyp_variant ((row_field t),true,None))
  | Ast.TyVrnSup (loc,t) ->
  ...
  | TyOfAmp (_,_,_)|TyAmp (_,_,_)|TySta (_,_,_)|TyCom (_,_,_)|TyVrn
      (_,_)|TyQuM (_,_)|TyQuP (_,_)|TyDcl (_,_,_,_,_)|TyAnP _|TyAnM
      _|TyTypePol (_,_,_)|TyObj (_,_,RvAnt _)|TyNil _|TyTup (_,_) -> assert
      false and row_field =
  function
  | Ast.TyNil _ -> []
...
let rec deep_mkrangepat loc c1 c2 =
  ...
  then mkghpat loc (Ppat_constant (Const_char c1))
    ...
    mkghpat loc
      (Ppat_or
         ((mkghpat loc (Ppat_constant (Const_char c1))),(deep_mkrangepat loc
                                                           (Char.chr
                                                              ((Char.code c1)
                                                                 + 1)) c2)))
let rec mkrangepat loc c1 c2 =
  ...
  then mkrangepat loc c2 c1
    ...
    then mkpat loc (Ppat_constant (Const_char c1))
      ...
      mkpat loc
        (Ppat_or
           ((mkghpat loc (Ppat_constant (Const_char c1))),(deep_mkrangepat
                                                             loc
                                                             (Char.chr
                                                                ((Char.code
                                                                    c1)
...
let rec patt =
  ...
  | Ast.PaApp
      ...
      mkpat loc
        (Ppat_construct
           ((lident_with_loc (conv_con s) sloc),(Some
                                                   (mkpat loc_any Ppat_any)),false))
  | PaApp (loc,_,_) as f ->
...
let rec expr =
      ...
      let (e,l) =
        (match Expr.sep_expr [] e with
         | (loc,ml,Ast.ExId (sloc,Ast.IdUid (_,s)))::l ->
             ...
             ((mkexp loc
                 (Pexp_construct ((mkli sloc (conv_con s) ml),None,ca))),l)
  ...
  | ExFor (loc,i,e1,e2,df,el) ->
      ...
      mkexp loc
        (Pexp_for
           ((with_loc i loc),(expr e1),(expr e2),(mkdirection df),(expr e3)))
  ...
  | ExLmd (loc,i,me,e) ->
      mkexp loc (Pexp_letmodule ((with_loc i loc),(module_expr me),(expr e)))
  ...
  | Ast.ExOpI (loc,i,e) -> mkexp loc (Pexp_open ((long_uident i),(expr e)))
  | Ast.ExPkg (loc,Ast.MeTyc (_,me,pt)) ->
      mkexp loc
        (Pexp_constraint
           ((mkexp loc (Pexp_pack (module_expr me))),(Some
                                                        (mktyp loc
                                                           (Ptyp_package
                                                              (package_type
                                                                 pt)))),None))
  ...
  | Ast.BiEq
      ...
      let pat =
        mkpat
          (Ppat_constraint
             ((mkpat (Ppat_var (with_loc bind_name sloc))),(mktyp _loc
                                                              (Ptyp_poly
                                                                 (ampersand_vars,ty'))))) in
      let e = mk_newtypes vars in (pat,e)::acc
  | Ast.BiEq (_loc,p,Ast.ExTyc (_,e,Ast.TyPol (_,vs,ty))) ->
      ((patt (Ast.PaTyc (_loc,p,(Ast.TyPol (_loc,vs,ty))))),(expr e))::acc
  ...
  match w with
  | Ast.ExNil _ -> expr e
  | w -> mkexp (loc_of_expr w) (Pexp_when ((expr w),(expr e))) and mklabexp x
  acc =
  match x with
  ...
  | Ast.SgExc (loc,Ast.TyOf (_,Ast.TyId (_,Ast.IdUid (_,s)),t)) ->
      (mksig loc
         (Psig_exception
            ((with_loc (conv_con s) loc),(List.map ctyp (list_of_ctyp t [])))))::l
  | SgExc (_,_) -> assert false
  | SgExt (loc,n,t,sl) ->
      (mksig loc
         (Psig_value
            ((with_loc n loc),(mkvalue_desc loc t (list_of_meta_list sl)))))::l
  ...
  | Ast.MePkg (loc,Ast.ExTyc (_,e,Ast.TyPkg (_,pt))) ->
      mkmod loc
        (Pmod_unpack
           (mkexp loc
              (Pexp_constraint
                 ((expr e),(Some (mktyp loc (Ptyp_package (package_type pt)))),None))))
  ...
  | Ast.StAnt (loc,_) -> error loc "antiquotation in str_item" and class_type
  =
  function
  ...
  | CtAnt (_,_)|CtEq (_,_,_)|CtCol (_,_,_)|CtAnd (_,_,_)|CtNil _ -> assert
      false and class_info_class_expr ci =
  match ci with
  ...
  | CeFun (loc,PaLab (_,lab,po),ce) ->
      mkcl loc (Pcl_fun (lab,None,(patt_of_lab loc lab po),(class_expr ce)))
  ...
  | CrMth (loc,s,ov,pf,e,t) ->
      ...
      (mkcf loc
         (Pcf_meth ((with_loc s loc),(mkprivate pf),(override_flag loc ov),e)))::l
  | CrVal (loc,s,ov,mf,e) ->
      (mkcf loc
         (Pcf_val
  ...
  | CrVir (loc,s,pf,t) ->
      (mkcf loc
         (Pcf_virt ((with_loc s loc),(mkprivate pf),(mkpolytype (ctyp t)))))::l
  | CrVvr (loc,s,mf,t) ->
