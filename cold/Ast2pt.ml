...
let rec expr =
  ...
  match w with
  | Ast.ExNil _ -> expr e
  | w -> mkexp (loc_of_expr w) (Pexp_when ((expr w), (expr e))) and mklabexp
  x acc =
  match x with
  ...
  | Ast.StExc (loc,Ast.TyId (_,Ast.IdUid (_,s)),Ast.OSome i) ->
      (mkstr loc (Pstr_exn_rebind ((with_loc (conv_con s) loc), (ident i))))
      :: l
  ...
  | CrMth (loc,s,ov,pf,e,t) ->
      ...
      (mkcf loc
         (Pcf_meth ((with_loc s loc), (mkprivate pf), (override_flag loc ov),
            e)))
        :: l
  | CrVal (loc,s,ov,mf,e) ->
      (mkcf loc
