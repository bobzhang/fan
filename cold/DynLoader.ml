...
module Make (U:sig  end) : S =
  struct type  t =  string  Queue.t  
    ...
    let find_in_path (x) (name) =
      ...
      if (Sys.file_exists name) then begin name
      end else begin (raise Not_found )
      end
      end else begin
      let res =
        (fold_load_path x (
          (fun (dir) ->
            ...
            | None  ->
                ...
                if (Sys.file_exists fullname) then begin Some (fullname)
                end else begin None
                end
            | x ->   x)) ) None ) in
    ...
    let load =
      ...
      (fun (_path) ->
        (fun (file) ->
          ...
          let fname = begin try (find_in_path _path file) with
            | Not_found  ->
              (raise ( Error ((file,"file not found in path")) )) end in
          begin try (Dynlink.loadfile fname) with
            | Dynlink.Error(e) ->
              (raise ( Error ((fname,( (Dynlink.error_message e) ))) )) end
          end)) let is_native = Dynlink.is_native end 