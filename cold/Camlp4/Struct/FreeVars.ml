 ...
 functor (Ast : Sig.Camlp4Ast) ->
  struct
   ...
   class ['accu] fold_free_vars (f : (string -> ('accu -> 'accu)))
    ?env_init:(env_init=S.empty) free_init =
    object (o)
     inherit Ast.fold as super
     ...
     method add_patt = fun p -> {< env = (fold_pattern_vars S.add p env) >}
     method add_binding =
   ...
   let free_vars =
    fun env_init ->
     fun e ->
      let fold = ((new fold_free_vars) S.add ~env_init:env_init S.empty) in
      ((fold#expr e)#free)
