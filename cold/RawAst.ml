...
type ctyp =
  ...
  | `Quote of (loc* position_flag* alident meta_option) | `Par of (loc* ctyp)
...
and binding =
  [ nil | `And of (loc* binding* binding) | `Bind of (loc* pat* exp) | ant] 
and module_binding =
...
and class_exp =
  ...
  | `CeLet of (loc* rec_flag* binding* class_exp) | `Obj of (loc* pat* cstru)
