                ...
                struct
                 ...
                 let empty_entry =
                  fun ename ->
                   fun _ ->
                    (raise (
                      (Stream.Error ("entry [" ^ ( (ename ^ "] is empty") )))
                      ))
                          ...
                          (match (Stream.peek __strm) with
                           | Some (tok, cur_loc) ->
                              ...
                              (Stream.lcons (
                                fun _ ->
                                 (tok,
                                  {prev_loc = prev_loc; cur_loc = cur_loc;
                                   prev_loc_only = false }) ) (
                 ...
                 let get_prev_loc =
                  fun strm ->
                   let result =
                    (match (Stream.peek strm) with
                     | Some
                        (_, {prev_loc = prev_loc; prev_loc_only = true; _ }) ->
                        ( (Stream.junk strm) ); prev_loc
                     | Some
                        (_, {prev_loc = prev_loc; prev_loc_only = false; _ }) ->
                        prev_loc
                 ...
                 let rec get_token_list =
                  fun entry ->
                   fun tokl ->
                    fun last_tok ->
                     function
                     | Node ({node = ((Stoken (_) | Skeyword (_)) as tok);
                        son = son; brother = DeadEnd}) ->
                        (get_token_list entry ( ( last_tok ) :: tokl  ) tok
                          son)
                     | tree ->
                        if (tokl = [] ) then None 
                        else
                         (Some
                           (( (List.rev ( ( last_tok ) :: tokl  )) ),
                            last_tok, tree))
                     ...
                     fun s2 ->
                      (match (s1, s2) with
                       | (Snterm (e1), Snterm (e2)) ->
                          (( e1.ename ) = ( e2.ename ))
                       | (Snterm (e1), Sself) ->
                          (( e1.ename ) = ( entry.ename ))
                       | (Sself, Snterm (e2)) ->
                          (( entry.ename ) = ( e2.ename ))
