...
let keep_prev_loc (strm) = begin match (Stream.peek strm) with
  | None  ->   Stream.sempty
  | Some(_tok0,init_loc) ->
      let rec go (prev_loc) (strm1) =
        let (__strm : _ Stream.t ) = strm1 in begin match
      ...
      (go init_loc strm) end
let drop_prev_loc (strm) =
  (Stream.map ( (fun ((tok,r)) -> (tok,( r.cur_loc ))) ) strm)
let get_cur_loc (strm) = begin match (Stream.peek strm) with
  | Some(_,r) ->   r.cur_loc
  | None  ->   FanLoc.ghost end
let get_prev_loc (strm) =
  let result = begin match (Stream.peek strm) with
...
let logically_eq_symbols (entry) =
  let rec eq_symbols (s1) (s2) = begin match (s1,s2) with
    ...
    | _ ->   (s1 = s2) end and eq_trees (t1) (t2) = begin match (t1,t2) with
    | (Node(n1),Node(n2)) ->
