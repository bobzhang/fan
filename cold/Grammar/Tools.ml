  ...
  | None  -> Stream.sempty
    | Some(_tok0,init_loc) ->
      ...
      | Some(tok,cur_loc) ->
        ...
        end | _ -> Stream.sempty) in (go init_loc strm))
...
let  get_cur_loc (strm) =
  ...
  (match (Stream.peek strm)
  with
  | Some(_,r) -> r.cur_loc | None  -> FanLoc.ghost)
let  get_prev_loc (strm) =
  ...
  | Some(_,{prev_loc = prev_loc;prev_loc_only = true ;_}) ->
    ...
    (Stream.junk strm);
    prev_loc
    end | Some(_,{prev_loc = prev_loc;prev_loc_only = false ;_}) -> prev_loc
    | None  -> FanLoc.ghost) in result
let  is_level_labelled (n) =
  
  (function
  | {lname = Some(n1);_} -> (n = n1) | _ -> false)
...
let  eq_Stoken_ids (s1) (s2) =
  ...
  (match (s1,s2)
  with
  | ((`Antiquot,_),_) -> false | (_,(`Antiquot,_)) -> false
    | ((_,s1),(_,s2)) -> (s1 = s2))
let  logically_eq_symbols (entry) =
  ...
  | (Node(n1),Node(n2)) ->
    ...
    | ((LocAct(_,_) |DeadEnd ),(LocAct(_,_) |DeadEnd )) -> true | _ -> false)
  in eq_symbols
