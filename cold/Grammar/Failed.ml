...
let rec  name_of_symbol_failed (entry) =
  ...
  | Node({node = s;brother = bro;son = son}) ->
    ...
    | None  ->
      ...
      with
      | ((`Sopt _),Node(_)) ->
        (txt ^ ( (" or " ^ ( (name_of_tree_failed entry son) )) )) | _ -> txt)
      in
      ...
      | (DeadEnd  |LocAct(_,_)) -> txt
        | Node(_) ->
          (txt ^ ( (" or " ^ ( (name_of_tree_failed entry bro) )) ))) in txt
      | Some(tokl,_,_) ->
        (List.fold_left (
          (fun (s) ->
            (fun (tok) ->
              (( if (s = "") then begin
                ...
                with
                | (`Stoken (_,descr)) -> (name_of_descr descr)
                  | (`Skeyword kwd) -> kwd | _ -> assert false) )))) ) ""
          tokl)) | (DeadEnd  |LocAct(_,_)) -> "???")
...
let  tree_failed (entry) (prev_symb_result) (prev_symb) (tree) =
  ...
  | (`Slist0 s) ->
    ...
    | (`Slist0sep (s,sep)) ->
      ...
      (match (magic "tree_failed: 'a -> list 'b" prev_symb_result)
      with
      | []  ->
