...
module Dump = (Print.MakeDump)(struct end)
...
let parse =
 fun entry ->
  fun loc -> fun cs -> (filter_and_parse_tokens entry ( (lex entry loc cs) ))


let parse_string =
 fun entry ->
  fun loc ->
   fun str -> (filter_and_parse_tokens entry ( (lex_string entry loc str) ))
