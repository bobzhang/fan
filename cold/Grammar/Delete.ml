...
let  delete_rule_in_tree (entry) =
  ...
  | Some(Some(dsl),DeadEnd ) -> Some ((( Some (n.node::dsl) ),( n.brother )))
    ...
    | Some(None ,t) ->
      ...
      let  t = Node ({node = ( n.node );son = t;brother = ( n.brother )}) in
      Some ((None ,t)) | None  -> None) in delete_in_tree
let rec  decr_keyw_use (gram) =
  ...
  | (`Skeyword kwd) -> (removing gram kwd)
    ...
    | (`Slist1sep (s1,s2)) ->
      ...
      (decr_keyw_use gram s1);
      (decr_keyw_use gram s2)
      end | (`Stree t) -> (decr_keyw_use_in_tree gram t)
    ...
    | Some(dsl,t) ->
      ...
      | DeadEnd  when (( lev.lprefix ) == DeadEnd ) -> levs
        | _ ->
          ...
          let  lev =
          {assoc = ( lev.assoc );lname = ( lev.lname );lsuffix = t;lprefix =
                                                                    (
                                                                    lev.lprefix
                                                                    )} in
          lev::levs)
