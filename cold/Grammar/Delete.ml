                    ...
                    fun n ->
                     (match (delete_in_tree sl ( n.son )) with
                      | Some (Some (dsl), DeadEnd) ->
                         (Some
                           (( (Some (( ( n.node ) ) :: dsl )) ), ( n.brother
                            )))
                      | Some (Some (dsl), t) ->
                         let t =
                          (Node
                            ({node = ( n.node ); son = t;
                              brother = ( n.brother )})) in
...
let rec decr_keyw_use =
                                   fun gram ->
                                    ...
                                    | (`Slist1sep (s1, s2)) ->
                                       ...
                                       (decr_keyw_use gram s2)
                                    | (`Stree t) ->
                                       (decr_keyw_use_in_tree gram t)
                                    | ((((`Sself | `Snext) | (`Snterm _))
                                        | (`Snterml (_, _))) | (`Stoken _)) ->
                                       ()
                                                                 ...
                                                                 fun lev ->
                                                                  fun bp ->
                                                                   fun a ->
                                                                    fun strm ->
                                                                    let f =
                                                                    (Parser.continue_parser_of_entry
                                                                    entry) in
                                                                    (
                                                                    entry.econtinue
                                                                    <- f
