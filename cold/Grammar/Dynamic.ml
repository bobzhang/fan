               ...
               functor (Lexer : Sig.Lexer) ->
                struct
                 ...
                 let lex_string =
                  fun g ->
                   fun loc ->
                    fun str -> (lex g loc ( (Stream.of_string str) ))

                 let filter =
                  fun g ->
                   fun ts ->
                    (Tools.keep_prev_loc (
                      (Token.Filter.filter ( g.gfilter ) ts) ))

                 let filter_and_parse_tokens =
                  fun entry ->
                   fun ts ->
                    (parse_origin_tokens entry ( (filter ( entry.egram ) ts)
                      ))
                   ...
                   fun rl ->
                    let t =
                     (List.fold_left (
                       fun tree ->
                        fun (symbols, action) ->
                         (Insert.insert_tree e symbols action tree) ) DeadEnd
                        rl) in
                    (Stree (t))
