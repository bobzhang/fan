  ...
  | Dlevels(levels) ->
    ...
    | []  -> tree
      | level::levels ->
        ...
        with
        | Some(tree) -> tree | None  -> (search_levels levels))) and
    search_level (level) =
    ...
    |
      ...
      | (`Slist0 symb) ->
        ...
        with
        | Some(symb) -> Some (`Slist0 (symb)) | None  -> None)
      | (`Slist0sep (symb,sep)) ->
        ...
        | Some(symb) -> Some (`Slist0sep ((symb,sep)))
          | None  ->
            ...
            with
            | Some(sep) -> Some (`Slist0sep ((symb,sep))) | None  -> None))
      | (`Slist1 symb) ->
        
        (match (search_symbol symb)
        with
        | Some(symb) -> Some (`Slist1 (symb)) | None  -> None)
      | (`Slist1sep (symb,sep)) ->
        ...
        | Some(symb) -> Some (`Slist1sep ((symb,sep)))
          | None  ->
            ...
            (match (search_symbol sep)
            with
            | Some(sep) -> Some (`Slist1sep ((symb,sep))) | None  -> None))
      | (`Sopt symb) ->
        
        (match (search_symbol symb)
        with
        | Some(symb) -> Some (`Sopt (symb)) | None  -> None)
      | (`Stry symb) ->
        
        (match (search_symbol symb)
        with
        | Some(symb) -> Some (`Stry (symb)) | None  -> None)
      | (`Stree t) ->
        
        (match (search_tree t)
        with
        | Some(t) -> Some (`Stree (t)) | None  -> None) | _ -> None) in
    (search_levels levels) | Dparser(_) -> tree)