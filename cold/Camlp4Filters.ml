...
module MakeExceptionTracer (Syn:Sig.Camlp4Syntax) =
  struct module Ast  = Camlp4Ast
    ...
    let rec  map_match_case =
      ...
      | Ast.McOr(_loc , m1 , m2 ) ->
        Ast.McOr
          ((_loc , ( (map_match_case m1 ) ) , ( (map_match_case m2 ) ) ))
    ...
    let  filter =
      ...
      method !  expr =
        ...
        function
        | Ast.ExFun(_loc , m ) ->
          Ast.ExFun ((_loc , ( (map_match_case m ) ) ))
        | x -> (super#expr x )
      method !  str_item =
...
module MakeFoldGenerator (Syn:Sig.Camlp4Syntax) =
  struct module Ast  = Camlp4Ast let  _loc = FanLoc.ghost
    ...
    let  list_init (f) (n) =
      ...
      let rec  self (m) =
      if (m = n) then []  else (f m ) :: (self ( (succ m ) ) )  in (self 0 )
    ...
    let  builtin_types =
      ...
      let  abstr = ["string" ; "int" ; "float" ; "int32" ; "int64" ;
      "nativeint" ; "char"]  in
      (List.fold_right (
        fun (name) ->
          (SMap.add name
      ...
      (List.fold_right (
        fun (((name , _ , _ , _ , _ ) as decl)) -> (SMap.add name decl ) )
        concr tyMap ) in tyMap let  used_builtins = (ref SMap.empty )
    ...
    module Gen (X:sig val size : int val mode : mode  end) =
      struct let  size = X.size let  mode = X.mode
        let  tuplify_expr (f) =
          if (size <= 0) then ( assert false )
           ...
           if (k = 2) then ( (f 2 ) )
            ...
            Ast.ExCom ((_loc , ( (loop ( (k - 1) ) ) ) , ( (f k ) ) )) in
           Ast.ExTup
             ((_loc , (
        ...
        let  tuplify_patt (f) =
          if (size <= 0) then ( assert false )
           ...
           if (k = 2) then ( (f 2 ) )
            ...
            Ast.PaCom ((_loc , ( (loop ( (k - 1) ) ) ) , ( (f k ) ) )) in
           Ast.PaTup
             ((_loc , (
        ...
        let  chain_tuple (mkp) (mke) (expr_of_ty) (ts) =
          ...
          let  exi1s =
          (list_init ( fun (i) -> (exik i 1 ) ) ( (List.length ts ) ) )
          in
          
          let  pxi1s =
          (list_init ( fun (i) -> (pxik i 1 ) ) ( (List.length ts ) ) )
          in
        ...
        let rec  expr_of_ty (x) (ty) =
          ...
          Ast.PaId ((_loc , ( Ast.IdUid ((_loc , s )) ) )) and
          out_constr_expr (s) =
          ...
          else (method_type_of_type t t params []  ) and
          method_type_of_type (t_in) (t_out) (params_in) (params_out) =
    ...
    let  generate_class_implem (mode) (c) (tydcl) (n) =
      ...
      let module M = (Gen) (struct let  size = n let  mode = mode  end)
        in
    ...
    let  decorate (decorate_fun) =
      ...
      method !  expr =
        ...
        | Ast.ExLet(_loc , r , b , e ) ->
          Ast.ExLet
            ((_loc , r , ( (decorate_binding decorate_fun b ) ) , (
              (o#expr e ) ) ))
                  ...
                  fun (cons) ->
                    fun (tyargs) ->
                      fun (acc) ->
                        ...
                        let  e =
                        if (List.mem cons ( ["BAnt" ; "OAnt" ; "LAnt" ;
                             "ReAnt" ; "DiAnt" ; "MuAnt" ; "PrAnt" ; "ViAnt"
                             ; "OvAnt" ; "RvAnt"]  ) ) then
                         (
                         Ast.ExApp
                           ((_loc , (
