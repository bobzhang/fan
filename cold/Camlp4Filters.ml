open LibUtil
module IdAstLifter =
 struct let  name = "Camlp4AstLifter" let  version = Sys.ocaml_version  end
module MakeAstLifter =
...
module MakeExceptionTracer =
 functor (Syn : Sig.Camlp4Syntax) ->
  ...
  let  msg =
  ("camlp4-debug: exc: %s at " ^ ( (( (FanLoc.to_string _loc ) ) ^ "@.") ))
  in
  (Ast.ExTry(_loc , e , (
             (Ast.McOr(_loc , (
                       (Ast.McArr(_loc , (
  ...
  | Ast.McArr(_loc , p , w , e ) ->
     (Ast.McArr(_loc , p , w , ( (add_debug_expr e ) ) ))
  | m -> m
  let  filter =
  object
...
module MakeFoldGenerator =
 functor (Syn : Sig.Camlp4Syntax) ->
  ...
  else (sf "_k%d" k ) in (sf "_x%s%s" i k )
  let  exik (i) (k) =
  (Ast.ExId(_loc , ( (Ast.IdLid(_loc , ( (xik i k ) ) )) ) ))
  let  pxik (i) (k) =
  (Ast.PaId(_loc , ( (Ast.IdLid(_loc , ( (xik i k ) ) )) ) ))
  let  elidk (y) (k) =
  (Ast.ExId(_loc , ( (Ast.IdLid(_loc , ( (sf "%s_%d" y k ) ) )) ) ))
  let  plidk (y) (k) =
  (Ast.PaId(_loc , ( (Ast.IdLid(_loc , ( (sf "%s_%d" y k ) ) )) ) ))
  let  xs (s) = ("_x_" ^ s)
  let  xsk = (sf "_x_%s_%d" )
  let  exsk (s) (k) =
  (Ast.ExId(_loc , ( (Ast.IdLid(_loc , ( (xsk s k ) ) )) ) ))
  ...
  let  list_init (f) (n) =
  let rec  self (m) =
  if (m = n) then []  else ( ( (f m ) ) ) :: (self ( (succ m ) ) )  in
  (self 0 )
  ...
  (List.fold_right (
    fun (name) ->
      (SMap.add name
        (name , ( (Ast.IdLid(_loc , name )) ) , []  , ( (Ast.TyNil(_loc)) ) ,
         false  ) ) ) abstr tyMap ) in
  let  tyMap =
  let  concr =
  [("bool" , ( (Ast.IdLid(_loc , "bool" )) ) , []  , (
    (Ast.TySum(_loc , (
               (Ast.TyOr(_loc , (
                         (Ast.TyId(_loc , ( (Ast.IdUid(_loc , "False" )) ) ))
                         ) , (
                         (Ast.TyId(_loc , ( (Ast.IdUid(_loc , "True" )) ) ))
                         ) )) ) )) ) , false  );
   ("list" , ( (Ast.IdLid(_loc , "list" )) ) , (
    [( (Ast.TyQuo(_loc , "a" )) )] ) , (
    (Ast.TySum(_loc , (
               (Ast.TyOr(_loc , (
                         (Ast.TyId(_loc , ( (Ast.IdUid(_loc , "[]" )) ) )) )
                         , (
                         (Ast.TyOf(_loc , (
                                   (Ast.TyId(_loc , (
                                             (Ast.IdUid(_loc , "::" )) ) )) )
   ...
   ("option" , ( (Ast.IdLid(_loc , "option" )) ) , (
    [( (Ast.TyQuo(_loc , "a" )) )] ) , (
    (Ast.TySum(_loc , (
               (Ast.TyOr(_loc , (
                         (Ast.TyId(_loc , ( (Ast.IdUid(_loc , "None" )) ) ))
                         ) , (
                         (Ast.TyOf(_loc , (
                                   (Ast.TyId(_loc , (
                                             (Ast.IdUid(_loc , "Some" )) ) ))
   ...
   ("ref" , ( (Ast.IdLid(_loc , "ref" )) ) , ( [( (Ast.TyQuo(_loc , "a" )) )]
    ...
    (Ast.TyRec(_loc , (
               (Ast.TyCol(_loc , (
                          (Ast.TyId(_loc , ( (Ast.IdLid(_loc , "contents" ))
                                    ) )) ) , ( (Ast.TyQuo(_loc , "a" )) ) ))
               ) )) ) , false  )] in
  (List.fold_right (
  ...
  module Gen =
   functor (X : sig val size : int val mode : mode  end) ->
    ...
    if (size <= 0) then ( assert false )
     ...
     let rec  loop (k) =
     if (k = 2) then ( (f 2 ) )
     else (Ast.ExCom(_loc , ( (loop ( (k - 1) ) ) ) , ( (f k ) ) )) in
     (Ast.ExTup(_loc , (
                (Ast.ExCom(_loc , ( (f 1 ) ) , ( (loop size ) ) )) ) ))
    let  tuplify_patt (f) =
    if (size <= 0) then ( assert false )
    else if (size = 1) then ( (f 1 ) )
    else
     let rec  loop (k) =
     if (k = 2) then ( (f 2 ) )
     else (Ast.PaCom(_loc , ( (loop ( (k - 1) ) ) ) , ( (f k ) ) )) in
     (Ast.PaTup(_loc , (
                (Ast.PaCom(_loc , ( (f 1 ) ) , ( (loop size ) ) )) ) ))
    ...
    if (size <= 0) then ( assert false )
     ...
     let rec  loop (k) =
     if (k = 2) then typ
     else (Ast.TySta(_loc , ( (loop ( (k - 1) ) ) ) , typ )) in
    ...
    (match mode with
     ...
     | Map ->
        (Ast.ExLet(_loc , Ast.ReNil  , ( (Ast.BiEq(_loc , p , e1 )) ) ,
                   e2 ))
     | Fold ->
        (Ast.ExLet(_loc , Ast.ReNil  , (
                   (Ast.BiEq(_loc , (
                             (Ast.PaId(_loc , ( (Ast.IdLid(_loc , "o" ))
                                       ) )) ) , e1 )) ) , e2 )))
    let  return (e) =
    (match mode with
     | Fold_map ->
    ...
    (match e1 with
     | (Ast.ExId(_ , _ )
        | Ast.ExSnd(_ , Ast.ExId(_ , Ast.IdLid(_ , _ ) ) , _ )) ->
        (mk_e2 e1 )
     | Ast.ExLet(_ , Ast.ReNil , Ast.BiEq(_ , p1 , e1 ) , e2 ) ->
        (Ast.ExLet(_loc , Ast.ReNil  , ( (Ast.BiEq(_loc , p1 , e1 )) ) ,
                   ( (opt_bind None  e2 mk_e2 ) ) ))
     | _ ->
        let  e2 =
        (mk_e2 ( (Ast.ExId(_loc , ( (Ast.IdLid(_loc , "o" )) ) )) ) ) in
        (match opt_patt with
    ...
    let  ps (k) =
    (mkp ( (list_init ( fun (i) -> (pxik i k ) ) ( (List.length ts ) ) )
      ) ) in
    let  p = (tuplify_patt ps ) in
    let  e1 = (mke exi1s ) in
    let  es =
    (List.map2 ( fun (x) -> (expr_of_ty ( (Some(x)) ) ) ) exiks ts ) in
    let  e =
    (List.fold_right2 (
      fun (pxi1) -> fun (e) -> fun (acc) -> (bind pxi1 e acc ) ) pxi1s es
      ( (return e1 ) ) ) in
    ...
    let  mk (k) =
    if (k = 1) then ( (Ast.PaId(_loc , ( (Ast.IdLid(_loc , "x" )) ) )) )
    else (Ast.PaAny(_loc)) in
    (match mode with
     | Fold_map ->
        (Ast.McArr(_loc , ( (tuplify_patt mk ) ) , ( (Ast.ExNil(_loc)) )
                   , (
                   (Ast.ExTup(_loc , (
                              (Ast.ExCom(_loc , (
                                         (Ast.ExId(_loc , (
    ...
    (match t with
     ...
     | _ -> (false)) in
    (match t with
     | Ast.TyId(_ , Ast.IdUid(_ , _ ) ) -> (false)
     | t -> (loop t ))
    ...
    (match ox with
     ...
     | _ -> mk_e2) in (opt_bind ( (opt_map patt_of_expr ox ) ) e1 mk_e2 )
    let  opt_app (e) (ox) =
    ...
    | Ast.TyQuo(_ , s ) ->
       (opt_app (
         (Ast.ExApp(_loc , (
                    (Ast.ExId(_loc , (
                              (Ast.IdLid(_loc , ( ("_f_" ^ s) ) )) ) )) )
                    , ( (Ast.ExId(_loc , ( (Ast.IdLid(_loc , "o" )) ) ))
                    ) )) ) ox )
    | _ ->
       (self ox ( (Ast.TyId(_loc , ( (Ast.IdLid(_loc , "unknown" )) ) ))
         ) ) in (self x ty ) and expr_of_ty' (e) (t) =
    (expr_of_ty ( (Some(e)) ) t ) and out_constr_patt (s) =
    (Ast.PaId(_loc , ( (Ast.IdUid(_loc , s )) ) )) and out_constr_expr
    (s) = (Ast.ExId(_loc , ( (Ast.IdUid(_loc , s )) ) )) and
    match_case_of_constructor (s) (t) =
    (chain_tuple ( (apply_patt ( (out_constr_patt s ) ) ) ) (
      (apply_expr ( (out_constr_expr s ) ) ) ) expr_of_ty (
      (Ast.list_of_ctyp t []  ) ) ) and match_case_of_sum_type =
    function
    | Ast.TyOr(_ , t1 , t2 ) ->
       (Ast.McOr(_loc , ( (match_case_of_sum_type t1 ) ) , (
    ...
    | _ -> assert false and match_case_of_poly_constructor (s) (ts) =
    (chain_tuple (
      function
      | [] -> (Ast.PaVrn(_loc , s ))
      | (p :: []) -> (Ast.PaApp(_loc , ( (Ast.PaVrn(_loc , s )) ) , p ))
      | ps ->
         (Ast.PaApp(_loc , ( (Ast.PaVrn(_loc , s )) ) , (
      ...
      | es ->
         (Ast.ExApp(_loc , ( (Ast.ExVrn(_loc , s )) ) , (
                    (Ast.ExTup(_loc , ( (Ast.exCom_of_list es ) ) )) ) ))
      ) expr_of_ty ts ) and match_case_of_poly_sum_type =
    function
    | Ast.TyOr(_ , t1 , t2 ) ->
       (Ast.McOr(_loc , ( (match_case_of_poly_sum_type t1 ) ) , (
    ...
    | _ -> assert false and record_patt_of_type (k) =
    function
    | Ast.TyCol(_ , Ast.TyId(_ , Ast.IdLid(_ , s ) ) , _ ) ->
       (Ast.PaEq(_loc , ( (Ast.IdLid(_loc , s )) ) , (
                 (Ast.PaId(_loc , ( (Ast.IdLid(_loc , ( (xsk s k ) ) )) )
                           )) ) ))
    | Ast.TySem(_ , t1 , t2 ) ->
       (Ast.PaSem(_loc , ( (record_patt_of_type k t1 ) ) , (
                  (record_patt_of_type k t2 ) ) ))
    | _ -> assert false and type_list_of_record_type (t)
    (((acc1 , acc2 ) as acc)) =
    (match t with
     | Ast.TyNil(_) -> acc
     | (Ast.TyCol(_ , Ast.TyId(_ , Ast.IdLid(_ , s ) ) ,
                  Ast.TyMut(_ , t ) )
        | Ast.TyCol(_ , Ast.TyId(_ , Ast.IdLid(_ , s ) ) , t )) ->
        (( ( s ) :: acc1  ) , ( ( t ) :: acc2  ) )
     | Ast.TySem(_ , t1 , t2 ) ->
        (type_list_of_record_type t1 ( (type_list_of_record_type t2 acc )
          ) )
     | _ -> assert false) and expr_of_record_type (t) =
    ...
    (Ast.PaRec(_loc , (
               (Ast.paSem_of_list (
                 (List.map2 (
                   fun (l) ->
                     fun (p) ->
                       (Ast.PaEq(_loc , ( (Ast.IdLid(_loc , l )) ) , p ))
                   ) ls ps ) ) ) ) )) in
    let  mke (es) =
    (Ast.ExRec(_loc , (
               (Ast.rbSem_of_list (
    ...
    (Ast.McArr(_loc , ( (tuplify_patt ( (pxik 0 ) ) ) ) , (
               (Ast.ExNil(_loc)) ) , (
               (Ast.ExApp(_loc , (
                          (Ast.ExSnd(_loc , (
                                     (Ast.ExId(_loc , (
                                     ...
                                     (sf "%s%d_failure" (
                                       (string_of_mode mode ) ) size ) )
                                     )) ) , (
                          (tuplify_expr ( (exik 0 ) ) ) ) )) ) )) and
    complete_match_case (mk) (t) =
    (match t with
     | Ast.TyOr(_ , _ , _ ) when (size > 1) ->
        (Ast.McOr(_loc , ( (mk t ) ) , failure_match_case ))
     | _ -> (mk t )) and fun_of_ctyp (tyid) =
    function
    | Ast.TySum(_ , t ) ->
       (Ast.ExFun(_loc , (
                  (complete_match_case match_case_of_sum_type t ) ) ))
    | Ast.TyRec(_ , t ) ->
       (Ast.ExFun(_loc , ( (expr_of_record_type t ) ) ))
    | Ast.TyTup(_ , t ) -> (mk_tuple expr_of_ty t )
    | Ast.TyId(_ , Ast.IdLid(_ , i ) ) when (i = tyid) -> default_expr
    | ((((Ast.TyApp(_ , _ , _ ) | Ast.TyArr(_ , _ , _ ))
         | Ast.TyQuo(_ , _ )) | Ast.TyId(_ , _ )) as t) ->
       (expr_of_ty None  t )
    | Ast.TyNil(_) ->
       (expr_of_ty None  (
         (Ast.TyId(_loc , ( (Ast.IdLid(_loc , "unknown" )) ) )) ) )
    | ((Ast.TyVrnEq(_ , t ) | Ast.TyVrnInf(_ , t ))
       | Ast.TyPrv(_ , Ast.TyVrnInf(_ , t ) )) ->
       (Ast.ExFun(_loc , (
                  (complete_match_case match_case_of_poly_sum_type t ) )
                  ))
    | (Ast.TyVrnSup(_ , t ) | Ast.TyPrv(_ , Ast.TyVrnSup(_ , t ) )) ->
       if (size > 1) then
        ...
        (Ast.ExFun(_loc , (
                   (Ast.McOr(_loc , ( (match_case_of_poly_sum_type t ) )
                             , default_match_case )) ) ))
    | _ -> assert false and string_of_type_param (t) =
    (match t with
     | ((Ast.TyQuo(_ , s ) | Ast.TyQuP(_ , s )) | Ast.TyQuM(_ , s )) -> s
     | _ -> assert false) and method_of_type_decl (_)
    (((id1 , _ , params , ctyp , priv ) as type_decl)) (acc) =
    let rec  lambda (acc) =
    function
    | [] -> acc
    | (x :: xs) ->
       (lambda (
         (Ast.ExFun(_loc , (
                    (Ast.McArr(_loc , (
                               (Ast.PaId(_loc , (
                                         (Ast.IdLid(_loc , ( ("_f_" ^ x)
                                                    ) )) ) )) ) , (
                               (Ast.ExNil(_loc)) ) , acc )) ) )) ) xs )
    in
    ...
    (Ast.CrSem(_loc , (
               (Ast.CrMth(_loc , id1 , Ast.OvNil  , priv , funs , ty )) )
               , acc )) and ctyp_name_of_name_params (name) (params) =
    (apply_ctyp ( (Ast.TyId(_loc , name )) ) params ) and
    method_type_of_type_decl ((_ , name , params , ctyp , _ )) =
    ...
    else (method_type_of_type t t params []  ) and method_type_of_type
    (t_in) (t_out) (params_in) (params_out) =
    let  rt (t) =
    (match mode with
     | Fold_map ->
        (Ast.TyTup(_loc , (
                   (Ast.TySta(_loc , ( (Ast.TyQuo(_loc , "self_type" )) )
                              , t )) ) ))
    ...
    (match (params_in , params_out ) with
     | ((param_in :: []) , (param_out :: []) ) ->
        ...
        (Ast.TyPol(_loc , ( (Ast.TyApp(_loc , param_in , param_out )) ) ,
                   (
                   (Ast.TyArr(_loc , (
                              (Ast.TyArr(_loc , (
                                         (Ast.TyQuo(_loc , "self_type" ))
                                         ) , (
                                         (Ast.TyArr(_loc , alphas , (
                                                    (rt param_out ) ) ))
                                         ) )) ) , (
                              (Ast.TyArr(_loc , ( (tuplify_type t_in ) )
                                         , ( (rt t_out ) ) )) ) )) ) ))
     | ((param :: []) , [] ) ->
        let  alphas = (tuplify_type param ) in
        (Ast.TyPol(_loc , param , (
                   (Ast.TyArr(_loc , (
                              (Ast.TyArr(_loc , (
                                         (Ast.TyQuo(_loc , "self_type" ))
                                         ) , (
    ...
    (Ast.CgSem(_loc , (
               (Ast.CgMth(_loc , name , Ast.PrNil  , (
                          (method_type_of_type_decl type_decl ) ) )) ) ,
               acc )) and generate_structure (tyMap) =
  ...
  (match t with
   ...
   | Ast.TyDcl(_ , name , tl , tk , _ ) ->
      (SMap.add name
        (name , ( (Ast.IdLid(_loc , name )) ) , tl , tk , false  ) acc )
   | _ -> assert false)
  ...
  let  gen_type =
  (Ast.TyPol(_loc , (
             (Ast.TyApp(_loc , ( (Ast.TyQuo(_loc , "a" )) ) , (
                        (Ast.TyQuo(_loc , "b" )) ) )) ) , (
  ...
  if (n > 1) then
   ...
   (Ast.CrMth(_loc , ( (sf "%s%d_failure" name n ) ) , Ast.OvNil  ,
              Ast.PrNil  , (
              (Ast.ExFun(_loc , (
                         (Ast.McArr(_loc , (
                                    (M.tuplify_patt ( (pxik 0 ) ) ) ) , (
                                    (Ast.ExNil(_loc)) ) , (
                                    (Ast.ExApp(_loc , (
                                               (Ast.ExId(_loc , (
                                                         (Ast.IdLid(_loc
  ...
  let  gen_type =
  (Ast.TyPol(_loc , ( (Ast.TyQuo(_loc , "a" )) ) , (
             (M.method_type_of_type ( (Ast.TyQuo(_loc , "a" )) ) (
               (Ast.TyQuo(_loc , "a" )) ) []  []  ) ) )) in
  let  unknown =
  (Ast.CrMth(_loc , "unknown" , Ast.OvNil  , Ast.PrNil  , M.default_expr
             , gen_type )) in
  (Ast.StCls(_loc , (
             (Ast.CeEq(_loc , (
                       (Ast.CeCon(_loc , Ast.ViNil  , (
                                  (Ast.IdLid(_loc , c )) ) , (
                                  (Ast.TyNil(_loc)) ) )) ) , (
                       (Ast.CeStr(_loc , (
                                  (Ast.PaTyc(_loc , (
                                  ...
                                  (Ast.CrSem(_loc , generated , (
                                             (Ast.CrSem(_loc , failure ,
                                                        unknown )) ) )) )
                                  )) ) )) ) ))
  ...
  if (n > 1) then
   (
   let  name = (string_of_mode mode ) in
   (Ast.CgMth(_loc , ( (sf "%s%d_failure" name n ) ) , Ast.PrNil  ,
              gen_type ))
   )
  ...
  (Ast.TyPol(_loc , ( (Ast.TyQuo(_loc , "a" )) ) , (
             (M.method_type_of_type ( (Ast.TyQuo(_loc , "a" )) ) (
               (Ast.TyQuo(_loc , "a" )) ) []  []  ) ) )) in
  let  unknown = (Ast.CgMth(_loc , "unknown" , Ast.PrNil  , gen_type ))
  in
  (Ast.SgCls(_loc , (
             (Ast.CtCol(_loc , (
                        (Ast.CtCon(_loc , Ast.ViNil  , (
                                   (Ast.IdLid(_loc , c )) ) , (
                                   (Ast.TyNil(_loc)) ) )) ) , (
                        (Ast.CtSig(_loc , (
                                   (Ast.TyQuo(_loc , "self_type" )) ) , (
                                   (Ast.CgSem(_loc , generated , (
                                              (Ast.CgSem(_loc , failure ,
                                                         unknown )) ) ))
                                   ) )) ) )) ) ))
  let  processor =
  ...
  method! str_item =
   fun (st) ->
     (match st with
      ...
      | Ast.StCls(_loc ,
                  Ast.CeEq(_ ,
                           Ast.CeCon(_ , Ast.ViNil , Ast.IdLid(_ , c ) ,
                                     Ast.TyNil(_) ) ,
                           Ast.CeCon(_ , Ast.ViNil ,
                                     Ast.IdAcc(_ ,
                                               Ast.IdUid(_ ,
                                                         "Camlp4Filters" )
                                               ,
      ...
      | Ast.StCls(_loc ,
                  Ast.CeEq(_ ,
                           Ast.CeCon(_ , Ast.ViNil , Ast.IdLid(_ , c ) ,
                                     Ast.TyNil(_) ) ,
                           Ast.CeCon(_ , Ast.ViNil ,
                                     Ast.IdAcc(_ , Ast.IdUid(_ , m ) ,
                                               Ast.IdLid(_ , "generated" ) )
                                     , Ast.TyNil(_) ) ) ) ->
  ...
  method! sig_item =
   fun (sg) ->
     (match sg with
      ...
      | Ast.SgCls(_loc ,
                  Ast.CtCol(_ ,
                            Ast.CtCon(_ , Ast.ViNil , Ast.IdLid(_ , c ) ,
                                      Ast.TyNil(_) ) ,
                            Ast.CtCon(_ , Ast.ViNil ,
                                      Ast.IdAcc(_ ,
                                                Ast.IdAcc(_ ,
                                                          Ast.IdUid(_ ,
                                                                    "Camlp4Filters"
...
module MakeProfiler =
 functor (Syn : Sig.Camlp4Syntax) ->
  ...
  ((object
    ...
    method! binding =
     function
     | Ast.BiEq(_loc , Ast.PaId(_ , Ast.IdLid(_ , id ) ) ,
                (Ast.ExFun(_ , _ ) as e) ) ->
        (Ast.BiEq(_loc , ( (Ast.PaId(_loc , ( (Ast.IdLid(_loc , id )) ) )) )
                  , ( (decorate_fun id e ) ) ))
     | b -> (super#binding b )
  ...
  method! expr =
   ...
   | Ast.ExLet(_loc , r , b , e ) ->
      (Ast.ExLet(_loc , r , ( (decorate_binding decorate_fun b ) ) , (
                 (o#expr e ) ) ))
  ...
  | Ast.ExFun(_loc , Ast.McArr(_ , p , Ast.ExNil(_) , e ) ) ->
     (Ast.ExFun(_loc , (
                (Ast.McArr(_loc , p , ( (Ast.ExNil(_loc)) ) , (
                           (decorate_fun id e ) ) )) ) ))
  | Ast.ExFun(_loc , m ) ->
     (decorate_this_expr ( (Ast.ExFun(_loc , ( (decorate_match_case m ) ) ))
       ) id )
...
module MakeMetaGenerator =
 functor (Syn : Sig.Camlp4Syntax) ->
  ...
  | Ast.IdAcc(_ , i1 , i2 ) ->
     ("acc_" ^ (
       (( (string_of_ident i1 ) ) ^ ( ("_" ^ ( (string_of_ident i2 ) )) )) ))
  | Ast.IdApp(_ , i1 , i2 ) ->
     ("app_" ^ (
       (( (string_of_ident i1 ) ) ^ ( ("_" ^ ( (string_of_ident i2 ) )) )) ))
  ...
  (match t with
   ...
   | Ast.TyNil(_) -> acc
   | _ -> assert false) in (loop init ty )
  ...
  let  m_uid (m) (s) =
  (m_id m ( (meta_ident m ( (Ast.IdUid(_loc , s )) ) ) ) )
  let  failure =
  (Ast.ExApp(_loc , ( (Ast.ExId(_loc , ( (Ast.IdLid(_loc , "raise" )) ) )) )
             , (
             (Ast.ExApp(_loc , (
                        (Ast.ExId(_loc , ( (Ast.IdUid(_loc , "Failure" )) ) ))
                        ) , (
                        (Ast.ExStr(_loc ,
                                   "MetaGenerator: cannot handle that kind of types"
                                   )) ) )) ) ))
  let  mk_meta (m) =
  let  m_name_uid (x) =
                ...
                fun (cons) ->
                  fun (tyargs) ->
                    fun (acc) ->
                      ...
                      let  init = (m_id m ( (meta_ident m m_name_cons ) ) )
                      in
                      let  p = (patt_of_data_ctor_decl m_name_cons tyargs )
                      in
                      let  e =
                      if (List.mem cons (
                           ...
                           fun (i) ->
                             fun (acc) ->
                               ...
                               (match ty with
                                ...
                                | Ast.TyApp(_ , t1 , t2 ) ->
                                   (Ast.ExApp(_loc , ( (fcall_of_ctyp t1 ) )
                                              , ( (fcall_of_ctyp t2 ) ) ))
                                | Ast.TyQuo(_ , s ) ->
                                   (Ast.ExId(_loc , (
              ...
              (List.fold_right (
                ...
                (Ast.ExFun(_loc , (
                           (Ast.McArr(_loc , (
                                      ...
                                      , ( (Ast.ExFun(_loc , match_case )) ) ))
                           ) )) ) ) in
              (Ast.BiAnd(_loc , binding_acc , (
                         (Ast.BiEq(_loc , (
                                   (Ast.PaId(_loc , (
                                             (Ast.IdLid(_loc , (
                                                        ("meta_" ^ tyname) )
     ...
     method! module_expr =
      fun (me) ->
        ...
        (match (super#module_expr me ) with
         | Ast.MeApp(_ ,
                     Ast.MeId(_ ,
                              Ast.IdAcc(_ , Ast.IdUid(_ , "Camlp4Filters" ) ,
                                        Ast.IdUid(_ , "MetaGeneratorExpr" ) ) )
                     , Ast.MeId(_ , i ) ) ->
            (mk_meta_module
