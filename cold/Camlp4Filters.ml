      ...
      fun ast ->
        ...
        Ast.StExp (_loc, (
          Ast.ExLet (_loc, Ast.ReNil, (
            Ast.BiEq (_loc, ( Ast.PaId (_loc, ( Ast.IdLid (_loc, "loc") )) ),
              (
              Ast.ExId (_loc, (
                Ast.IdAcc (_loc, ( Ast.IdUid (_loc, "FanLoc") ), (
  ...
  let add_debug_expr e =
    ...
    Ast.ExTry (_loc, e, (
      Ast.McOr (_loc, (
        ...
        Ast.McArr (_loc, ( Ast.PaId (_loc, ( Ast.IdLid (_loc, "exc") )) ), (
          Ast.ExNil _loc ), (
          Ast.ExSeq (_loc, (
            Ast.ExSem (_loc, (
              Ast.ExIfe (_loc, (
                ...
                Ast.ExApp (_loc, (
                  Ast.ExApp (_loc, (
                    Ast.ExId (_loc, (
                      Ast.IdAcc (_loc, ( Ast.IdUid (_loc, "Format") ), (
                        Ast.IdLid (_loc, "eprintf") ))
                      ))
                    ), ( Ast.ExStr (_loc, ( Ast.safe_string_escaped msg )) ))
                  ), (
                  Ast.ExApp (_loc, (
                    Ast.ExId (_loc, (
                      Ast.IdAcc (_loc, ( Ast.IdUid (_loc, "Printexc") ), (
...
module MakeFoldGenerator(Syn:Sig.Camlp4Syntax) = struct
  ...
  let xik i k =
    let i =
      if i < 0 then assert false else if i = 0 then "" else sf "_i%d" i in
    let k =
      if k < 1 then assert false else if k = 1 then "" else sf "_k%d" k in
    sf "_x%s%s" i k
  ...
  let list_init f n =
    let rec self m = if m = n then [] else ( f m ) :: ( self ( succ m ) ) in
    self 0
      ...
      let concr = [(* ("bool", ( Ast.IdLid (_loc, "bool") ), [], ( *)
        ...
        Ast.TySum (_loc, (
          Ast.TyOr (_loc, ( Ast.TyId (_loc, ( Ast.IdUid (_loc, "[]") )) ), (
            Ast.TyOf (_loc, ( Ast.TyId (_loc, ( Ast.IdUid (_loc, "::") )) ),
              (
              Ast.TyAnd (_loc, ( Ast.TyQuo (_loc, "a") ), (
                Ast.TyApp (_loc, (
        ...
        ), false); ("option", ( Ast.IdLid (_loc, "option") ),
        [Ast.TyQuo (_loc, "a")], (
        Ast.TySum (_loc, (
          Ast.TyOr (_loc, ( Ast.TyId (_loc, ( Ast.IdUid (_loc, "None") )) ),
            (
            Ast.TyOf (_loc, ( Ast.TyId (_loc, ( Ast.IdUid (_loc, "Some") ))
              ), ( Ast.TyQuo (_loc, "a") ))
            ))
          ))
        ), false); ("ref", ( Ast.IdLid (_loc, "ref") ),
        [Ast.TyQuo (_loc, "a")], (
        Ast.TyRec (_loc, (
          Ast.TyCol (_loc, (
  ...
  module Gen(X:sig val size : int  val mode : mode  end) =
    ...
    let bind p e1 e2 =
      ...
      | Fold_map  ->
          Ast.ExLet (_loc, Ast.ReNil, (
            Ast.BiEq (_loc, (
              Ast.PaTup (_loc, (
                Ast.PaCom (_loc, ( Ast.PaId (_loc, ( Ast.IdLid (_loc, "o") ))
                  ), p)
                ))
              ), e1)
            ), e2)
      ...
      | Fold  ->
          Ast.ExLet (_loc, Ast.ReNil, (
            Ast.BiEq (_loc, ( Ast.PaId (_loc, ( Ast.IdLid (_loc, "o") )) ),
              e1)
            ), e2)
    let return e =
      match mode with
      | Fold_map  ->
          Ast.ExTup (_loc, (
            Ast.ExCom (_loc, ( Ast.ExId (_loc, ( Ast.IdLid (_loc, "o") )) ),
              e)
            ))
      | Map  -> e
    ...
    let chain_tuple mkp mke expr_of_ty ts =
      ...
      let ps k =
        mkp ( list_init ( fun i -> pxik i k ) ( List.length ts ) ) in
      let p = tuplify_patt ps in
      let e1 = mke exi1s in
      let es = List.map2 ( fun x -> expr_of_ty ( Some x ) ) exiks ts in
      let e =
        List.fold_right2 ( fun pxi1 -> fun e -> fun acc -> bind pxi1 e acc )
    ...
    let mkfuno e =
      ...
      | _ ->
          Ast.ExFun (_loc, (
            Ast.McArr (_loc, ( Ast.PaId (_loc, ( Ast.IdLid (_loc, "o") )) ),
              ( Ast.ExNil _loc ), e)
            ))
    let is_unknown t =
      let rec loop t =
    ...
    let rec expr_of_ty x ty =
      ...
      self x ty and expr_of_ty' e t = expr_of_ty ( Some e ) t and
      out_constr_patt s = Ast.PaId (_loc, ( Ast.IdUid (_loc, s) )) and
      out_constr_expr s = Ast.ExId (_loc, ( Ast.IdUid (_loc, s) )) and
      ...
      | _ -> assert false and type_list_of_record_type t ( (acc1,acc2) as acc
      ) =
      match t with
      | Ast.TyNil _ -> acc
      ...
      let mkp ps =
        Ast.PaRec (_loc, (
          Ast.paSem_of_list (
            List.map2 (
              fun l -> fun p -> Ast.PaEq (_loc, ( Ast.IdLid (_loc, l) ), p) )
              ls ps )
          )) in
      let mke es =
        Ast.ExRec (_loc, (
          Ast.rbSem_of_list (
            List.map2 (
              fun l -> fun e -> Ast.RbEq (_loc, ( Ast.IdLid (_loc, l) ), e) )
              ls es )
          ), ( Ast.ExNil _loc )) in
      chain_tuple mkp mke expr_of_ty ts and failure_match_case =
      Ast.McArr (_loc, ( tuplify_patt ( pxik 0 ) ), ( Ast.ExNil _loc ), (
        Ast.ExApp (_loc, (
          Ast.ExSnd (_loc, ( Ast.ExId (_loc, ( Ast.IdLid (_loc, "o") )) ), (
      ...
      Ast.CrSem (_loc, ( Ast.CrMth (_loc, id1, Ast.OvNil, priv, funs, ty) ),
      ...
      | (param_in::[],param_out::[]) ->
          ...
          Ast.TyPol (_loc, ( Ast.TyApp (_loc, param_in, param_out) ), (
            Ast.TyArr (_loc, (
              Ast.TyArr (_loc, ( Ast.TyQuo (_loc, "self_type") ), (
  ...
  let generate_class_implem mode c tydcl n =
    ...
    let module M = Gen(struct
      ...
      let failure =
        ...
        then
          ...
          Ast.CrMth (_loc, ( sf "%s%d_failure" name n ), Ast.OvNil,
            Ast.PrNil, (
            Ast.ExFun (_loc, (
              Ast.McArr (_loc, ( M.tuplify_patt ( pxik 0 ) ), (
                Ast.ExNil _loc ), (
                Ast.ExApp (_loc, (
                  Ast.ExId (_loc, ( Ast.IdLid (_loc, "failwith") )) ), (
                  Ast.ExStr (_loc, (
                    Ast.safe_string_escaped (
                      sf "%s%d_failure: default implementation" name n )
                    ))
                  ))
                ))
              ))
      ...
      Ast.StCls (_loc, (
        Ast.CeEq (_loc, (
          Ast.CeCon (_loc, Ast.ViNil, ( Ast.IdLid (_loc, c) ), (
            Ast.TyNil _loc ))
          ), (
          Ast.CeStr (_loc, (
            Ast.PaTyc (_loc, ( Ast.PaId (_loc, ( Ast.IdLid (_loc, "o") )) ),
              ( Ast.TyQuo (_loc, "self_type") ))
            ), (
            Ast.CrSem (_loc, generated, ( Ast.CrSem (_loc, failure, unknown)
              ))
            ))
          ))
        ))
  let generate_class_interf mode c tydcl n =
    ...
    let module M = Gen(struct
      ...
      Ast.SgCls (_loc, (
        Ast.CtCol (_loc, (
          Ast.CtCon (_loc, Ast.ViNil, ( Ast.IdLid (_loc, c) ), (
            Ast.TyNil _loc ))
          ), (
          Ast.CtSig (_loc, ( Ast.TyQuo (_loc, "self_type") ), (
            Ast.CgSem (_loc, generated, ( Ast.CgSem (_loc, failure, unknown)
              ))
            ))
          ))
        ))
...
module MakeProfiler(Syn:Sig.Camlp4Syntax) = struct
  ...
  let decorate_this_expr e id =
    ...
    Ast.ExLet (_loc, Ast.ReNil, (
      Ast.BiEq (_loc, ( Ast.PaId (_loc, ( Ast.IdUid (_loc, "()") )) ), (
        Ast.ExApp (_loc, (
          Ast.ExId (_loc, (
            Ast.IdAcc (_loc, ( Ast.IdUid (_loc, "Camlp4prof") ), (
...
module MakeMetaGenerator(Syn:Sig.Camlp4Syntax) = struct
  ...
  let fold_args ty f init =
    let (_,res) =
      List.fold_left ( fun (i,acc) -> fun ty -> (( succ i ), ( f ty i acc ))
        ) (0, init) ty in
    res
  ...
  let patt_of_data_ctor_decl cons tyargs =
    fold_args tyargs (
      fun _ ->
        fun i ->
          fun acc -> Ast.PaApp (_loc, acc, ( Ast.PaId (_loc, ( x i )) )) ) (
      Ast.PaId (_loc, cons) )
  let expr_of_data_ctor_decl cons tyargs =
    fold_args tyargs (
      fun _ ->
        fun i ->
          fun acc -> Ast.ExApp (_loc, acc, ( Ast.ExId (_loc, ( x i )) )) ) (
      Ast.ExId (_loc, cons) )
  ...
  let failure =
    Ast.ExApp (_loc, ( Ast.ExId (_loc, ( Ast.IdLid (_loc, "raise") )) ), (
      Ast.ExApp (_loc, ( Ast.ExId (_loc, ( Ast.IdUid (_loc, "Failure") )) ),
        ( Ast.ExStr (_loc, "MetaGenerator: cannot handle that kind of types")
        ))
      ))
  let mk_meta m =
    let m_name_uid x =
                    ...
                    fun cons ->
                      fun tyargs ->
                        fun acc ->
                          ...
                          let e =
                            if
                              List.mem cons ["BAnt"; "OAnt"; "LAnt"; "ReAnt";
                                "DiAnt"; "MuAnt"; "PrAnt"; "ViAnt"; "OvAnt";
                                "RvAnt"]
                            then
                              Ast.ExApp (_loc, (
                                Ast.ExApp (_loc, ( Ast.ExId (_loc, ( m.ant ))
                                  ), (
                                  Ast.ExId (_loc, ( Ast.IdLid (_loc, "_loc")
                                    ))
                                  ))
            ...
            Ast.MeStr (_loc, (
              Ast.StSem (_loc, (
                Ast.StVal (_loc, Ast.ReNil, (
                  Ast.BiEq (_loc, (
                    Ast.PaId (_loc, ( Ast.IdLid (_loc, "meta_string") )) ), (
                    Ast.ExFun (_loc, (
                      Ast.McArr (_loc, (
                        Ast.PaId (_loc, ( Ast.IdLid (_loc, "_loc") )) ), (
                        Ast.ExNil _loc ), (
                ...
                Ast.StSem (_loc, (
                  Ast.StVal (_loc, Ast.ReNil, (
                    Ast.BiEq (_loc, (
                      Ast.PaId (_loc, ( Ast.IdLid (_loc, "meta_int") )) ), (
                      Ast.ExFun (_loc, (
                        Ast.McArr (_loc, (
                          Ast.PaId (_loc, ( Ast.IdLid (_loc, "_loc") )) ), (
                          Ast.ExNil _loc ), (
                  ...
                  Ast.StSem (_loc, (
                    Ast.StVal (_loc, Ast.ReNil, (
                      Ast.BiEq (_loc, (
                        Ast.PaId (_loc, ( Ast.IdLid (_loc, "meta_float") ))
                        ), (
                        Ast.ExFun (_loc, (
                          Ast.McArr (_loc, (
                            Ast.PaId (_loc, ( Ast.IdLid (_loc, "_loc") )) ),
                    ...
                    Ast.StSem (_loc, (
                      Ast.StVal (_loc, Ast.ReNil, (
                        Ast.BiEq (_loc, (
                          Ast.PaId (_loc, ( Ast.IdLid (_loc, "meta_char") ))
                          ), (
                          Ast.ExFun (_loc, (
                            Ast.McArr (_loc, (
                              Ast.PaId (_loc, ( Ast.IdLid (_loc, "_loc") ))
                      ...
                      Ast.StSem (_loc, (
                        Ast.StVal (_loc, Ast.ReNil, (
                          Ast.BiEq (_loc, (
                            Ast.PaId (_loc, ( Ast.IdLid (_loc, "meta_bool")
                              ))
                            ), (
                            Ast.ExFun (_loc, (
                              Ast.McArr (_loc, (
                        ...
                        Ast.StSem (_loc, (
                          Ast.StVal (_loc, Ast.ReRecursive, (
                            Ast.BiEq (_loc, (
                              Ast.PaId (_loc, ( Ast.IdLid (_loc, "meta_list")
                                ))
                              ), (
                              Ast.ExFun (_loc, (
                                Ast.McArr (_loc, (
                                  Ast.PaId (_loc, ( Ast.IdLid (_loc, "mf_a")
                                    ))
                                  ), ( Ast.ExNil _loc ), (
                                  Ast.ExFun (_loc, (
                                    Ast.McArr (_loc, (
