...
module MakeFoldGenerator =
 functor (Syn : Sig.Camlp4Syntax) ->
  struct
   ...
   let elidk =
    fun y ->
     fun k -> (Ast.ExId (_loc, ( (Ast.IdLid (_loc, ( (sf "%s_%d" y k) ))) )))
   let plidk =
    fun y ->
     fun k -> (Ast.PaId (_loc, ( (Ast.IdLid (_loc, ( (sf "%s_%d" y k) ))) )))
   let xs = fun s -> ("_x_" ^ s)
   let xsk = (sf "_x_%s_%d")
   let exsk =
    fun s ->
           ...
           | Fold ->
              (Ast.ExLet
                (_loc, Ast.ReNil , (
                 (Ast.BiEq
                   (_loc, (
                    (Ast.PaId (_loc, ( (Ast.IdLid (_loc, "o")) ))) ), e1))
                 ), e2)))
      let return =
       fun e ->
        (match mode with
        ...
        fun mke ->
         fun expr_of_ty ->
          fun ts ->
           ...
           let e =
            (List.fold_right2 (
              fun pxi1 -> fun e -> fun acc -> (bind pxi1 e acc) ) pxi1s
              es ( (return e1) )) in
       ...
       fun _ ->
        fun ((id1, _, params, ctyp, priv) as type_decl) ->
         fun acc ->
          ...
          (Ast.CrSem
            (_loc, ( (Ast.CrMth (_loc, id1, Ast.OvNil , priv, funs, ty))
             ), acc))
      ...
      and generate_signature =
       fun tyMap ->
        (SMap.fold class_sig_item_of_type_decl ( used_builtins.contents )
          (
          (SMap.fold class_sig_item_of_type_decl tyMap (
            (Ast.CgNil (_loc)) )) ))
     end
                   ...
                   (_loc, (
                    (Ast.McArr
                      (_loc, (
                       ...
                       (Ast.ExNil (_loc)) ), ( (Ast.ExFun (_loc, match_case))
                       ))) ))) )) in
              (Ast.BiAnd
                (_loc, binding_acc, (
                 (Ast.BiEq
