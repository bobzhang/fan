...
module MakeAstLifter =
 functor (Syn : Sig.Camlp4Syntax) ->
  ...
  module MetaLoc =
   struct
    let meta_loc_patt =
     fun _loc -> fun _ -> (Ast.PaId (_loc , ( (Ast.IdLid (_loc , "loc")) )))
   let meta_loc_expr =
 ...
 functor (Syn : Sig.Camlp4Syntax) ->
  ...
  let add_debug_expr =
   fun e ->
    ...
    let msg =
     ("camlp4-debug: exc: %s at " ^ ( (( (FanLoc.to_string _loc) ) ^ "@.") )) in
    (Ast.ExTry
      (_loc , e , (
       (Ast.McOr
         (_loc , (
...
module MakeFoldGenerator =
 functor (Syn : Sig.Camlp4Syntax) ->
  ...
  let opt_map = fun f -> function | Some (x) -> (Some (f x)) | None -> (None)
  let list_init =
   fun f ->
    fun n ->
     let rec self =
      fun m -> if (m = n) then []  else ( ( (f m) ) ) :: (self ( (succ m) ))  in
     (self 0)
       ...
       let rec self =
        fun ?(arity = 0) ->
         fun ox ->
          ...
          | _ ->
             (self ox (
               (Ast.TyId (_loc , ( (Ast.IdLid (_loc , "unknown")) ))) )) in
       (self x ty)
     ...
     (match st with
      ...
      | Ast.StCls
         (_loc ,
          Ast.CeEq
           (_ ,
            Ast.CeCon (_ , Ast.ViNil , Ast.IdLid (_ , c) , Ast.TyNil (_))
            ,
            Ast.CeCon
             (_ , Ast.ViNil ,
              Ast.IdAcc
     ...
     (match sg with
      ...
      | Ast.SgCls
         (_loc ,
          Ast.CtCol
           (_ ,
            Ast.CtCon (_ , Ast.ViNil , Ast.IdLid (_ , c) , Ast.TyNil (_))
            ,
            Ast.CtCon
             (_ , Ast.ViNil ,
              Ast.IdAcc
...
module MakeMetaGenerator =
 functor (Syn : Sig.Camlp4Syntax) ->
  ...
  let m_uid =
   fun m -> fun s -> (m_id m ( (meta_ident m ( (Ast.IdUid (_loc , s)) )) ))
  let failure =
   (Ast.ExApp
     (_loc , ( (Ast.ExId (_loc , ( (Ast.IdLid (_loc , "raise")) ))) ) , (
      (Ast.ExApp
             ...
             (_ ,
              Ast.MeId
               (_ ,
                Ast.IdAcc
                 (_ , Ast.IdUid (_ , "Camlp4Filters") ,
                  Ast.IdUid (_ , "MetaGeneratorExpr"))) , Ast.MeId (_ , i)) ->
             (mk_meta_module
             ...
             (_ ,
              Ast.MeId
               (_ ,
                Ast.IdAcc
                 (_ , Ast.IdUid (_ , "Camlp4Filters") ,
                  Ast.IdUid (_ , "MetaGeneratorPatt"))) , Ast.MeId (_ , i)) ->
             (mk_meta_module
               {name = i ; type_decls = ( (Lazy.force type_decls) ) ;
                app = (
                 (Ast.ExId
