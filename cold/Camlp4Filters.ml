  ...
  let builtin_types =
    ...
    let tyMap =
      let concr = [("bool", (Ast.IdLid (_loc, "bool")), [],
        ...
        false); ("list", (Ast.IdLid (_loc, "list")), [Ast.TyQuo (_loc, "a")],
        (Ast.TySum (_loc,
        ...
        (Ast.TySum (_loc,
           (Ast.TyOr (_loc, (Ast.TyId (_loc, (Ast.IdUid (_loc, "None")))),
              (Ast.TyOf (_loc, (Ast.TyId (_loc, (Ast.IdUid (_loc, "Some")))),
                 (Ast.TyQuo (_loc, "a")))))))),
        false); ("ref", (Ast.IdLid (_loc, "ref")), [Ast.TyQuo (_loc, "a")],
        (Ast.TyRec (_loc,
           (Ast.TyCol (_loc,
      ...
      | Fold_map  ->
          Ast.ExLet (_loc, Ast.ReNil,
            (Ast.BiEq (_loc,
               (Ast.PaTup (_loc,
                  (Ast.PaCom (_loc,
                     (Ast.PaId (_loc, (Ast.IdLid (_loc, "o")))), p)))),
               e1)),
            e2)
    ...
    let rec expr_of_ty x ty =
        ...
        (function
         ...
         | es ->
             Ast.ExApp (_loc, (Ast.ExVrn (_loc, s)),
               (Ast.ExTup (_loc, (Ast.exCom_of_list es))))) expr_of_ty ts and
      ...
      Ast.McArr (_loc, (tuplify_patt (pxik 0)), (Ast.ExNil _loc),
        (Ast.ExApp (_loc,
           ...
           (tuplify_expr (exik 0))))) and complete_match_case mk t =
      match t with
      ...
      apply_ctyp (Ast.TyId (_loc, name)) params and method_type_of_type_decl
      (_,name,params,ctyp,_) =
    ...
    let module M = Gen(struct
      ...
      let failure =
        ...
        then
          ...
          Ast.CrMth (_loc, (sf "%s%d_failure" name n), Ast.OvNil, Ast.PrNil,
            (Ast.ExFun (_loc,
               (Ast.McArr (_loc, (M.tuplify_patt (pxik 0)), (Ast.ExNil _loc),
                  (Ast.ExApp (_loc,
...
module MakeMetaGenerator(Syn:Sig.Camlp4Syntax) = struct
  ...
  let fold_args ty f init =
    let (_,res) =
      List.fold_left (fun (i,acc) -> fun ty -> ((succ i), (f ty i acc))) (0,
        init) ty in
    res
  ...
  let patt_of_data_ctor_decl cons tyargs =
    fold_args tyargs
      (fun _ ->
         fun i -> fun acc -> Ast.PaApp (_loc, acc, (Ast.PaId (_loc, (x i)))))
      (Ast.PaId (_loc, cons))
  let expr_of_data_ctor_decl cons tyargs =
    fold_args tyargs
      (fun _ ->
         fun i -> fun acc -> Ast.ExApp (_loc, acc, (Ast.ExId (_loc, (x i)))))
      (Ast.ExId (_loc, cons))
