                                  ...
                                  functor (Syn : Sig.Camlp4Syntax) ->
                                   struct
                                    ...
                                    let rec map_match_case =
                                     ...
                                     | Ast.McOr (_loc, m1, m2) ->
                                        (Ast.McOr
                                          (_loc, ( (map_match_case m1) ), (
                                           (map_match_case m2) )))
                                     | Ast.McArr (_loc, p, w, e) ->
                                        (Ast.McArr
                                          (_loc, p, w, ( (add_debug_expr e)
                                           )))
                                     | m -> m
...
module MakeFoldGenerator =
                                               functor (Syn : Sig.Camlp4Syntax) ->
                                                struct
                                                               ...
                                                               (_,
                                                                ...
                                                                Ast.IdLid
                                                                 (_,
                                                                  "generated")),
                                                              Ast.TyNil (_)))) ->
                                                         (generate_class_from_module_name
                                                           generate_class_implem
                                                           c st m)
                                                      | Ast.StSem
                                                         (_, st1, st2) ->
                                                         let st1 =
                                                          (self#str_item st1) in
                                                         (Ast.StSem
                                                           (_loc, st1, (
                                                            (self#str_item
                                                              st2) )))
                                                 ...
                                                 let _ = (Syn.AstFilters.register_sig_item_filter
                                                           (
                                                           processor#sig_item
                                                           ))
                           ...
                           functor (Syn : Sig.Camlp4Syntax) ->
                            struct
                             ...
                             let x =
                              fun i ->
                               (Ast.IdLid
                                 (_loc, ( ("x" ^ ( (string_of_int i) )) )))

                             let meta_ =
                              fun s -> (Ast.IdLid (_loc, ( ("meta_" ^ s) )))
                                  ...
                                  fun acc ->
                                   fun t ->
                                    (match t with
                                     | Ast.TyOf
                                        (_,
                                         Ast.TyId (_, Ast.IdUid (_, cons)),
                                         ty) ->
                                        (f cons ( (Ast.list_of_ctyp ty [] ) )
                                          acc)
                                     | Ast.TyId (_, Ast.IdUid (_, cons)) ->
                                        (f cons []  acc)
                                     | Ast.TyOr (_, t1, t2) ->
                                           ...
                                           fun cons ->
                                            fun tyargs ->
                                             fun acc ->
                                              ...
                                              let init =
                                               (m_id m (
                                                 (meta_ident m m_name_cons)
                                                 )) in
                                              let p =
                                               (patt_of_data_ctor_decl
                                                 m_name_cons tyargs) in
                                              let e =
                                               if (List.mem cons (
                                                    ["BAnt"; "OAnt"; "LAnt";
                                                     "ReAnt"; "DiAnt";
                                               ...
                                               else if (is_antiquot_data_ctor
                                                         cons) then
                                                     (
                                                     (expr_of_data_ctor_decl
                                                       ( m.ant ) tyargs)
                                                     )
                                               else
                                                (fold_args tyargs (
...
let f_lift =
                                  fun ((module
                                   ...
                                   (P.syntax_plugin (module IdAstLifter)
                                     (module MakeAstLifter))
