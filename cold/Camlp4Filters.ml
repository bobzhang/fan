...
module MakeAstLifter (Syn:Sig.Camlp4Syntax) =
                                    ...
                                    ((_loc,(
                                      Ast.ExId
                                        ((_loc,(
                                          Ast.IdAcc
                                            ((_loc,(
                                              Ast.IdUid ((_loc,"Format")) ),(
                                              Ast.IdLid ((_loc,"eprintf")) )))
                                          ))) ),(
                                      Ast.ExStr
                                        ((_loc,(
...
module IdFoldGenerator  =
  struct let name = "Camlp4FoldGenerator" let version = Sys.ocaml_version end
module MakeFoldGenerator (Syn:Sig.Camlp4Syntax) =
  struct module Ast  = Camlp4Ast let _loc = FanLoc.ghost
    let sf = Printf.sprintf
    let xik (i) (k) =
      let i =
        if (i < 0) then begin assert false
        end else begin
        if (i = 0) then begin ""
        end else begin (sf "_i%d" i)
        end
        end in
      let k =
        if (k < 1) then begin assert false
        end else begin
        if (k = 1) then begin ""
        end else begin (sf "_k%d" k)
        end
        end in
      (sf "_x%s%s" i k)
    ...
    let list_init (f) (n) =
      let rec self (m) =
        if (m = n) then begin []
        end else begin (f m)::(self ( (succ m) ))
        end in
      (self 0)
        ...
        let concr =
          [("bool",( Ast.IdLid ((_loc,"bool")) ),[] ,(
            Ast.TySum
              ((_loc,(
                Ast.TyOr
                  ((_loc,( Ast.TyId ((_loc,( Ast.IdUid ((_loc,"False")) )))
    ...
    let store_if_builtin_type (id) =
      if (SMap.mem id builtin_types) then begin
      (used_builtins := (
    ...
    module Gen (X:sig val size :  int  val mode :  mode  end) =
      struct let size = X.size let mode = X.mode
        let tuplify_expr (f) =
          if (size <= 0) then begin assert false
          end else begin
          if (size = 1) then begin (f 1)
          end else begin
          let rec loop (k) =
            if (k = 2) then begin (f 2)
            end else begin
            Ast.ExCom ((_loc,( (loop ( (k - 1) )) ),( (f k) )))
            end in
        ...
        let tuplify_patt (f) =
          ...
          let rec loop (k) =
            ...
            Ast.PaCom ((_loc,( (loop ( (k - 1) )) ),( (f k) )))
            end in
          Ast.PaTup ((_loc,( Ast.PaCom ((_loc,( (f 1) ),( (loop size) ))) )))
          end
          end let xiks (i) = (tuplify_expr ( (exik i) ))
        let tuplify_type (typ) =
          if (size <= 0) then begin assert false
          end else begin
          if (size = 1) then begin typ
          end else begin
          let rec loop (k) =
            if (k = 2) then begin typ
            end else begin Ast.TySta ((_loc,( (loop ( (k - 1) )) ),typ))
            end in
        ...
        let rec patt_of_expr =
          ...
          | _ ->   assert false)
        let bind (p) (e1) (e2) = begin match mode with
          | Fold_map  ->
              Ast.ExLet
                ((_loc,Ast.ReNil ,(
                  Ast.BiEq
          ...
          | Map  ->
              Ast.ExLet ((_loc,Ast.ReNil ,( Ast.BiEq ((_loc,p,e1)) ),e2))
          | Fold  ->
              Ast.ExLet
                ((_loc,Ast.ReNil ,(
                  Ast.BiEq
        ...
        let return (e) = begin match mode with
          | Fold_map  ->
              Ast.ExTup
                ((_loc,(
                  Ast.ExCom
          ...
          | Map  ->   e
          | Fold  ->   Ast.ExId ((_loc,( Ast.IdLid ((_loc,"o")) ))) end
        let rec opt_bind (opt_patt) (e1) (mk_e2) = begin match e1 with
        ...
        let chain_tuple (mkp) (mke) (expr_of_ty) (ts) =
          ...
          let ps (k) =
            (mkp (
              (list_init ( (fun (i) -> (pxik i k)) ) ( (List.length ts) )) )) in
          ...
          begin match mode with
            ...
            | Map  ->
                Ast.McArr
                  ((_loc,( (tuplify_patt mk) ),( Ast.ExNil (_loc) ),(
                    Ast.ExId ((_loc,( Ast.IdLid ((_loc,"x")) ))) )))
            end let default_expr = Ast.ExFun ((_loc,default_match_case))
        let mkfuno (e) = begin match e with
        ...
        let is_unknown (t) =
          ...
          begin match t with
            ...
            | t ->   (loop t) end
        let contains_unknown (t) = begin try
          let (_ : < .. >) =
            ((object
                inherit  Ast.fold as super
                method !  ctyp =
                  (fun (t) ->
                    if (is_unknown t) then begin (raise Exit )
                    end else begin (super#ctyp t)
                    end) end)#ctyp t) in
          false with | Exit  ->   true end
        let opt_bind' (ox) (e1) (mk_e2) =
          let mk_e2 = begin match ox with
            | Some(x) ->   (fun (e1) -> Ast.ExApp ((_loc,( (mk_e2 e1) ),x)))
            | _ ->   mk_e2 end in
          (opt_bind ( (opt_map patt_of_expr ox) ) e1 mk_e2)
        let opt_app (e) (ox) = begin match ox with
          | Some(x) ->   Ast.ExApp ((_loc,e,x))
          | _ ->   e end
        let rec expr_of_ty (x) (ty) =
            ...
            | Ast.TyQuo(_,s) ->
                (opt_app (
                  Ast.ExApp
                    ((_loc,(
                      Ast.ExId
                        ((_loc,( Ast.IdLid ((_loc,( ("_f_" ^ s) ))) ))) ),(
                      Ast.ExId ((_loc,( Ast.IdLid ((_loc,"o")) ))) ))) ) ox)
            | _ ->
                (self ox ( Ast.TyId ((_loc,( Ast.IdLid ((_loc,"unknown")) )))
                  ))) in
          (self x ty) and expr_of_ty' (e) (t) = (expr_of_ty ( Some (e) ) t)
          and out_constr_patt (s) =
          Ast.PaId ((_loc,( Ast.IdUid ((_loc,s)) ))) and out_constr_expr (s)
          = Ast.ExId ((_loc,( Ast.IdUid ((_loc,s)) ))) and
          ...
          (((acc1,acc2) as acc)) = begin match t with
          | Ast.TyNil(_) ->   acc
          | (Ast.TyCol(_,Ast.TyId(_,Ast.IdLid(_,s)),Ast.TyMut(_,t))
              |Ast.TyCol(_,Ast.TyId(_,Ast.IdLid(_,s)),t))
            ->   (( s::acc1 ),( t::acc2 ))
          | Ast.TySem(_,t1,t2) ->
              (type_list_of_record_type t1 (
                (type_list_of_record_type t2 acc) ))
          | _ ->   assert false end and expr_of_record_type (t) =
          ...
          complete_match_case (mk) (t) = begin match t with
          | Ast.TyOr(_,_,_) when (size > 1) ->
              Ast.McOr ((_loc,( (mk t) ),failure_match_case))
          | _ ->   (mk t) end and fun_of_ctyp (tyid) =
          (function
          | Ast.TySum(_,t) ->
              Ast.ExFun
          ...
          | ((((Ast.TyApp(_,_,_) |Ast.TyArr(_,_,_)) |Ast.TyQuo(_,_))
               |Ast.TyId(_,_)) as t)
            ->   (expr_of_ty None  t)
          | Ast.TyNil(_) ->
              (expr_of_ty None  (
          ...
          | _ ->   assert false) and string_of_type_param (t) = begin match t
          with
          | ((Ast.TyQuo(_,s) |Ast.TyQuP(_,s)) |Ast.TyQuM(_,s)) ->   s
          | _ ->   assert false end and method_of_type_decl (_)
          ...
          let priv =
            if priv then begin Ast.PrPrivate
            end else begin Ast.PrNil
            end in
          Ast.CrSem
          ...
          end else begin (method_type_of_type t t params [] )
          end and method_type_of_type (t_in) (t_out) (params_in) (params_out)
          =
          let rt (t) = begin match mode with
            | Fold_map  ->
                Ast.TyTup
                  ((_loc,(
            ...
            | Map  ->   t end in
          begin match (params_in,params_out) with
            | (param_in::[] ,param_out::[] ) ->
                  ...
                  ((_loc,( Ast.TyApp ((_loc,param_in,param_out)) ),(
                    Ast.TyArr
                      ((_loc,(
                        ...
                        Ast.TyArr
                          ((_loc,( (tuplify_type t_in) ),( (rt t_out) ))) )))
                    )))
            | (param::[] ,[] ) ->
            ...
            | _ ->
                ...
                (failwith (
                  (Printf.sprintf
                    ...
                    i) ))
            end and class_sig_item_of_type_decl (_)
          ...
          (SMap.fold method_of_type_decl ( used_builtins.contents ) (
            (SMap.fold method_of_type_decl tyMap ( Ast.CrNil (_loc) )) )) and
          generate_signature (tyMap) =
          (SMap.fold class_sig_item_of_type_decl ( used_builtins.contents ) (
            (SMap.fold class_sig_item_of_type_decl tyMap ( Ast.CgNil (_loc)
              )) )) end
    let rec tyMap_of_type_decls (t) (acc) = begin match t with
      | Ast.TyNil(_) ->   acc
      | Ast.TyAnd(_,t1,t2) ->
        ...
        Ast.StCls
          ((_loc,(
            Ast.CeEq
              ((_loc,(
                Ast.CeCon
                  ((_loc,Ast.ViNil ,( Ast.IdLid ((_loc,c)) ),(
                    Ast.TyNil (_loc) ))) ),(
                Ast.CeStr
                  ((_loc,(
                    Ast.PaTyc
    ...
    let generate_class_interf (mode) (c) (tydcl) (n) =
      ...
      let module M = (Gen) (struct let size = n let mode = mode end) in
        ...
        let failure =
          ...
          let name = (string_of_mode mode) in
          Ast.CgMth
            ((_loc,( (sf "%s%d_failure" name n) ),Ast.PrNil ,gen_type))
          end else begin Ast.CgNil (_loc)
          end in
        let gen_type =
          Ast.TyPol
            ((_loc,( Ast.TyQuo ((_loc,"a")) ),(
    ...
    let processor =
      ...
      let generate_class' (generator) (default) (c) (s) (n) = begin match s
        with
        ...
        (Scanf.sscanf m "Camlp4%[^G]Generator" (
          (fun (m') -> begin try
            (Scanf.sscanf m' "%[^0-9]%d" (
              (generate_class' generator default c) )) with
            | (End_of_file  |Scanf.Scan_failure(_)) ->
              (generate_class' generator default c m' 1) end) ))
        with | (End_of_file  |Scanf.Scan_failure(_)) ->   default end in
      object
         (self)inherit  Ast.map as super
        method !  str_item =
          (fun (st) -> begin match st with
            | Ast.StTyp(_,t) ->   begin
                                  (last := t);
                                  st
                                  end
...
module IdLocationStripper  =
  struct let name = "Camlp4LocationStripper" let version = Sys.ocaml_version
    end
            ...
            ((_loc,( Ast.PaId ((_loc,( Ast.IdUid ((_loc,"()")) ))) ),(
              Ast.ExApp
                ((_loc,(
                  ...
                  Ast.ExStr ((_loc,( (Ast.safe_string_escaped s) ))) ))) )))
          ),e))
    let rec decorate_fun (id) =
      ...
      | Ast.ExFun(_loc,Ast.McArr(_,p,Ast.ExNil(_),e)) ->
          Ast.ExFun
            ((_loc,(
              Ast.McArr
                ((_loc,p,( Ast.ExNil (_loc) ),( (decorate_fun id e) ))) )))
      | Ast.ExFun(_loc,m) ->
          (decorate_this_expr (
...
module IdTrashRemover  =
  struct let name = "Camlp4TrashRemover" let version = Sys.ocaml_version end
...
module IdMetaGenerator  =
  struct let name = "Camlp4MetaGenerator" let version = Sys.ocaml_version end
module MakeMetaGenerator (Syn:Sig.Camlp4Syntax) =
  struct module Ast  = Camlp4Ast
    ...
    let fold_data_ctors (ty) (f) (init) =
      let rec loop (acc) (t) = begin match t with
        ...
        ((_loc,(
          Ast.ExApp
            ((_loc,(
              Ast.ExApp
                ((_loc,( m.app ),(
                  Ast.ExId ((_loc,( Ast.IdLid ((_loc,"_loc")) ))) ))) ),x))
          ),y))
    let m_id (m) (i) =
      Ast.ExApp
        ((_loc,(
          Ast.ExApp
            ((_loc,( m.id ),( Ast.ExId ((_loc,( Ast.IdLid ((_loc,"_loc")) )))
              ))) ),i))
    ...
    let mk_meta (m) =
      ...
      (fold_type_decls m (
        (fun (tyname) ->
          (fun (tydcl) ->
            (fun (binding_acc) -> begin match tydcl with
              | Ast.TyDcl(_,_,tyvars,Ast.TySum(_,ty),_) ->
                      ...
                      (fun (cons) ->
                        (fun (tyargs) ->
                          (fun (acc) ->
                            ...
                            let init =
                              (m_id m ( (meta_ident m m_name_cons) )) in
                            let p =
                              (patt_of_data_ctor_decl m_name_cons tyargs) in
                            let e =
                              if
                              (List.mem cons (
                                ["BAnt";"OAnt";"LAnt";"ReAnt";"DiAnt";"MuAnt";"PrAnt";"ViAnt";"OvAnt";"RvAnt"]
                                ))
                              then begin
                              Ast.ExApp
                                ((_loc,(
                                  ...
                                  Ast.ExId
                                    ((_loc,( Ast.IdLid ((_loc,"x0")) ))) )))
                              end else begin
                              if (is_antiquot_data_ctor cons) then begin
                              ...
                              (fold_args tyargs (
                                (fun (ty) ->
                                  (fun (i) ->
                                    (fun (acc) ->
                                      let rec fcall_of_ctyp (ty) = begin
                                        match ty with
                                        | Ast.TyId(_,id) ->
                                            Ast.ExId
                                              ((_loc,(
                  ...
                  let funct =
                    (List.fold_right (
                      (fun (tyvar) ->
                        (fun (acc) -> begin match tyvar with
              ...
              | _ ->   assert false end))) ) ( Ast.BiNil (_loc) ))
    let find_type_decls =
      object
      ...
      ((object
          ...
          method !  module_expr =
            (fun (me) ->
              ...
              begin match (super#module_expr me) with
