...
module Camlp4Bin(PreCast:Sig.PRECAST) =
  struct
  let printers = (Hashtbl.create 30
    :(string ,(module Sig.PRECAST_PLUGIN)) Hashtbl.t  )
  ...
  let rewrite_and_load n x =
    ...
    let load n =
      ...
      then ()
        ...
        add_to_loaded_modules n; DynLoader.load dyn_loader ( n ^ objext ) ) in
    (
  ...
  let initial_spec_list = [("-I", (
    ...
    "More verbose in parsing errors."); ("-loc", ( Arg.Set_string FanLoc.name
    ), (
