...
module Tags =
  Set.Make(struct type t = tag_info 
                  let compare = tag_compare end)
module TagMap =
  Map.Make(struct type t = tag_info 
                  let compare = tag_compare end)
module IdSet =
  Set.Make(struct
             type t = ident 
             let compare (x : t) y =
               match (x, y) with
               | (`Lid (_,id1),`Lid (_,id2)) -> String.compare id1 id2
           end)
module IdMap =
  Map.Make(struct
             type t = ident 
...
let rec do_remove_nested to_remove =
  function
  | Bind (e,x) ->
      if IdSet.mem x to_remove
      then do_remove_nested to_remove e
      else Bind ((do_remove_nested (IdSet.add x to_remove) e), x)
  ...
  | Alternative (e1,e2) ->
      Alternative
        ((do_remove_nested to_remove e1), (do_remove_nested to_remove e2))
  | Repetition e -> Repetition (do_remove_nested to_remove e)
...
let rec find_all_vars =
  function
  | Characters _|Epsilon |Eof  -> IdSet.empty
  | Bind (e,x) -> IdSet.add x (find_all_vars e)
...
let rec do_find_opt =
  ...
  | Bind (e,x) -> let (opt,all) = do_find_opt e in (opt, (IdSet.add x all))
  | Sequence (e1,e2) ->
      let (opt1,all1) = do_find_opt e1 and (opt2,all2) = do_find_opt e2 in
      ((IdSet.union opt1 opt2), (IdSet.union all1 all2))
  | Alternative (e1,e2) ->
      let (opt1,all1) = do_find_opt e1 and (opt2,all2) = do_find_opt e2 in
      ((IdSet.union (IdSet.union opt1 opt2) (stringset_delta all1 all2)),
        (IdSet.union all1 all2))
  | Repetition e -> let r = find_all_vars e in (r, r)
let find_optional e = let (r,_) = do_find_opt e in r
let rec do_find_double =
  function
  | Characters _|Epsilon |Eof  -> (IdSet.empty, IdSet.empty)
  | Bind (e,x) ->
      let (dbl,all) = do_find_double e in
      ((if IdSet.mem x all then IdSet.add x dbl else dbl), (IdSet.add x all))
  | Sequence (e1,e2) ->
      let (dbl1,all1) = do_find_double e1 and (dbl2,all2) = do_find_double e2 in
      ((IdSet.union (IdSet.inter all1 all2) (IdSet.union dbl1 dbl2)),
        (IdSet.union all1 all2))
  | Alternative (e1,e2) ->
      let (dbl1,all1) = do_find_double e1 and (dbl2,all2) = do_find_double e2 in
      ((IdSet.union dbl1 dbl2), (IdSet.union all1 all2))
  | Repetition e -> let r = find_all_vars e in (r, r)
...
let rec do_find_chars sz =
  ...
  | Bind (e,x) ->
      ...
      (match e_sz with
       | Some 1 -> ((IdSet.add x c), s, (add_some 1 sz))
       | _ -> (c, (IdSet.add x s), (add_some_some sz e_sz)))
  | Sequence (e1,e2) ->
      let (c1,s1,sz1) = do_find_chars sz e1 in
      let (c2,s2,sz2) = do_find_chars sz1 e2 in
      ((IdSet.union c1 c2), (IdSet.union s1 s2), sz2)
  | Alternative (e1,e2) ->
      let (c1,s1,sz1) = do_find_chars sz e1
      and (c2,s2,sz2) = do_find_chars sz e2 in
      ((IdSet.union c1 c2), (IdSet.union s1 s2),
...
let rec encode_regexp char_vars act =
  function
  | Epsilon  -> Empty
  | Characters cl ->
  ...
  | Eof  ->
      let n = chars_count.contents in
      (chars := (Fcset.eof :: (chars.contents));
       incr chars_count;
       Chars (n, true))
  | Sequence (r1,r2) ->
...
let mk_seq r1 r2 =
  ...
  | (_,_) -> Seq (r1, r2)
let add_pos p i =
  match p with | Some (Sum (a,n)) -> Some (Sum (a, (n + i))) | None  -> None
let mem_name name id_set =
  IdSet.exists (fun (`Lid (_,id_name))  -> name = id_name) id_set
let opt_regexp all_vars char_vars optional_vars double_vars r =
  let env = Hashtbl.create 17 in
  let rec size_forward pos =
    function
  ...
  let rec simple_forward pos r =
    ...
    | Seq (r1,r2) ->
        ...
        (match pos with
         ...
         | Some pos ->
             let (r2,pos) = simple_forward pos r2 in ((mk_seq r1 r2), pos))
    | Alt (r1,r2) ->
        let pos1 = size_forward pos r1 and pos2 = size_forward pos r2 in
        (r, (if pos1 = pos2 then pos1 else None))
    | Star _ -> (r, None)
    | Action _ -> assert false in
  let rec size_backward pos =
    function
    | Empty |Chars (_,true )|Tag _ -> Some pos
    | Chars (_,false ) -> Some (pos - 1)
    | Seq (r1,r2) ->
  ...
  let rec simple_backward pos r =
    ...
    | Seq (r1,r2) ->
        ...
        (match pos with
         | None  -> ((mk_seq r1 r2), None)
         | Some pos ->
             let (r1,pos) = simple_backward pos r1 in ((mk_seq r1 r2), pos))
    | Alt (r1,r2) ->
        let pos1 = size_backward pos r1 and pos2 = size_backward pos r2 in
        (r, (if pos1 = pos2 then pos1 else None))
    | Star _ -> (r, None)
  ...
  let r =
    if opt
    then
      let (r,_) = simple_forward 0 r in let (r,_) = simple_backward 0 r in r
    else r in
  ...
  let rec alloc_exp pos r =
    ...
    | Alt (_,_) ->
        ...
        (match off with | Some i -> (r, (add_pos pos i)) | None  -> (r, None))
    | Star _ -> (r, None)
    | Action _ -> assert false in
  ...
  let r =
    List.fold_left
      (fun (reg,actions,count,ntags)  (expr,act)  ->
         ...
         ((Alt (reg, (Seq (r, (Action count))))), ((count, m, act) ::
           actions), (succ count), (max loc_ntags ntags))) (Empty, [], 0, 0)
      casedef in
  r
...
let trans_compare (t1,tags1) (t2,tags2) =
  ...
  | r -> r
module TransSet =
  Set.Make(struct type t = transition 
...
let rec emptymatch =
  ...
  | Star r -> if nullable r then emptymatch r else Tags.empty
...
let env_to_class m =
  let env1 =
    MemMap.fold
      ...
      TagMap.empty in
  TagMap.fold (fun tag  ss  r  -> MemKey.add { tag; equiv = ss } r) env1
    MemKey.empty
...
let old_in_map m r =
  TagMap.fold
    (fun _  addr  r  -> if is_old_addr addr then Ints.add addr r else r) m r
let alloc_map used m mvs =
  TagMap.fold
    (fun tag  a  (r,mvs)  ->
       let (a,mvs) =
         if is_new_addr a
         then let a = do_alloc_cell used tag in (a, (Ints.add a mvs))
         else (a, mvs) in
...
let create_new_state { final = (act,(_,m_act)); others = o } =
  let used =
    MemMap.fold (fun _  (_,m)  r  -> old_in_map m r) o
      (old_in_map m_act Ints.empty) in
...
let sort_mvs mvs =
  let rec do_rec r mvs =
    ...
    | _ ->
        let dests =
          List.fold_left (fun r  mv  -> Ints.add (dest mv) r) Ints.empty mvs in
        let (rem,here) =
          List.partition (fun mv  -> Ints.mem (orig mv) dests) mvs in
        (match here with
         | [] ->
             (match rem with
              | (Copy (d,_))::_ ->
...
let do_tag_actions n env m =
  ...
  let (_,r) =
    TagMap.fold
      (fun tag  m  (used,r)  ->
         if (not (Ints.mem m used)) && tag.start
         then ((Ints.add m used), ((EraseTag m) :: r))
...
let make_tag_entry id start act a r =
  match a with
  | Sum (Mem m,0) -> TagMap.add { id; start; action = act } m r
  | _ -> r
