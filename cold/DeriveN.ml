...
let tuple_exp_of_ctyp ?(arity= 1)  ?(names= [])  ~mk_tuple  ~f  (ty : ctyp) =
  (match ty with
   | `Par t ->
       ...
       ExpN.mkfun names
         (ExpN.currying [(`Case (pat, tys) : AstN.case )] ~arity)
...
let rec normal_simple_exp_of_ctyp ?arity  ?names  ~mk_tuple  ~right_type_id 
  ...
  let rec aux =
    ...
    | `App (t1,t2) -> (`App ((aux t1), (aux t2)) : AstN.exp )
    | `Quote (_,`Lid s) -> tyvar s
    | `Arrow (t1,t2) ->
        aux (`App ((`App ((`Lid "arrow"), t1)), t2) : AstN.ctyp )
    | `Par _ as ty ->
         ...
         | (#ident' as tctor)::ls ->
             appl_of_list ((trans (IdN.to_vid tctor)) ::
               (ls |>
                  (List.map
                     (function
                      | `Quote (_,`Lid s) -> (`Lid (var s) : AstN.exp )
                      | t ->
                          (`Fun (`Case ((`Lid "self"), (aux t))) : AstN.exp )))))
         | _ ->
...
let exp_of_ctyp ?cons_transform  ?(arity= 1)  ?(names= [])  ~default 
  ...
  let f (cons : string) (tyargs : ctyp list) =
    (let args_length = List.length tyargs in
     ...
     let e = mk (cons, tyargs) in (`Case (p, e) : AstN.case ) : case ) in
...
let mk_prefix (vars : opt_decl_params) (acc : exp) ?(names= []) 
  ...
  let f (var : decl_params) acc =
    match var with
    | `Quote (_,`Lid s) -> (`Fun (`Case ((`Lid (varf s)), acc)) : AstN.exp )
            ...
            (match repr with
             | `Record t ->
                 ...
                 mk_prefix ~names ~left_type_variable tyvars
                   (ExpN.currying ~arity
                      [(`Case (pat, (mk_record info)) : AstN.case )])
             | `Sum ctyp ->
...
let bind_of_tydcl ?cons_transform  simple_exp_of_ctyp ?(arity= 1)  ?(names=
  ...
  let (_ty,result) =
    CtypN.mk_method_type_of_name ~number:arity ~prefix:names (name, len)
      destination in
  let (annot,result) =
    match annot with
    | None  -> (None, result)
  ...
  let fun_exp =
    if not (CtypN.is_abstract tydcl)
    then
   ...
   let body: clfield =
     let items =
       List.map
         (fun (dest,src,len)  ->
            let (ty,_dest) =
              CtypN.mk_method_type ~number:arity ~prefix:names (src, len)
                (Obj k) in
...
let gen_stru ?module_name  ?(arity= 1)  ?(default=
  ...
  AstN.exp ))  ?cons_transform  ?annot  ~id:(id : basic_id_transform) 
  ...
  let default (_,number) =
    ...
    then
      ...
      Some (`Case (pat, default) : AstN.case )
    else None in
  let names = names in
...
let gen_object ?module_name  ?(arity= 1)  ?(default=
  (`App ((`Lid "failwith"), (`Str "arity >= 2 in other branches")) : 
  AstN.exp ))  ?cons_transform  ~kind  ~base  ~class_name  =
    ...
    let right_type_variable =
      `Exp
        (fun v  ->
           ...
           (`App ((`Lid v), (`Lid "self")) : AstN.exp )) in
