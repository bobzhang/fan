...
let usage_string speclist errmsg =
  let b = Buffer.create 200 in usage_b b speclist errmsg; Buffer.contents b
...
let parse_argv ?(current= current)  argv speclist anonfun errmsg =
  ...
  let stop error =
    ...
    else raise (Bad (Buffer.contents b)) in
  incr current;
  while current.contents < l do
    (let s = argv.(current.contents) in
     ...
     then
       let action =
         try assoc3 s speclist.contents with | Not_found  -> stop (Unknown s) in
       ((try
           let rec treat_action =
             ...
             | Symbol (symb,f) when (current.contents + 1) < l ->
                 ...
                 then (f (argv.(current.contents + 1)); incr current)
                 else
                   raise
                     (Stop
                        (Wrong
             ...
             | Rest f ->
                 while current.contents < (l - 1) do
                   (f (argv.(current.contents + 1)); incr current) done
