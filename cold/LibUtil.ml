...
module Hashset  =
  struct type 'a t = ('a, unit ) Hashtbl.t   let create = Hashtbl.create
    ...
    let to_list (set) = (fold ( (fun (x) -> (fun (y) -> x::y)) ) set [] ) end
...
let callcc (type u) ((f : ( u  cont  ->  u ) )) =
  let module M = struct exception Return of  u  end in begin try
    (f ( (fun (x) -> (raise ( M.Return (x) ))) )) with | M.Return(u) ->   u
    end
...
module Stream  =
  struct include BatStream include Stream
    ...
    let tail ((__strm : _ Stream.t )) = begin match (Stream.peek __strm) with
      ...
      | _ ->   Stream.sempty end
    let rec map (f) ((__strm : _ Stream.t )) = begin match
      ...
      | _ ->   Stream.sempty end end
module ErrorMonad  =
  struct type  log =  string  
    type 'a result =  
      | Left of 'a
      | Right of  log   let return (x) = Left (x) let fail (x) = Right (x)
