...
module type S =
  sig type 'a expand_fun = FanLoc.t  -> string  option  -> string  -> 'a 
  val add : string  -> 'a DynAst.tag  -> 'a expand_fun  -> unit 
  val find : string  -> 'a DynAst.tag  -> 'a expand_fun 
  val default : string  ref  val default_tbl : (string ,string ) Hashtbl.t 
...
module Make(TheAntiquotSyntax:AntiquotSyntax) : S =
  ...
  let expander_name (pos_tag) (name) =
    let str = (DynAst.string_of_tag pos_tag) in begin match
      ...
      | "" ->
          begin try (Hashtbl.find default_tbl str)
          with
  ...
  let expand_quotation (loc) (expander) (pos_tag) (quot) =
    let open FanSig in
      ...
      begin try
        (expander loc loc_name_opt ( quot.q_contents ))
        with
