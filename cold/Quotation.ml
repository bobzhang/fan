...
module Make (TheAntiquotSyntax:AntiquotSyntax) : S =
  struct
    ...
    let expander_name (pos_tag) (name) =
      let str = (DynAst.string_of_tag pos_tag) in begin match
        ((translate.contents) name) with
        | "" ->
            begin try (Hashtbl.find default_tbl str) with
            | Not_found  ->   default.contents end
        | name ->   name end
    let find (name) (tag) =
    ...
    let quotation_error_to_string ((name,position,ctx,exn)) =
      ...
      let name =
        if (name = "") then begin default.contents
        end else begin name
        end in
      let pp (x) =
        (bprintf ppf "@?@[<2>While %s %S in a position of %S:" x name
          position) in
      let ()  = begin match ctx with
        | Finding  ->
            begin
    ...
    let expand_quotation (loc) (expander) (pos_tag) (quot) =
      let open FanSig in
        let loc_name_opt =
          if (( quot.q_loc ) = "") then begin None
          end else begin Some (quot.q_loc)
          end in
    ...
    let parse_quotation_result (parse) (loc) (quot) (pos_tag) (str) =
      let open FanSig in begin try (parse loc str) with
    ...
    let expand (loc) (quotation) (tag) =
      let open FanSig in
        ...
        let expander = begin try (find name tag) with
          | (FanLoc.Exc_located(_,Quotation(_)) as exc) ->   (raise exc)
      ...
      let expand_patt (_loc) (loc_name_opt) (s) =
        (BatRef.protect FanConfig.antiquotations true  (
          (fun (_) ->
            ...
            let exp_ast = (anti_filter#patt meta_ast) in begin match
              loc_name_opt with
              | None  ->   exp_ast
              | Some(name) ->
                  let rec subst_first_loc =
                    (function
                    | Ast.PaApp(_loc,Ast.PaId(_,Ast.IdAcc(_,Ast.IdUid
                                                          (_,"Ast"),Ast.IdUid
                                                          (_,u))),_)
                      ->
                        Ast.PaApp
    ...
    let add_quotation_of_match_case ~name  ~entry  =
      (add name DynAst.match_case_tag (
        (parse_quot_string ( (Gram.eoi_entry entry) )) )) end 