  ...
  struct
    ...
    let  quotation_error_to_string ((name , position , ctx , exn )) =
      ...
      | ParsingResult(loc , str ) ->
        ...
        | Some(dump_file) ->
          ...
          let  oc = (open_out_bin dump_file ) in begin
          ...
          | _ ->
            (bprintf ppf
              "Error while dumping result in file %S; dump aborted" dump_file
              ))
        | None  ->
          (bprintf ppf
    ...
    let  parse_quotation_result (parse) (loc) (quot) (pos_tag) (str) =
      let open FanSig
        ...
        with
        |
          FanLoc.Exc_located(iloc ,
                             Quotation(n , pos_tag , Expanding  , exc ) ) ->
      ...
      (BatRef.protect FanConfig.antiquotations true  (
        fun (_) ->
          ...
          | Some(name) ->
            ...
            | p -> p in (subst_first_loc exp_ast )) ) ) in begin
    ...
    let  add_quotation_of_expr ~name  ~entry  =
      ...
      Ast.StExp ((loc , ( (expand_fun loc loc_name_opt s ) ) )) in begin
