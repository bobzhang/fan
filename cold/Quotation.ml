...
module type S =
                 sig
                  ...
                  val add :
                   (string -> ('a DynAst.tag -> ('a expand_fun -> unit)))
                  ...
                  val parse_quotation_result :
                   ((FanLoc.t -> (string -> 'a)) ->
                    (FanLoc.t ->
                     (FanSig.quotation -> (string -> (string -> 'a)))))

                  val translate : (string -> string) ref

                  val expand :
                   (FanLoc.t -> (FanSig.quotation -> ('a DynAst.tag -> 'a)))
                                    ...
                                    functor (TheAntiquotSyntax : AntiquotSyntax) ->
                                     (struct
                                       ...
                                       let default_at_pos =
                                        fun pos ->
                                         fun str ->
                                          (Hashtbl.replace default_tbl pos
                                            str)
                                       ...
                                       let expander_name =
                                        fun pos_tag ->
                                         fun name ->
                                          let str =
                                           (DynAst.string_of_tag pos_tag) in
                                          (match
                                             ((translate.contents) name) with
                                           | "" ->
                                              (try
                                                (Hashtbl.find default_tbl
                                                  str)
                                               with
                                       ...
                                       let add =
                                        fun name ->
                                         fun tag ->
                                          fun f ->
                                           let elt =
                                            ((name, ( (Exp_key.pack tag () )
                                              )), ( (Exp_fun.pack tag f) )) in
                                           (expanders_table := (
                                             ( elt ) ::
                                              expanders_table.contents  ))
                                          ...
                                          fun pos_tag ->
                                           fun quot ->
                                            ...
                                            let loc_name_opt =
                                             ...
                                             else (Some (quot.q_loc)) in
                                            (try
                                              (expander loc loc_name_opt (
                                                quot.q_contents ))
                                             with
                                             | (FanLoc.Exc_located
                                                 (_, Quotation (_)) as exc) ->
                                                (raise exc)
                                            ...
                                            fun str ->
                                             ...
                                             (try (parse loc str) with
                                              | FanLoc.Exc_located
                                                 ...
                                                 let ctx =
                                                  (ParsingResult
                                                    (iloc, ( quot.q_contents
                                                     ))) in
                                                 let exc1 =
                                                  (Quotation
                                                    (n, pos_tag, ctx, exc)) in
                                                 (raise (
                                                   (FanLoc.Exc_located
                                                     (iloc, exc1)) ))
                                              | FanLoc.Exc_located
                                         ...
                                         fun quotation ->
                                          fun tag ->
                                           ...
                                           let loc =
                                            (FanLoc.join (
                                              (FanLoc.move `start (
                                                quotation.q_shift ) loc) )) in
                                           (expand_quotation loc expander
                                             pos_tag quotation)
                                        ...
                                        fun name ->
                                         fun entry ->
                                          fun mexpr ->
                                           fun mpatt ->
                                            let entry_eoi =
                                             (Gram.eoi_entry entry) in
                                            let expand_expr =
                                             fun loc ->
                                              fun loc_name_opt ->
                                               fun s ->
                                                (parse_quot_string_with_filter
                                                  entry_eoi loc loc_name_opt
                                                  s (
                                                  fun ast ->
                                                   (anti_filter#expr (
                                                     (mexpr loc ast) )) )) in
