...
and exp =
  ...
  | `ObjEnd of loc | `ObjPat of (loc* pat* cstru) | `ObjPatEnd of (loc* pat)
  ...
  | `LocalTypeFun of (loc* alident* exp) | `Package_exp of (loc* module_exp)] 
and rec_exp =
...
and class_exp =
  ...
  | `ObjEnd of loc | `ObjPat of (loc* pat* cstru) | `ObjPatEnd of (loc* pat)
  | `Constraint of (loc* class_exp* class_type)
