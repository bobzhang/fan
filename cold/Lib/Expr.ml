  ...
  let rec loop top =
    ...
    | e1::el ->
        (let _loc =
         ...
         Ast.ExApp (_loc,
           (Ast.ExApp (_loc, (Ast.ExId (_loc, (Ast.IdUid (_loc, "::")))), e1)),
           (loop false el))) in
  loop true
    ...
    method! patt =
      ...
      | Ast.PaAnt (_loc,s)|Ast.PaStr (_loc,s) as p ->
          (let mloc _loc = Meta.MetaLocQuotation.meta_loc_patt _loc _loc in
           handle_antiquot_in_string ~s ~default:p ~parse:parse_patt
             ~loc:_loc
             ~decorate:(fun n ->
                          fun p ->
                            match n with
    ...
    method! expr =
      ...
      | Ast.ExAnt (_loc,s)|Ast.ExStr (_loc,s) as e ->
          (let mloc _loc = Meta.MetaLocQuotation.meta_loc_expr _loc _loc in
           handle_antiquot_in_string ~s ~default:e ~parse:parse_expr
             ~loc:_loc
             ~decorate:(fun n ->
                          fun e ->
                            match n with
