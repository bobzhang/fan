...
let rec  fa (al) =
  ...
  (function
  | Ast.ExApp(_,f,a) -> (fa ( a::al ) f) | f -> (f,al))
let rec  apply (accu) =
...
let  mklist_last ?last  (_loc) =
  ...
  | []  ->
    ...
    (match last
    with
    | Some(e) -> e | None  -> Ast.ExId ((_loc,( Ast.IdUid ((_loc,"[]")) ))))
    | e1::el ->
...
let  mksequence' (_loc) =
  ...
  (function
  | (Ast.ExSem(_,_,_) as e) -> Ast.ExSeq ((_loc,e)) | e -> e)
let  bigarray_get (_loc) (arr) (arg) =
  ...
  | (Ast.ExTup(_,Ast.ExCom(_,e1,e2)) |Ast.ExCom(_,e1,e2)) ->
    (Ast.list_of_expr e1 ( (Ast.list_of_expr e2 [] ) )) | _ -> [arg]) in
           ...
           ((_loc,(
             Ast.ExApp
               ((_loc,(
                 ...
                 ),( Ast.ExArr ((_loc,coords)) ))) ),newval))) | _ -> None)
let  map (_loc) (p) (e) (l) =
    ...
    method !  expr =
      ...
      | ((Ast.ExId(_,Ast.IdLid(_,x)) |Ast.ExId(_,Ast.IdUid(_,x))) as e) ->
        ...
        |
          ...
          with
          | Not_found  -> (super#expr e)) | e -> (super#expr e))
    method !  patt =
      ...
      | ((Ast.PaId(_,Ast.IdLid(_,x)) |Ast.PaId(_,Ast.IdUid(_,x))) as p) ->
        ...
        with
        | Not_found  -> (super#patt p)) | p -> (super#patt p)) end
let  map_expr =
    ...
    method !  patt =
      ...
      | ((Ast.PaAnt(_loc,s) |Ast.PaStr(_loc,s)) as p) when (is_antiquot s) ->
        ...
        | Some(_name,code) ->
          ...
          Ast.PaId ((_loc,( Ast.IdLid ((_loc,code')) ))) | None  -> p)
