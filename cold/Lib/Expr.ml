...
let map_expr =
  ...
  | Ast.ExId (_loc,Ast.IdLid (_,"__FILE__")) ->
      Ast.ExStr (( _loc ),
        (Ast.safe_string_escaped (FanLoc.file_name ( _loc ))))
  ...
  object 
    ...
    method! patt =
      ...
      | Ast.PaAnt (_loc,s)|Ast.PaStr (_loc,s) as p ->
          ...
          handle_antiquot_in_string ~s ~default:p ~parse:parse_patt ~loc:(
            _loc )
            ~decorate:(fun n ->
                         fun p ->
                           match n with
    ...
    method! expr =
      ...
      | Ast.ExAnt (_loc,s)|Ast.ExStr (_loc,s) as e ->
          ...
          handle_antiquot_in_string ~s ~default:e ~parse:parse_expr ~loc:(
            _loc )
            ~decorate:(fun n ->
                         fun e ->
                           match n with
    ...
    method! patt =
      ...
      | Ast.PaAnt (_loc,s)|Ast.PaStr (_loc,s) as p when is_antiquot s ->
          (match view_antiquot s with
           | Some (_name,code) ->
               let cons =
                 Ast.ExId (( _loc ), (Ast.IdLid (( _loc ), code))) in
               let code' = "__" ^ code in
               let cons' =
                 Ast.ExId (( _loc ), (Ast.IdLid (( _loc ), code'))) in
...
let filter_patt_with_captured_variables patt =
  ...
  (let patt = capture_antiquot#patt patt in
   let constraints = capture_antiquot#get_captured_variables in (patt,
     constraints))