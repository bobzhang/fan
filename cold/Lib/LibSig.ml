...
type col = {col_label:string ; col_mutable:bool ; col_ctyp:Ast.ctyp }  
type ty_info = {ty_name_expr:Ast.expr ; ty_expr:Ast.expr ;
                 ty_id_expr:Ast.expr ; ty_id_patt:Ast.patt ;
                 ty_id_exprs:Ast.expr list  ; ty_id_patts:Ast.patt list  }  
type record_col = {record_label:string ; record_mutable:bool ;
                    record_info:ty_info }  
...
type full_id_transform =
 [ basic_id_transform | `Idents of (Ast.ident list  -> Ast.ident)   |
   `Ident of (Ast.ident -> Ast.ident)   | `Last of (string -> Ast.ident)   |
...
module type Config =
 sig val mk_variant : (string -> (ty_info list  -> Ast.expr) ) 
 val mk_tuple : (ty_info list  -> Ast.expr) 
 val mk_record : (record_info -> Ast.expr)  val arity : int
 val left_type_variable : basic_id_transform
 val right_type_variable : rhs_basic_id_transform
 val right_type_id : full_id_transform val left_type_id : basic_id_transform
 val trail : (trail_info -> Ast.match_case)  val names : string list   end
...
module type Grammar =
 ...
 val parse_quot_string_with_filter :
   ('a  t  ->
     (('a  -> 'b )  -> (loc -> (string option  -> (string -> 'b ) ) ) ) ) 
 ...
 val add_quotation_of_match_case :
   (name:string -> (entry:Ast.match_case t  -> unit) )   end
