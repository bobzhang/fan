               ...
               ((_loc , (
                 Ast.ExApp
                   ((_loc , (
                     ...
                     (meta_loc _loc x0 ) ) )) ) , (
                 (meta_rec_binding _loc x1 ) ) )) ) , ( (meta_expr _loc x2 )
             ) ))
       | Ast.ExOvr(x0 , x1 ) ->
         Ast.ExApp
           ((_loc , (
...
class clean_ast =
 ...
 method! with_constr =
  fun (wc) ->
    ...
    with
    | (Ast.WcAnd(_ , Ast.WcNil(_) , wc ) |Ast.WcAnd(_ , wc , Ast.WcNil(_) ))
      -> wc
    | wc -> wc)
 method! expr =
  fun (e) ->
 ...
 method! binding =
  fun (bi) ->
    ...
    with
    | (Ast.BiAnd(_ , Ast.BiNil(_) , bi ) |Ast.BiAnd(_ , bi , Ast.BiNil(_) ))
      -> bi
    | bi -> bi)
 method! rec_binding =
  fun (rb) ->
    ...
    with
    | (Ast.RbSem(_ , Ast.RbNil(_) , bi ) |Ast.RbSem(_ , bi , Ast.RbNil(_) ))
      -> bi
    | bi -> bi)
 method! module_binding =
  fun (mb) ->
    ...
    with
    | (Ast.MbAnd(_ , Ast.MbNil(_) , mb ) |Ast.MbAnd(_ , mb , Ast.MbNil(_) ))
      -> mb
    | mb -> mb)
 method! ctyp =
  fun (t) ->
 ...
 method! sig_item =
  fun (sg) ->
    ...
    with
    | (Ast.SgSem(_ , Ast.SgNil(_) , sg ) |Ast.SgSem(_ , sg , Ast.SgNil(_) ))
      -> sg
 ...
 method! str_item =
  fun (st) ->
    ...
    with
    | (Ast.StSem(_ , Ast.StNil(_) , st ) |Ast.StSem(_ , st , Ast.StNil(_) ))
      -> st
 ...
 method! class_expr =
  fun (ce) ->
    ...
    with
    | (Ast.CeAnd(_ , Ast.CeNil(_) , ce ) |Ast.CeAnd(_ , ce , Ast.CeNil(_) ))
      -> ce
    | ce -> ce)
 method! class_type =
  fun (ct) ->
    ...
    with
    | (Ast.CtAnd(_ , Ast.CtNil(_) , ct ) |Ast.CtAnd(_ , ct , Ast.CtNil(_) ))
      -> ct
    | ct -> ct)
...
class reloc _loc = object inherit map method! loc = fun (_) -> _loc  end
let  wildcarder =
  object (self)
   inherit map as super
  method! patt =
