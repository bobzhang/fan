...
let ident_of_ctyp =
 let error =
  fun ()  -> (invalid_arg "ident_of_ctyp: this type is not an identifier") in
 let rec self =
  function
...
let ident_of_patt =
 let error =
  fun ()  -> (invalid_arg "ident_of_patt: this pattern is not an identifier") in
 let rec self =
  function
...
let bi_of_pe =
 fun (p, e) -> let _loc = (loc_of_patt p) in (Ast.BiEq (_loc, p, e))

let sum_type_of_list = fun l -> (tyOr_of_list ( (List.map ty_of_stl l) ))

let record_type_of_list = fun l -> (tySem_of_list ( (List.map ty_of_sbt l) ))

let binding_of_pel = fun l -> (biAnd_of_list ( (List.map bi_of_pe l) ))
...
let rec list_of_class_sig_item =
 fun x ->
  fun acc ->
   (match x with
    | Ast.CgNil (_) -> acc
...
let rec list_of_class_str_item =
 fun x ->
  fun acc ->
   (match x with
    | Ast.CrNil (_) -> acc
...
let rec list_of_module_binding =
 fun x ->
  fun acc ->
   (match x with
    | Ast.MbAnd (_, x, y) ->
                       ...
                       (_loc, (
                        (Ast.ExApp
                          (_loc, (
                           ...
                           (meta_string _loc x1) ))) ), (
                        (meta_override_flag _loc x2) ))) ), (
                     (meta_private_flag _loc x3) ))) ), ( (meta_expr _loc x4)
                  ))) ), ( (meta_ctyp _loc x5) )))
         | Ast.CrIni (x0, x1) ->
            (Ast.ExApp
              (_loc, (
                 ...
                 fun _loc ->
                  fun (x1, x2) ->
                   (Ast.ExTup
                     (_loc, (
                      (Ast.ExCom
                        (_loc, ( (meta_ctyp _loc x1) ), ( (meta_ctyp _loc x2)
                         ))) ))) ) _loc x4) )))
         | Ast.TyMan (x0, x1, x2) ->
            (Ast.ExApp
              (_loc, (
               ...
               (_loc, (
                (Ast.ExId
                  (_loc, (
                   (Ast.IdAcc
                     (_loc, ( (Ast.IdUid (_loc, "Ast")) ), (
                      (Ast.IdUid (_loc, "OSome")) ))) ))) ), ( (mf_a _loc x0)
                )))
          | Ast.ONone ->
             (Ast.ExId
               (_loc, (
   ...
   | TyDcl (_x, _x_i1, _x_i2, _x_i3, _x_i4) ->
      ...
      let _x_i4 =
       (o#list (
         fun o ->
          fun (_x, _x_i1) ->
           let _x = (o#ctyp _x) in let _x_i1 = (o#ctyp _x_i1) in (_x, _x_i1)
         ) _x_i4) in
      (TyDcl (_x, _x_i1, _x_i2, _x_i3, _x_i4))
