...
let rec is_irrefut_patt =
 ...
 | Ast.PaId (_ , _ ) -> (false)
 | Ast.PaMod (_ , _ ) -> (true)
 | (((((((((((Ast.PaVrn (_ , _ ) | Ast.PaStr (_ , _ ))
             | Ast.PaRng (_ , _ , _ )) | Ast.PaFlo (_ , _ ))
           | Ast.PaNativeInt (_ , _ )) | Ast.PaInt64 (_ , _ ))
...
let record_type_of_list =
 fun l -> (tySem_of_list ( (List.map ty_of_sbt l ) ) )
   ...
   functor (MetaLoc : META_LOC) ->
    ...
    module Expr =
     struct
      let meta_string =
       fun _loc ->
        fun s -> (Ast.ExStr (_loc , ( (safe_string_escaped s ) ) ))
    ...
    module Patt =
     struct
      let meta_string =
       fun _loc ->
        fun s -> (Ast.PaStr (_loc , ( (safe_string_escaped s ) ) ))
...
class fold =
 ...
 method meta_list :
  'a .
    (('self_type -> ('a -> 'self_type) )  -> ('a meta_list  -> 'self_type) )  =
  fun _f_a ->
   function
   | LNil -> o
   | LCons (_x , _x_i1 ) ->
