...
(** Common utility functions which has nothing
    to do with AST transform
 *)
 ...
 *           [ [] -> assert False
 *           | [y] -> y
 *           | [y::ys] -> f y (loop ys) ] in loop xs]; *)

(* value  reduce ?(dir=`Left)  f lst =
 *       match lst with
 *       [ [] -> invalid_arg "reduce lst length is zero"
 *       | [x::xs] ->
 *           match dir with
 *           [ `Left -> 
 *           loop x xs
