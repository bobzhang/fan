...
open Dypgen_lexer
open Extract_type
let _ = Arg.parse Argument.list_arg Argument.process_argument "usage: dypgen [-v] [--merge-warning] [--Werror] [--pv-obj] [--pv-token] [--noemit-token-type] [--no-pp] [--no-obj-type] [--no-undef-nt] [--ocamlc \"options\" | --command \"command\"] [--no-mli] [--use-rule-order] [--use-all-actions] [--cpp | --cpp-options \"options\"] file_name.dyp\n"

let _ = if !Argument.string_ref = "" then
  ...
  with Failure msg -> (
    ...
    if !b then (
      ...
      if line1=line2 then
        ...
        eprintf "File \"%s\", from l:%d, c:%d to l:%d, c:%d :\nLexing failed with message: %s\n"
