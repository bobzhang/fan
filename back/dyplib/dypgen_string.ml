...
module Make (S:Params) =struct
  let {token_list;
       relation;
       start=non_terminal_start_list;
       generic_merge;
       cons=cons_list;
       nt_type=nt_type_list;
       single_nt=single_nt_list;
       additional_cons=add_cons_list;
       layout=ppi_layout;
     } = S.parser_param_info
...
then (Printf.eprintf
  ...
  let grammar =
    let make_epsilon_rule n =
      ...
      ((nt,[dummy_pat_inh]),
       ...
       ("",(Lexing.dummy_pos, false)),(true,true)) in
    let rec add_epsilon_rules n res =
      if n = 0 then res else
      add_epsilon_rules (n-1) ((make_epsilon_rule n)::res) in
    add_epsilon_rules max_eps grammar

  let sharp_line_number fname =
    function 0 | -1 -> "\n"
  ...
  (* let topheader_main =
   *   if topheader_main="" then "\n" else
   *   (sharp_line_number
   *      topheader_main_pos.pos_fname topheader_main_pos.pos_lnum)^
   *   (space_string
   *      (topheader_main_pos.pos_cnum-topheader_main_pos.pos_bol))^
  ...
  let code_type_token,token_map =
    ...
    let aux (code_type_token,token_map) (tok,typ) =
      if typ = "No_type" then
        ...
        (String_map.add tok typ token_map) in
    let code_type_token, token_map =
      List.fold_left aux (code_type_token,token_map) token_list in
    let code_type_token = code_type_token^rbra^"\n" in
    (if !Argument.emit_token_type && not !Dypgen_parser.use_dyplex
    then code_type_token else ""), token_map

  let token_name_map,code_token_nb, token_nb =
    let token_name_map = String_map.empty in
    let aux tok _ (n,token_name_map) =
      (n+1),String_map.add tok n token_name_map in
    let n,token_name_map =
      String_map.fold aux token_map (0,token_name_map) in
    token_name_map, "  let token_nb = "^(string_of_int n)^"\n", n
  let code_get_token_name, code_str_token,
    code_str_token_name, code_token_name_array =
  ...
  let map_card m = String_map.fold (fun _ _ i-> i+1) m 0
  ...
  let nt_par_set =
    ...
    | (Symb_non_terminal_NL ((nt,pos),_,_,inh)),_ ->
        ...
        (try
          ...
          eprintf "Inconsistency with non terminal `%s'.\nIt has a Caml parameter in file \"%s\" line %d, characters %d-%d, and no parameter in file \"%s\" line %d, characters %d-%d.\n"
            nt f1 l1 c11 c12 f2 l2 c21 c22;
          exit 2)
        with Not_found -> String_map.add nt (pos, b) ntpm)
    | _ -> ntpm  in
    let aux2 ntpm ((lhs_nt,_),_,ld_list,_,_) =
      List.fold_left aux1 ntpm ld_list  in
    let nt_par_map = List.fold_left aux2 String_map.empty grammar in
    String_map.fold (fun nt (_,b) ntps
      -> if b then String_set.add nt ntps else ntps)
    ...
    let non_terminal_set =
      ...
      let f hs nt =
        ...
        if !Argument.werror then
          (eprintf "Error: non terminal `%s' is never in a %s\n" nt hs;
           exit 2)
        else
  ...
  let symb_cons_map, cons_entry_table, var_cons_map =
    let cm =
      ...
      String_map.fold
        (fun t _ cm -> String_map.add t ("Obj_"^t) cm) token_map cm  in
    let vcm =
...
(*let _ =
  ...
  let priority_set =
    ...
    | Rel_single p1 -> String_set.add p1 str_set   in
    ...
    | (Symb_non_terminal_NL (_,(p,(line,col1,col2,fname)),_,_)),_ ->
        ...
        then
          (printf "File \"%s\", line %d, characters %d-%d:\n"
           ...
           if !Argument.werror then
             (eprintf "Error: the priority `%s' is not declared\n" p;
              exit 2)
           else
  ...
  let code_nt_cons_list,  cons_map, cons_set, cons_array, code_test_cons =
    ...
    code_ntcl,  cons_map, s, cons_array, code_tc
  ...
  let code_parser =
    let aux ((lhs_nt, lhs_pat_l), (prio,_), symb_list,
      ...
      | (Symb_early_action _), _ -> assert false  in
      ...
      let code_literal_list =
        ...
        else String.sub code_literal_list 0 (string_length-1) in
      let rule_options =
        "["^(if allow_layout_inside then "" else "Dyp.No_layout_inside;")^
        (if allow_layout_follows then "" else "Dyp.No_layout_follows;")^"]" in
      let code_rule =
  ...
  let code_parser_lexer =
    String.concat ",\n\n"
      ["let __dypgen_ra_list, __dypgen_main_lexer, __dypgen_aux_lexer =\n"^
       code_aux_lexer_fun^
       code_parser; code_main_lexer; code_aux_lexer]
  let code_main_2 =
    let aux =
      if  !Dypgen_parser.use_dyplex then
        (fun str (nts,_) -> try
  ...
  let code_type_obj, cons_no_type_array =
    let cons_no_type_array =
      Array.make (String_set.cardinal cons_no_type_set) ""  in
    let code_obj =
...
(*
  ...
  let () = if !Argument.no_mli then () else begin 
    let ec =
      if !Argument.command = "" then
        Sys.command
          ("ocamlc "^(!Argument.ocamlc_options)
           ^" -i -impl "^temp_output_file^
           " > "^extract_type)  else
        Sys.command !Argument.command  in
    if ec = 2 then exit 2;
    let error_regexp filename =
      let (a,b,c,d,e,f) = "@","onzon","e",".","gmail","com" in
      let ema = c^d^b^a^e^d^f in
      eprintf "Error while extracting strings from %s\nPlease send me the file %s at %s to fix this bug\n"
        filename filename ema  in
  let parser_code_mli =
    let token_type, obj_type, parser_type, entry_fun_type_map =
      ...
      let token_type, obj_type, parser_type, entry_fun_type_map =
        try
          ...
          a,b,c,d
        with Failure _ -> error_regexp extract_type; exit 2 in
      close_in extract_obj_file;
      token_type, obj_type, parser_type, entry_fun_type_map in
  let aux str (nts,_) =
    let fun_type =
  ...
  topmli_code^  token_type^  obj_type^  midmli_code^  parser_type^
  (List.fold_left aux "" non_terminal_start_list)^ mli_code in
