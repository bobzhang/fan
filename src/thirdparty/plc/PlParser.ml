    ...
    List.fold_left (fun m -> fun
      | `Rule (p,ts,body,_loc) ->
        let (l1,l2) = try PredMap.find p m with Not_found -> ([],[]) in
          PredMap.add p ((ts,body,_loc)::l1,l2) m
      | `Mask (p,args,_loc) ->
         let (l1,l2) = try PredMap.find p m with Not_found -> ([],[]) in
           PredMap.add p (l1,(args,_loc)::l2) m)
      PredMap.empty rd
    ;;

Foptions.add ("-nogroup", (Arg.Unit (fun () -> group_rs := PlTranslate.nogroup_rs)),"Don't try to optimally group predicate rules" )
let g = Gramf.create_lexer ~annot:"prolog"
    ...
    ();;
%create{(g:Gramf.t) prog rule_or_mask rule body args term bar_term mask var arg_mask};;

%extend{
prog:
  [ L0 rule_or_mask as rd %{(
    let res = unwrap_rule_or_masks rd
    in
      %stru{ ${Ast_gen.sem_of_list (PlTranslate.prog_statics _loc res)} ;;
        ${Ast_gen.sem_of_list (PlTranslate.prog_rules _loc !group_rs res)} }
    )}
  ]
rule_or_mask: [ rule as x %{`Rule x} | mask as x %{`Mask x} ]
rule: [ Lid x; ? args as t;  ? body as b;  "." %{(
...
term:
  { "relop" NA
      ...
      | S as x; "=\\="; S as y %{Comp (PlNames.ne,[x;y],_loc)}
      | S as x; "<"; S as y %{Comp (PlNames.lt,[x;y],_loc)}
      | S as x; "=<"; S as y %{Comp (PlNames.lte,[x;y],_loc)}
      | S as x; ">"; S as y %{Comp (PlNames.gt,[x;y],_loc)}
      | S as x; ">="; S as y %{Comp (PlNames.gte,[x;y],_loc)} ]
 "add" LA
      [ S as x; "+"; S as y %{Comp (PlNames.add,[x;y],_loc)}
      | S as x; "-"; S as y %{Comp (PlNames.sub,[x;y],_loc)} ]
 "unary minus" NA
      [ "-"; Int s %{Integer (-(int_of_string s), _loc)}  (* FIXME-NRG *)
      | "-"; S as x %{Comp (PlNames.neg,[x],_loc)} ]
 "simple" NA
      [ Lid x; ? args as t %{
        (match (x,t) with
        | (* ("_",None) -> Anon _loc *)
