...
let output_mem_acces (i:int) = {:exp|lexbuf.Lexing.lex_mem.($`int:i)|}
...
let output_memory_actions (mvs:memory_action list) : exp list =
  List.map
    (function
      | Copy(tgt,src) ->
          let u = output_memory_actions tgt in
          let v = output_memory_actions src in 
          {:exp| $u <- $v |}
      | Set tgt ->
          let u = output_memory_actions tgt in
          {:exp| $u <- $cur_pos |}) mvs
...
let output_clause
  ...
  let action = output_action mems r in 
  {:case| $pat:pat -> $action |}
...
let output_tag_actions (mvs:Lexgen.tag_action list) : exp list =
  List.map
    (function
      | Lexgen.SetTag(t,m) ->
          let u = output_mem_acces t in
          let v = output_mem_acces m in 
          {:exp| $u <- $v |}
      | Lexgen.EraseTag(t) ->
          let u = output_mem_acces t in 
          {:exp| u <- -1 |}) mvs
...
let output_trans (i:int) (trans:automata)=
  ...
  | Shift(trans,move) ->
      let moves = output_moves move in
      seq_sem
          (match trans with
          | Remember(n,mvs) ->
              let es = output_tag_actions mvs in
...
let output_automata (transitions:Lexgen.automata array) : stru  =
  sem auto_defs
    (and_of_list (Array.mapi (fun i auto -> output_trans i auto)))
