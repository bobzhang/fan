...
let process_intf  name =
  match parse_file ~directive_handler:sig_handler name PreCast.parse_interf with
  | None ->
...
let process_impl name =
  match parse_file ~directive_handler:str_handler name PreCast.parse_implem with
  | None ->
  ...
  |Some x ->
      ...
      PreCast.CurrentPrinter.print_implem
        ?input_file:(Some name)
        ?output_file:(!output_file) (Some x )
