...
let refill_lexbuf s p ppf buffer len =
  ...
  then 0
    ...
    len''
let u (s) = 
  match Prelude.parse_implem (Locf.mk "<>") (Streamf.of_string s) with 
  | None -> []
  | Some x -> Ast2pt.stru x
let execute printval ?pp_code ?highlight_location  pp_answer s =
  (* let lb = Lexing.from_function (refill_lexbuf s (ref 0) pp_code) in *)
  (* try *)
  (*   while true do *)
      (* try *)
        (* let phr = !Toploop.parse_use_file lb in *)
  try
        ignore (Toploop.execute_phrase printval pp_answer 
                  (Ptop_def (u s)) (* phr *))
  with 
  | Locf.Exc_located(loc,y) -> 
      Format.fprintf pp_answer "@[<0>%a%s@]@." 
        Toploop.print_location loc (Printexc.to_string y)
  | e -> Errors.report_error pp_answer  e 
...
(* let eval_file (file_as_string : string) : string =   *)
(*   ignore (Format.flush_str_formatter ()); *)

(*   Format.flush_str_formatter () *)
let export (field : string) v = 
  Js.Unsafe.set (Js.Unsafe.global(* variable "exports" *)) field v

(**
    https://github.com/ocsigen/js_of_ocaml/issues/248
   {[
   Now you can do 
   ocaml.eval("#fan;;")
   ocaml.eval("let f _loc = %exp{x + y};;")
   "val f : Astf.loc -> Astf.exp = <fun>"
   ]}
*)
...
let _ =
  ...
  export "ocaml" jsobject
    method eval code  = Js.string (eval (Js.to_string code))
  end
...
(*     Tyxml_js.Html5.(span ~a:[a_class [cl]] [pcdata s]) in *)

(*   Sys_js.set_channel_flusher caml_chan  (append text output "caml"); *)
(*   Sys_js.set_channel_flusher sharp_chan (append text output "sharp"); *)
(*   Sys_js.set_channel_flusher stdout     (append text  output "stdout"); *)
(*   Sys_js.set_channel_flusher stderr     (append text  output "stderr")); *)
...
(* local variables: *)
(* compile-command: "ocamlfind ocamlc -I ../utils -I ../common  -I ../cold -c -syntax camlp4o -safe-string -package js_of_ocaml.syntax,lwt,js_of_ocaml.tyxml,js_of_ocaml.toplevel -c toplevel.ml" *)
(* end: *)
