...
(* This is a map from a priority to a couple of priority sets :
   ...
   The [prd_ind]  maps the string of a priority to its int value.
   [prd_nb] is the number of priorities. *)

(* REMARQUE : Puisque les priorités sont des entiers,
   la structure qu'il nous faut c'est un tableau à 2 entrées :
