...
module type TRIE = sig
  ...
  val find : key -> t 'a -> 'a;
  val remove : key ->  t 'a->  t 'a;
  val mem : key ->  t 'a -> bool;
  val iter : (key -> 'a -> unit) -> t 'a -> unit;
  val map : ('a -> 'b) -> t 'a ->  t 'b;
  val mapi : (key -> 'a -> 'b) -> t 'a ->  t 'b;
  val fold : (key -> 'a -> 'b -> 'b) -> t 'a -> 'b -> 'b;
  val compare : ('a -> 'a -> int) -> t 'a ->  t 'a -> int;
  val equal : ('a -> 'a -> bool) ->  t 'a ->  t 'a -> bool;
end;

module MakeTrie (M : TRIE) (* : TRIE with type key  = list M.key *) = struct

  type key =  list M.key;
  type  t 'a =
      [Node of option 'a and M.t ( t 'a) ];
  ...
  let rec mem l t =
    ...
    | ([], Node (Some _,_)) -> true
    | ([x::r], Node (_,m))    ->
        try mem r (M.find x m) with [Not_found -> false]];

  let rec has_path l t =
    match (l,t) with
    [ ([], Node(None,_)) -> `path t
    | ([], Node(Some v,_)) -> `leaf v
    | ([x::r], Node(_,m)) ->
      try has_path r (M.find x m) with [Not_found -> `miss ]  
    ];
  (* {[
     STrie.is_empty (STrie.add [] 3 STrie.empty);
     - : bool = false
     ]}*)     
  let add l v t =
  ...
  let mapi f t = 
    ...
    | Node (Some v,m) -> 
        Node (Some (f (List.rev revp) v), M.mapi (fun x -> maprec [x::revp]) m)] in
    maprec [] t;
  ...
  let iter f t =
    let rec traverse revp = function
      ...
      | Node (Some v,m) -> 
          begin f (List.rev revp) v; M.iter (fun x t -> traverse [x::revp] t) m end] in
    traverse [] t;
  ...
  let  fold f t acc =
    let rec traverse revp t acc = match t with
      ...
      | Node (Some v,m) -> 
          f (List.rev revp) v (M.fold (fun x -> traverse [x::revp]) m acc)] in
    traverse [] t acc;
  ...
  let compare cmp a b =
    let rec comp a b = match (a,b) with
      ...
      | (Node (Some a, m1), Node (Some b, m2)) ->
          let c = cmp a b in
          if c <> 0 then c else M.compare comp m1 m2] in
    comp a b;
