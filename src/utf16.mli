...
(** [stream_from_char_stream opt_stro] creates a new int stream
   ...
   [to_int_array]. *)
val stream_from_char_stream: byte_order option -> char XStream.t -> int XStream.t
...
(** [from_stream bo s] reads the next code point from a stream encoded
   in byte order [bo]. *)
val from_stream: byte_order -> char XStream.t -> int
...
val from_utf16_stream: char XStream.t -> byte_order option -> Ulexing.lexbuf
