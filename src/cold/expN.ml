...
let mkfun names acc =
  List.fold_right
    (fun name  acc  -> (`Fun (`Case ((`Lid name), acc)) : Astfn.exp )) names
    acc
let currying cases ~arity  =
  ...
  then
    ...
    let x = tuple_com exps in mkfun names (`Match (x, cases) : Astfn.exp )
...
let mk_record label_exps =
  (let rec_exps =
     List.map
       (fun (label,exp)  -> (`RecBind ((`Lid label), exp) : Astfn.rec_exp ))
       label_exps in
...
let mk_record_ee label_exps =
  (label_exps |> (List.map (fun (label,exp)  -> mee_record_col label exp)))
    ...
    (fun es  ->
       ...
       (`App ((`Vrn "Record"), (`Par (`Com ((`Lid "_loc"), x)))) : Astfn.exp ))
