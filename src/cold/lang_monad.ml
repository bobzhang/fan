...
let f (loc : Locf.t) meta content =
  ...
  let tmp_entry__016_ = Gramf.get_levels exp in
  try
    Gramf.extend_single
      ({
    ...
    (let result = Gramlib.parse_string_eoi exp ~loc content in
     Gramf.fresh_with_levels exp tmp_entry__016_; result)
  with | x -> (Gramf.fresh_with_levels exp tmp_entry__016_; raise x)
