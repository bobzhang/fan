       ...
       | _a0 ->
           ((function
             | fmt ->
                 (function
                  | (_a0,_a1) ->
                      Format.fprintf fmt "@[<1>(%a,@,%a)@]" pp_print_string
                        _a0 pp_print_typedecl _a1))) fmt _a0)
and pp_print_and_types: Format.formatter -> and_types -> unit =
  function
  | fmt -> (function | _a0 -> pp_print_list pp_print_named_type fmt _a0)
...
and pp_print_mtyps: Format.formatter -> mtyps -> unit =
  function | fmt -> (function | _a0 -> pp_print_list pp_print_types fmt _a0)
       ...
       | (m : mtyps) ->
           (let f =
              ...
              | `Mutual ls ->
                  let x =
                    Listf.filter_map
                      (function | (s,_) as x -> if f s then Some x else None)
                      ls in
                  (match x with
                   | [] -> None
                   | x::[] -> Some (`Single x)
            ...
            let xs: stru list =
              List.map
                (function
                 ...
                 | `Single ty ->
                     let v = aux ty in
                     (`Type (v :> Astfn.typedecl) :> Astfn.stru)) x in
            Some (sem_of_list xs)) : stru option)
let stru_from_ty ~f:(f : string -> stru)  =
  function
  | (x : mtyps) ->
      (let tys: string list =
         Listf.concat_map
          ...
          (function
           | x ->
               (match Ctyp.qualified_app_list x with
                | Some (i,lst) ->
                    let lst =
                      List.map (function | ctyp -> self#ctyp ctyp) lst in
                    let src = i
                    and dest = IdN.to_string i in
                    (Hashtbl.replace transformers dest
                       (src, (List.length lst));
        ...
        method type_transformers =
          Hashtbl.fold
            (function
             | dest ->
                 (function
                  | (src,len) -> (function | acc -> (dest, src, len) :: acc)))
            transformers []
      end
let transform_mtyps =
  function
      ...
      let item1 =
        List.map
          (function
           | `Mutual ls ->
               `Mutual
                 (List.map (function | (s,ty) -> (s, (obj#typedecl ty))) ls)
