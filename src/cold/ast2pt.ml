...
let mk_constant_pat _loc (x : Astf.literal) =
  (match x with
   | `Chr (_,s) ->
...
let mkvariant (x : Astf.or_ctyp) =
  (let _loc = unsafe_loc_of x in
   ...
   | t -> (Locf.failf _loc "mkvariant %s ") @@ (! dump_or_ctyp t) : (string
                                                                    ...
                                                                    option*
                                                                    Locf.t) )
let type_kind (x : Astf.type_repr) =
  (match x with
   | `Record (_,t) ->
...
let mkvalue_desc loc t (p : Astf.strings list) =
  ({
     pval_type = (ctyp t);
     pval_prim =
       (List.map
...
let paolab (lab : string) (p : Astf.pat) =
  (match (lab, p) with
...
let mk_type_parameters (tl : Astf.opt_decl_params) =
  (match tl with
   ...
   | `Some (_,x) ->
       (List.map
          (function
           | #Astf.decl_param as x -> quote_map (x :>Astf.ctyp)
           | _ -> assert false))
...
let normalize_vid (x : Astf.vid) =
  ...
  | `Ant _ -> assert false
let rec exp_desc _loc (x : Astf.exp) =
  (match x with
       ...
       let cas =
         let rec f (x : Astf.case) =
           (match x with
            ...
            | `Ant (_loc,_) -> ant_error _loc : Astf.case ) in
         f cas in
       exp_desc _loc
         (`App
            (_loc,
  ...
  Listf.filter_map
    (function
     | (`RecBind (_loc,i,e) : Astf.rec_exp) ->
         Some ((ident (i : Astf.vid  :>Astf.ident)), (exp e))
     | x ->
...
and sigi_item_desc _loc (s : Astf.sigi) =
  (match s with
   | `Class (_,cd) ->
...
and sigi (s : Astf.sigi) (l : Parsetree.signature) =
  (match s with
...
and stru_item_desc _loc (s : Astf.stru) =
  (match s with
   | `Class (_,cd) ->
...
and stru (s : Astf.stru) (l : Parsetree.structure) =
  (match s with
   | `StExp (_,`Unit _) -> l
...
and clsigi_desc loc (c : Astf.clsigi) =
  (match c with
...
and clsigi (c : Astf.clsigi) (l : Parsetree.class_type_field list) =
  (match c with
...
let sigi (ast : Astf.sigi) = (sigi ast [] : Parsetree.signature )
let stru ast = stru ast []
let directive (x : Astf.exp) =
  (match x with
