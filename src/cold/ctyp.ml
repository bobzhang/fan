             ...
             (function
              | name ->
                  (function
                   | acc ->
                       (`Arrow
                          ((`Quote (`Normal, (`Lid name))),
                            (acc :> Astfn.ctyp)) :> Astfn.ctyp))) names ty)
let (+>) =
  function
  | (params : ctyp list) ->
      (function | (base : ctyp) -> List.fold_right arrow params base)
      ...
      ((match x with
        ...
        | tydcl ->
            failwithf "name_length_of_tydcl  %s \n"
              (ObjsN.dump_typedecl tydcl)) : (string* int))
let gen_quantifiers1 ~arity  =
  function
  | n ->
      (((Listf.init arity
           (function
...
let of_id_len ~off  =
  ...
  | ((id : ident),len) ->
      appl_of_list ((id :> ctyp) ::
        (Listf.init len
           (function
            | i -> (`Quote (`Normal, (`Lid (allx ~off i))) :> Astfn.ctyp))))
let of_name_len ~off  =
  function | (name,len) -> let id = lid name in of_id_len ~off (id, len)
let gen_ty_of_tydcl ~off  =
  function
  | (tydcl : typedecl) ->
...
let gen_tuple_n =
  function
  | ty -> (function | n -> (Listf.init n (function | _ -> ty)) |> tuple_sta)
let repeat_arrow_n =
  function
  | ty ->
      (function | n -> (Listf.init n (function | _ -> ty)) |> arrow_of_list)
let result_id = ref 0
let mk_method_type ~number  ~prefix  =
  function
  | (id,len) ->
      (function
       | (k : destination) ->
           (let prefix =
              List.map
                (function
                 | s -> Stringf.drop_while (function | c -> c = '_') s)
                prefix in
            let app_src =
              app_arrow @@
                (Listf.init number
                   (function | _ -> of_id_len ~off:0 (id, len))) in
            let result_type =
              (`Quote
                 (`Normal, (`Lid ("result" ^ (string_of_int (!result_id))))) :> 
              Astfn.ctyp) in
            let _ = incr result_id in
            let self_type =
              (`Quote (`Normal, (`Lid "self_type")) :> Astfn.ctyp) in
            let (quant,dst) =
              match k with
              ...
              (let quantifiers = gen_quantifiers1 ~arity:quant len in
               ((`TyPol
                   ((quantifiers :> Astfn.ctyp),
                     (params +> base :> Astfn.ctyp)) :> Astfn.ctyp), dst)) : 
           (ctyp* ctyp)))
let mk_method_type_of_name ~number  ~prefix  =
  function
  | (name,len) ->
      (function
       | (k : destination) ->
...
let is_abstract =
  function
  | (x : typedecl) -> (match x with | `TyAbstr _ -> true | _ -> false)
let abstract_list =
  function
  | (x : typedecl) ->
      (match x with
                          ...
                          | x ->
                              (match (x : or_ctyp) with
                               | `Of (`Uid cons,tys) ->
                                   compose
                                     (f cons
                                        (Ast_basic.N.list_of_star tys []))
                                     acc
                               | `Uid cons -> compose (f cons []) acc
                               | t ->
                                   failwithf "reduce_data_ctors: %s"
                                     (ObjsN.dump_or_ctyp t)))) init branches))
let view_sum =
  function
  | (t : or_ctyp) ->
      ...
      (let lst = Ast_basic.N.list_of_bar t [] in
       List.map
         (function
          ...
          | u -> failwithf "%s %s" "view_variant" (ObjsN.dump_row_field u))
         lst : vbranch list)
...
let transform: full_id_transform -> vid -> exp =
  let open IdN in
    function
    | `Pre pre ->
        (function | x -> (ident_map (function | x -> pre ^ x) x : exp))
    | `Post post ->
        (function | x -> (ident_map (function | x -> x ^ post) x : exp))
    | `Fun f -> (function | x -> ident_map f x)
    | `Last f -> (function | x -> (ident_map_of_ident f x : vid  :> exp))
    | `Id f -> (function | x -> (f x : vid  :> exp))
    | `Idents f ->
        (function | x -> (f (Ast_basic.N.list_of_dot x []) : vid  :> exp))
    | `Obj f ->
        (function
...
let basic_transform =
  function
  | `Pre pre -> (function | x -> pre ^ x)
  | `Post post -> (function | x -> x ^ post)
  | `Fun f -> f
let right_transform =
  function
  | #basic_id_transform as x ->
      let f = basic_transform x in
      (function | x -> (`Lid (f x) :> Astfn.exp))
  | `Exp f -> f
let gen_tuple_abbrev ~arity  ~annot  ~destination  =
  function
  | name ->
      (function
       | e ->
                ...
                | i ->
                    (`Alias
                       ((`ClassPath (name :> Astfn.ident)),
                         (`Lid (x ~off:i 0))) :> Astfn.pat)) in
           let exps =
             (Listf.init arity) @@
               (function | i -> ((xid ~off:i 0 :> Astfn.vid) :> Astfn.exp)) in
