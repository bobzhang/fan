...
let add ?(check= true)  =
  ...
  | ((loc,id),v) ->
      (function
       | env ->
           if check && (List.exists (function | ((_,i),_) -> i = id) (!env))
                  ...
                  (List.iter
                     (function
                      | (((loc,_) as v),opt) ->
                          (match opt with
                           | None  ->
                               add (v, (`Lid (loc, s) :> Astf.exp)) env
                           | Some l ->
                               add
                                 (v,
                                   (`Field
                                      (loc, (`Lid (loc, s)), (`Lid (loc, l))) :> 
                                   Astf.exp)) env)))))
let mk_prule ~prod  ~action  =
  ...
  let prod =
    Listf.filter_map
      (function
       | (p : Gram_def.osymbol list Gram_def.decorate) ->
           (match p with
            | { kind = KSome ; txt } ->
                Some
                  (txt |>
                     ...
                     (function
                      | (symbol : Gram_def.osymbol) ->
                          (match symbol with
                           ...
                           | { outer_pattern = Some (_,id); bounds;_} as
                               symbol ->
                               (enhance_env id bounds env; incr i; symbol)))
                     txt)
            | { kind = KNone ; txt } ->
                 ...
                 (function
                  | (x : Gram_def.text) ->
                      (match x with
                       ...
                       | Keyword (_loc,kwd) ->
                           (`App
                              (_loc, (`Vrn (_loc, "Keyword")),
                                (`Str (_loc, kwd))) :> Astf.exp)
                       | Nterm (_loc,n,lev) ->
                           let obj =
                             (`App
                                (_loc,
                           ...
                           (match lev with
                            ...
                            | None  ->
                                ...
                                then (`Uid (_loc, "Self") :> Astf.exp)
                                  ...
                                  (`App
                                     (_loc, (`Uid (_loc, "Nterm")),
                                       (obj :> Astf.exp)) :> Astf.exp))
                       | Try (_loc,t) ->
                           (`App
                              (_loc, (`Uid (_loc, "Try")),
                                (aux "" t :> Astf.exp)) :> Astf.exp)
                       | Peek (_loc,t) ->
                           (`App
                              (_loc, (`Uid (_loc, "Peek")),
                                (aux "" t :> Astf.exp)) :> Astf.exp)
                       | Token (_loc,meta) ->
                           (`App
                              (_loc, (`Uid (_loc, "Token")),
                                (meta :> Astf.exp)) :> Astf.exp))) in
           aux tvar x)
and make_exp_rules =
  function
  | (rl : (Gram_def.text list* exp* Gram_def.action) list) ->
      (function
       | (tvar : string) ->
           (rl |>
              (List.map
                 (function
                  | (sl,action,(raw : Gram_def.action)) ->
                      let action_string =
                        match raw with
                        | E (None ) -> ""
                        ...
                        | Ant _ -> "" in
                      let sl =
                        (sl |> (List.map (make_exp tvar))) |> list_of_list in
  ...
  | (_loc : loc) ->
      (function
       | (x : Gram_def.rule) ->
           (function
            | (rtvar : string) ->
                (let locid = (`Lid (_loc, (!Locf.name)) :> Astf.pat) in
                 let make_ctyp =
                   function
                   | (styp : Gram_def.styp) ->
                       (function
                                   ...
                                   (match (v : Gram_def.styp) with
                                    ...
                                    | `Self _loc ->
                                        ...
                                        then
                                          (Locf.raise _loc) @@
                                            (Streamf.Error
                                               "S: illegal in anonymous entry level")
                                        else
                                          (`Quote
                                             (_loc, (`Normal _loc),
                                               (`Lid (_loc, tvar))) :> 
                         ...
                         (_loc,
                           (`Dot
                              (_loc, (gm () :> Astf.vid),
                                (`Lid (_loc, "mk_action")))),
                           (`Constraint
                              (_loc, (txt :> Astf.exp), (ty :> Astf.ctyp)))) :> 
                        Astf.exp)) : exp)))
let make_single_extend_statement =
  function
  | (e : Gram_def.entry) ->
       ...
       let ent =
         (`Constraint
            (_loc, ((e.name).id :> Astf.exp),
              (`App
                 (_loc,
                   (`Dot (_loc, (gmid :> Astf.ident), (`Lid (_loc, "t")))),
                   (`Quote
                      (_loc, (`Normal _loc), (`Lid (_loc, ((e.name).tvar)))))))) :> 
         Astf.exp) in
       let pos =
         match e.pos with
         | Some pos ->
             (`App (_loc, (`Uid (_loc, "Some")), (pos :> Astf.exp)) :> 
             Astf.exp)
         | None  -> (`Uid (_loc, "None") :> Astf.exp) in
       let apply =
         function
         | (level : Gram_def.level) ->
             let ass =
                    ...
                    (function
                     | (r : Gram_def.rule) ->
                         let sl =
                           r.prod |>
                             (List.map
                                (function | (s : Gram_def.osymbol) -> s.text)) in
            ...
            (`Record
               (_loc,
                 (`Sem
                    (_loc,
                      (`RecBind
                         (_loc, (`Lid (_loc, "entry")), (ent :> Astf.exp))),
                      (`RecBind
                         (_loc, (`Lid (_loc, "olevel")),
                           (apply l :> Astf.exp))))))),
            (`App
               (_loc,
                 (`Dot
           ...
           (let _loc = (e.name).loc in
            let f =
              ...
              then
                (`Dot
                   (_loc, (gm () :> Astf.vid),
                     (`Lid (_loc, "extend_single"))) :> Astf.exp)
              else
                (`Dot
                   (_loc, (gm () :> Astf.vid),
                     (`Lid (_loc, "unsafe_extend_single"))) :> Astf.exp) in
            (`App
               (_loc, (f :> Astf.exp),
                 (make_single_extend_statement e :> Astf.exp)) :> Astf.exp) : 
           exp))
let make_localbinds =
  function
  | _loc ->
      ...
      (function
       | (x : Gram_def.entries) ->
           (function
            | action ->
                let (_,globals) =
                  List.partition (function | (x : Gram_def.entry) -> x.local)
                    x.items in
                let binds =
                  List.map
                    (function
