         ...
         ({
            ...
            productions =
              [{
                 ...
                 fn =
                   (Gramf.mk_action
                      (fun (__fan_1 : Tokenf.quot)  (r : 'regexp) 
                         (_loc : Locf.t)  ->
                         let x = __fan_1 in
                        ...
                        (let res =
                           ...
                           | Not_found  ->
                               (Fan_warnings.emitf xloc.loc_start
                                ...
                                raise UnboundCase) in
                         res
                           { tokens_opt = (Some l); quot_opt = y; loc = xloc
                           } : 'case ) : Tokenf.txt ->
                                           Tokenf.txt list ->
                                             Tokenf.txt ->
                                               Tokenf.txt ->
                ...
                fn =
                  (Gramf.mk_action
                     (fun (__fan_5 : Tokenf.quot)  _  (l : Tokenf.txt list) 
                        _  (__fan_1 : Tokenf.txt)  _  (_loc : Locf.t)  ->
                        let xloc = __fan_1.loc in
                        let x = __fan_1.txt in
                        let y = __fan_5 in
                        let y = Some y in
                 ...
                 fn =
                   (Gramf.mk_action
                      (fun (__fan_2 : Tokenf.txt)  _  (r1 : 'regexp) 
                         ...
                         (Bind (r1, (xloc, y)) : 'regexp ) : Tokenf.txt ->
                                                               Tokenf.txt ->
                                                                 'regexp ->
                                                                   Locf.t ->
                                                                    'regexp ))
