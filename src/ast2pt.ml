...
let ctyp_long_id (t:FAst.ctyp) : (bool *   Longident.t Location.loc) =
  ...
  | t -> errorf (unsafe_loc_of t) "invalid type %s" (!dump_ctyp t) 
...
and row_field (x:row_field) acc =
  ...
  | t -> errorf (unsafe_loc_of t) "row_field: %s" (!dump_row_field t)
...
and package_type (x : mtyp) =
  ...
  | mt -> errorf (unsafe_loc_of mt) "unexpected package type: %s" (!dump_mtyp mt)
...
let mkvariant (x:or_ctyp) = 
  ...
  | t -> errorf (unsafe_loc_of t) "mkvariant %s " (!dump_or_ctyp t) 
...
let paolab (lab:string) (p:pat) : string =
  ...
  | ("",(`Lid(_loc,i) | `Constraint(_loc,`Lid(_,i),_)))
    -> i
  | ("", p) ->
      errorf (unsafe_loc_of p) "paolab %s" (!dump_pat p)
  | _ -> lab 
...
let quote_map x =
  ...
  |`QuoteAny(_loc,p) ->
      ...
      (None,tuple)
  | _ ->
      errorf (unsafe_loc_of x) "quote_map %s" (!dump_ctyp x)
  ...
  List.filter_map
    (function
      |`Ctyp(_, x) ->
        ...
        |(Some x,v) -> Some (x,v)
        | (None,_) ->
            errorf (unsafe_loc_of t) "class_parameters %s" (!dump_type_parameters t)
        end
      | _ ->  errorf (unsafe_loc_of t) "class_parameters %s" (!dump_type_parameters t) )
    (list_of_com t [])
...
let rec pat (x:pat) =  with pat  match x with 
  ...
  | `App (loc, _, _) as f ->
     ...
     | _ ->
         error (unsafe_loc_of f) "this is not a constructor, it cannot be applied in a pattern"
     end
  ...
  | `ModuleConstraint (loc,`Uid(sloc,m),ty) ->
          mkpat loc
            (Ppat_constraint
               ...
               (ctyp ty))
  |  p -> errorf (unsafe_loc_of p) "invalid pattern %s" (!dump_pat p) 
...
let rec exp (x : exp) = with exp match x with 
  ...
  | `ArrayDot (loc, e1, e2) ->
      ...
      | `Fun (loc,`CaseWhen(_,`OptLablS(_,`Lid(sloc,lab)),w,e2)) ->
          mkexp loc
            (Pexp_function ("?" ^ lab) None
               [(pat (`Lid(sloc,lab)), mkexp (unsafe_loc_of w) (Pexp_when (exp w) (exp e2)))])
      ...
      | x -> errorf (unsafe_loc_of x ) "exp:%s" (!dump_exp x)
and label_exp (x : exp) =
  match x with 
  ...
  | {:bind@_loc| $(pat: {:pat@sloc| $lid:bind_name |} ) =
    ($e : $(`TyTypePol (_, vs, ty))) |} ->
      let rec id_to_string (x:ctyp) =
        ...
        | x -> errorf (unsafe_loc_of x) "id_to_string %s" (!dump_ctyp x) in
  ...
  List.filter_map
    (function
      ...
      | {:case| $pat:p when $w -> $e |} ->
          Some (pat p,
            mkexp (unsafe_loc_of w) (Pexp_when (exp w) (exp e)))
      | x -> errorf (unsafe_loc_of x ) "case %s" (!dump_case x ) ) cases
...
and mklabexp (x:rec_exp)  =
    ...
    List.filter_map
      (function
        ...
        |  x ->errorf (unsafe_loc_of x) "mklabexp : %s" (!dump_rec_exp x) ) binds
  ...
  let type_decl tl cl loc (x:type_info) =
    ...
    List.map
      (function 
            ...
            | `Some(_,cl) ->
              ...
              |> List.map
                  (function
                    |`Eq(loc,t1,t2) ->
                      (ctyp t1, ctyp t2, loc)
                    | _ -> errorf (unsafe_loc_of x) "invalid constraint: %s" (!dump_type_constr cl)) in
          (with_loc c sloc,
           type_decl
        ...
        | (t:typedecl) ->
            errorf (unsafe_loc_of t) "mktype_decl %s" (!dump_typedecl t)) tys
and mtyp : FAst.mtyp -> Parsetree.module_type =
  let  mkwithc (wc:constr)  =
    ...
    List.filter_map (function
      ...
      | t -> errorf (unsafe_loc_of t) "bad with constraint (antiquotation) : %s" (!dump_constr t)) constrs in
       function 
       ...
       | t -> errorf (unsafe_loc_of t) "mtyp: %s" (!dump_mtyp t) 
and sigi (s:sigi) (l:signature) :signature =
  ...
  | t -> errorf (unsafe_loc_of t) "sigi: %s" (!dump_sigi t)
and module_sig_bind (x:mbind) 
  ...
  | t -> errorf (unsafe_loc_of t) "module_sig_bind: %s" (!dump_mbind t) 
and module_str_bind (x:FAst.mbind) acc =
  ...
  | t -> errorf (unsafe_loc_of t) "module_str_bind: %s" (!dump_mbind t)
and mexp (x:FAst.mexp)=
  ...
  | t -> errorf (unsafe_loc_of t) "mexp: %s" (!dump_mexp t) 
and stru (s:stru) (l:structure) : structure =
  ...
  | x-> errorf (unsafe_loc_of x) "stru : %s" (!dump_stru x) 
and cltyp (x:FAst.cltyp) =
  ...
  |  x -> errorf (unsafe_loc_of x) "class type: %s" (!dump_cltyp x) 
...
and class_info_clexp (ci:cldecl) =
  ...
  | `ClDeclS(loc,vir,`Lid(nloc,name),ce) -> 
        {pci_virt = mkvirtual vir;
         ...
         pci_loc =  loc;
         pci_variance = []}
  | ce -> errorf  (unsafe_loc_of ce) "class_info_clexp: %s" (!dump_cldecl ce) 
and class_info_cltyp (ci:cltdecl)  =
  ...
  | (`CtDeclS (loc,vir,`Lid(nloc,name),ct) : cltdecl) -> 
        {pci_virt = mkvirtual vir;
         ...
         pci_loc =  loc;
         pci_variance = []}
  | ct -> errorf (unsafe_loc_of ct) "bad class/class type declaration/definition %s " (!dump_cltdecl ct)
  and clsigi (c:clsigi) (l:  class_type_field list) : class_type_field list =
    ...
    | t -> errorf (unsafe_loc_of t) "clsigi :%s" (!dump_clsigi t) 
and clexp  (x:FAst.clexp) =
  ...
  | t -> errorf (unsafe_loc_of t) "clexp: %s" (!dump_clexp t)
...
and clfield (c:clfield) l =
  ...
  | x  -> errorf  (unsafe_loc_of  x) "clfield: %s" (!dump_clfield x) 
