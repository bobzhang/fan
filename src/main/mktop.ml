...
let _ = begin (* FIXME make the printer more restict later *)
  of_stru_with_filter ~name:{domains; name = "ocaml"} ~entry:strus
    ~filter:(fun s  ->
...
let domains = Ns.macro

let _ = begin 
  of_exp_with_filter ~name:{domains; name =  "exp"} ~entry:exp
    ~filter:(Ast_macros.macro_expander#exp) ();
  of_clfield_with_filter ~name:{domains; name =  "clfield"} ~entry:clfield
    ~filter:(Ast_macros.macro_expander#clfield) ();
  of_stru_with_filter ~name:{domains; name =  "stru"} ~entry:stru
    ~filter:(Ast_macros.macro_expander#stru)
end
    
let domains = `Absolute ["Fan"; "Lang"; "Meta"]

let _ = begin 
  add_quotation {domains; name =  "sigi'"} sigi_quot ~mexp:(Filters.me#sigi)
    ~mpat:(Filters.mp#sigi) ~exp_filter ~pat_filter;
  add_quotation {domains; name =  "stru'"} stru_quot ~mexp:(Filters.me#stru)
    ~mpat:(Filters.mp#stru) ~exp_filter ~pat_filter;
  add_quotation {domains; name =  "ctyp'"} ctyp_quot ~mexp:(Filters.me#ctyp)
    ~mpat:(Filters.mp#ctyp) ~exp_filter ~pat_filter;
  add_quotation {domains; name =  "pat'"} pat_quot ~mexp:(Filters.me#pat)
    ~mpat:(Filters.mp#pat) ~exp_filter ~pat_filter;
  add_quotation {domains; name =  "exp'"} exp_quot ~mexp:(Filters.me#exp)
    ~mpat:(Filters.mp#exp) ~exp_filter ~pat_filter;
  add_quotation {domains; name =  "mtyp'"} mtyp_quot ~mexp:(Filters.me#mtyp)
    ~mpat:(Filters.mp#mtyp) ~exp_filter ~pat_filter;
  add_quotation {domains; name =  "mexp'"} mexp_quot ~mexp:(Filters.me#mexp)
    ~mpat:(Filters.mp#mexp) ~exp_filter ~pat_filter;
  add_quotation {domains; name =  "cltyp'"} cltyp_quot ~mexp:(Filters.me#cltyp)
    ~mpat:(Filters.mp#cltyp) ~exp_filter ~pat_filter;
  add_quotation {domains; name =  "clexp'"} clexp_quot ~mexp:(Filters.me#clexp)
    ~mpat:(Filters.mp#clexp) ~exp_filter ~pat_filter;
  add_quotation {domains; name =  "clsigi'"} clsigi_quot ~mexp:(Filters.me#clsigi)
    ~mpat:(Filters.mp#clsigi) ~exp_filter ~pat_filter;
  add_quotation {domains; name =  "clfield'"} clfield_quot ~mexp:(Filters.me#clfield)
    ~mpat:(Filters.mp#clfield) ~exp_filter ~pat_filter;
  add_quotation {domains; name =  "constr'"} constr_quot ~mexp:(Filters.me#constr)
    ~mpat:(Filters.mp#constr) ~exp_filter ~pat_filter;
  add_quotation {domains; name =  "bind'"} bind_quot ~mexp:(Filters.me#bind)
    ~mpat:(Filters.mp#bind) ~exp_filter ~pat_filter;
  add_quotation {domains; name =  "rec_exp'"} rec_exp_quot ~mexp:(Filters.me#rec_exp)
    ~mpat:(Filters.mp#rec_exp) ~exp_filter ~pat_filter;
  add_quotation {domains; name =  "case'"} case_quot ~mexp:(Filters.me#case)
    ~mpat:(Filters.mp#case) ~exp_filter ~pat_filter;
  add_quotation {domains; name =  "mbind'"} mbind_quot ~mexp:(Filters.me#mbind)
    ~mpat:(Filters.mp#mbind) ~exp_filter ~pat_filter;
  add_quotation {domains; name =  "ident'"} ident_quot ~mexp:(Filters.me#ident)
    ~mpat:(Filters.mp#ident) ~exp_filter ~pat_filter;
  add_quotation {domains; name =  "rec_flag'"} rec_flag_quot ~mexp:(Filters.me#flag)
    ~mpat:(Filters.mp#flag) ~exp_filter ~pat_filter;
  add_quotation {domains; name =  "private_flag'"} private_flag_quot
    ~mexp:(Filters.me#flag) ~mpat:(Filters.mp#flag)
    ~exp_filter ~pat_filter;
  add_quotation {domains; name =  "row_var_flag'"} row_var_flag_quot
    ~mexp:(Filters.me#flag) ~mpat:(Filters.mp#flag)
    ~exp_filter ~pat_filter;
  add_quotation {domains; name =  "mutable_flag'"} mutable_flag_quot
    ~mexp:(Filters.me#flag) ~mpat:(Filters.mp#flag)
    ~exp_filter ~pat_filter;
  add_quotation {domains; name =  "virtual_flag'"} virtual_flag_quot
    ~mexp:(Filters.me#flag) ~mpat:(Filters.mp#flag)
    ~exp_filter ~pat_filter;
  add_quotation {domains; name =  "override_flag'"} override_flag_quot
    ~mexp:(Filters.me#flag) ~mpat:(Filters.mp#flag)
    ~exp_filter ~pat_filter;
  add_quotation {domains; name =  "direction_flag'"} direction_flag_quot
    ~mexp:(Filters.me#flag) ~mpat:(Filters.mp#flag)
    ~exp_filter ~pat_filter;
  add_quotation {domains; name =  "or_ctyp'"} constructor_declarations
    ~mexp:(Filters.me#or_ctyp) ~mpat:(Filters.me#or_ctyp) ~exp_filter
    ~pat_filter;
  add_quotation {domains; name =  "row_field'"} row_field ~mexp:(Filters.me#row_field)
...
let _ = begin
  ...
  add_quotation {domains; name =  "ep"} exp_quot ~mexp:(Filters.me#exp)
  ...
  of_exp ~name:{domains; name =  "with_exp"} ~entry:with_exp_lang ();
  of_stru ~name:{domains; name =  "with_stru"} ~entry:with_stru_lang ();
  add {domains; name =  "str"} Dyn_tag.exp
    (fun _loc  _loc_option  s  -> `Str (_loc, s));
  add {domains; name =  "str"} Dyn_tag.stru
...
begin
    add_quotation {domains; name =  "sigi-"} sigi_quot ~mexp:(fun loc p -> m#sigi loc (Objs.strip_sigi p))
    ~mpat:(fun loc p -> m#sigi loc (Objs.strip_sigi p))
     ~exp_filter:(efilter "sigi")
    ~pat_filter:(pfilter "sigi");
  add_quotation {domains; name =  "stru-"} stru_quot ~mexp:(fun loc p -> m#stru loc (Objs.strip_stru p))
    ~mpat:(fun loc p -> m#stru loc (Objs.strip_stru p)) ~exp_filter:(efilter "stru")
    ~pat_filter:(pfilter "stru");
  add_quotation {domains; name =  "ctyp-"} ctyp_quot ~mexp:(fun loc p -> m#ctyp loc (Objs.strip_ctyp p))
    ~mpat:(fun loc p -> m#ctyp loc (Objs.strip_ctyp p)) ~exp_filter:(efilter "ctyp")
    ~pat_filter:(pfilter "ctyp");
  add_quotation {domains; name =  "pat-"} pat_quot ~mexp:(fun loc p -> m#pat loc (Objs.strip_pat p))
    ~mpat:(fun loc p -> m#pat loc (Objs.strip_pat p)) ~exp_filter:(efilter "pat")
    ~pat_filter:(pfilter "pat");
  add_quotation {domains; name =  "ep-"} exp_quot ~mexp:(fun loc p -> m#exp loc (Objs.strip_exp p))
    ~mpat:(fun loc p -> m#exp loc (Objs.strip_exp p)) ~exp_filter:(efilter "ep")
    ~pat_filter:(pfilter "ep");
  add_quotation {domains; name =  "exp-"} exp_quot
    ~mexp:(fun loc p -> m#exp loc (Objs.strip_exp p))
    ~mpat:(fun loc p -> m#exp loc (Objs.strip_exp p))
    ~exp_filter:(efilter "exp")
    ~pat_filter:(pfilter "exp");
  add_quotation {domains; name =  "mtyp-"} mtyp_quot ~mexp:(fun loc p -> m#mtyp loc (Objs.strip_mtyp p))
    ~mpat:(fun loc p -> m#mtyp loc (Objs.strip_mtyp p)) ~exp_filter:(efilter "mtyp")
    ~pat_filter:(pfilter "mtyp");
  add_quotation {domains; name =  "mexp-"} mexp_quot ~mexp:(fun loc p -> m#mexp loc (Objs.strip_mexp p))
    ~mpat:(fun loc p -> m#mexp loc (Objs.strip_mexp p)) ~exp_filter:(efilter "mexp")
    ~pat_filter:(pfilter "mexp");
  add_quotation {domains; name =  "cltyp-"} cltyp_quot ~mexp:(fun loc p -> m#cltyp loc (Objs.strip_cltyp p))
    ~mpat:(fun loc p -> m#cltyp loc (Objs.strip_cltyp p)) ~exp_filter:(efilter "cltyp")
    ~pat_filter:(pfilter "cltyp");
  add_quotation {domains; name =  "clexp-"} clexp_quot ~mexp:(fun loc p -> m#clexp loc (Objs.strip_clexp p))
    ~mpat:(fun loc p -> m#clexp loc (Objs.strip_clexp p)) ~exp_filter:(efilter "clexp")
    ~pat_filter:(pfilter "clexp");
  add_quotation {domains; name =  "clsigi-"} clsigi_quot ~mexp:(fun loc p -> m#clsigi loc (Objs.strip_clsigi p))
    ~mpat:(fun loc p -> m#clsigi loc (Objs.strip_clsigi p)) ~exp_filter:(efilter "clsigi")
    ~pat_filter:(pfilter "clsigi");
  add_quotation {domains; name =  "clfield-"} clfield_quot ~mexp:(fun loc p -> m#clfield loc (Objs.strip_clfield p))
    ~mpat:(fun loc p -> m#clfield loc (Objs.strip_clfield p)) ~exp_filter:(efilter "clfield")
    ~pat_filter:(pfilter "clfield");
  add_quotation {domains; name =  "constr-"} constr_quot ~mexp:(fun loc p -> m#constr loc (Objs.strip_constr p))
    ~mpat:(fun loc p -> m#constr loc (Objs.strip_constr p)) ~exp_filter:(efilter "constr")
    ~pat_filter:(pfilter "constr");
  add_quotation {domains; name =  "bind-"} bind_quot ~mexp:(fun loc p -> m#bind loc (Objs.strip_bind p))
    ~mpat:(fun loc p -> m#bind loc (Objs.strip_bind p)) ~exp_filter:(efilter "bind")
    ~pat_filter:(pfilter "bind");
  add_quotation {domains; name =  "rec_exp-"} rec_exp_quot ~mexp:(fun loc p -> m#rec_exp loc (Objs.strip_rec_exp p))
    ~mpat:(fun loc p -> m#rec_exp loc (Objs.strip_rec_exp p)) ~exp_filter:(efilter "rec_exp")
    ~pat_filter:(pfilter "rec_exp");
  add_quotation {domains; name =  "case-"} case_quot ~mexp:(fun loc p -> m#case loc (Objs.strip_case p))
    ~mpat:(fun loc p -> m#case loc (Objs.strip_case p)) ~exp_filter:(efilter "case")
    ~pat_filter:(pfilter "case");
  add_quotation {domains; name =  "mbind-"} mbind_quot ~mexp:(fun loc p -> m#mbind loc (Objs.strip_mbind p))
    ~mpat:(fun loc p -> m#mbind loc (Objs.strip_mbind p)) ~exp_filter:(efilter "mbind")
    ~pat_filter:(pfilter "mbind");
  add_quotation {domains; name =  "ident-"} ident_quot ~mexp:(fun loc p -> m#ident loc (Objs.strip_ident p))
  ...
  add_quotation {domains; name =  "or_ctyp-"} constructor_declarations
    ...
    ~exp_filter:(efilter "or_ctyp") ~pat_filter:(pfilter "or_ctyp");
  add_quotation {domains; name =  "row_field-"} row_field ~mexp:(fun loc p -> m#row_field loc (Objs.strip_row_field p))
...
let ()  =
  of_exp ~name:{domains; name = "p"} ~entry:p () ;;
...
let () =
  of_stru ~name:{domains; name = "import"} ~entry:import ();;
...
let () =
  ...
  let f2 = fun (loc:Locf.t) _meta _content ->
    ...
    %stru@loc{$str:s} in
  begin 
    Ast_quotation.add {domains; name = "here"} Dyn_tag.exp f;
    Ast_quotation.add {domains; name = "here"} Dyn_tag.stru f2
  end
