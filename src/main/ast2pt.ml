...
let mk_constant_exp _loc (x: Astf.literal) : Parsetree.expression_desc =
  ...
  | `Str (_,s) -> Pexp_constant (Const_string (Escape.string_of_string_token _loc s))
  | `Bool(_,b) ->
...
let mk_constant_pat _loc ( x : Astf.literal) : Parsetree.pattern_desc =
  ...
  | `Str (_,s) -> Ppat_constant (Const_string (Escape.string_of_string_token _loc s))
  | `Bool (_,b) ->
...
(***    Utils                        *)
(*************************************)                                                                           
let mktrecord (x: Astf.name_ctyp) :
...
let mkvalue_desc loc t (p:  Astf.strings list) : Parsetree.value_description =
  {pval_type = ctyp t;
   pval_prim =
   List.map (fun p ->
     match p with
...
(* ['a,'b,'c']*)
let  class_parameters (t : Astf.type_parameters) =
...
let type_parameters_and_type_name (t : Astf.ctyp)  =
  let rec aux (t : Astf.ctyp) acc = 
    match t with
    |`App(_,t1,t2) ->
      aux (t1 :> Astf.ctyp) (optional_type_parameters (t2 :> Astf.ctyp) @ acc)
    | #Astf.ident' as i  -> (ident i, acc)
    | x ->
...
and pat (x : Astf.pat) : Parsetree.pattern =
  let _loc = unsafe_loc_of x in
  {ppat_desc =  pat_desc _loc x ; ppat_loc = _loc} 
...
let rec exp_desc _loc (x : Astf.exp) : Parsetree.expression_desc =
  ...
  | `For (_loc, `Lid(loc,txt), e1, e2, df, el) ->
      Pexp_for
         ({txt;loc}, exp e1, exp e2, mkdirection df, exp (`Seq (_loc, el)))
  | `Fun(_,
         `Case(_,
...
and top_bind (x : Astf.bind)  =
  let  rec aux (x : Astf.bind) acc =
  match x with
  ...
  | `Bind(_loc,`Lid(sloc,bind_name), e ) ->
      ...
      |  _ ->
          (mkpat sloc (Ppat_var {loc = sloc;txt = bind_name}) , exp e) :: acc 
      end)
...
and mktype_decl (x : Astf.typedecl)  =
  let type_decl tl cl loc (x : Astf.type_info) =
    match x with
    |`TyMan(_,t1,p,t2) ->
      mktype loc tl cl ~type_kind:(type_kind t2) ~priv:(mkprivate p) ~manifest:(Some (ctyp t1))
    | `TyRepr (_,p1,repr) ->
      mktype loc tl cl ~type_kind:(type_kind repr) ~priv:(mkprivate p1) ~manifest:None
    | `TyEq (_loc,p1,t1) ->
      mktype loc tl cl ~type_kind:(Ptype_abstract) ~priv:(mkprivate p1) ~manifest:(Some (ctyp t1 ))
    | `Ant (_loc,_) -> ant_error _loc  in
...
and  mkwithc (wc : Astf.constr)  =
  ...
  Listf.filter_map (function
    ...
    | t -> Locf.failf (unsafe_loc_of t) "bad with constraint (antiquotation) : %s" (!dump_constr t)) constrs 
...
and mexp (x:Astf.mexp) : Parsetree.module_expr =
  let _loc = unsafe_loc_of x in
  {pmod_desc = mexp_desc _loc x ; pmod_loc = _loc}
...
and cltyp_desc loc (x:Astf.cltyp) : Parsetree.class_type_desc =
  ...
  | `ObjEnd _ ->
      Pcty_signature {pcsig_self= mktyp loc Ptyp_any; pcsig_fields=[];pcsig_loc=loc}
  | `ObjTyEnd(_,t) ->
...
and class_info_clexp (ci : Astf.cldecl) =
  match ci with 
...
and clfield_desc loc (c:Astf.clfield) : Parsetree.class_field_desc =
  ...
  | `Inherit (_, ov, ce) ->
      Pcf_inher (flag loc ov,clexp ce, None)
  ...
  | `CrMthS(loc,`Lid(sloc,s),ov,pf,e) ->
    Pcf_meth ({loc=sloc; txt = s}, mkprivate pf, flag loc ov, mkexp loc (Pexp_poly (exp e,None)))
