...
(* | '(' ocaml_blank* ("or"|"mod"|"land"|"lor"|"lxor"|"lsl"|"lsr"|"asr" as op) ocaml_blank* ')' ->   {| (`Eident op, !! lexbuf) |} *)
...
(* (\* quotation handling *\) *)
(* | "{||}" ->  {| (`Quot { Ftoken.name=Ftoken.empty_name; loc=None; shift=2; content="" }, !! lexbuf) |} *)
...
(*            (`Ant(name,buff_contents c), old -- Lexing.lexeme_end_p lexbuf) *)
(*          end |} *)
(*      | '(' ->       (\* $(xxxx)*\) {| *)
