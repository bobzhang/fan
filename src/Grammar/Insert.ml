...
let change_lev entry lev n lname assoc =
  let a = match assoc with
      [ None -> lev.assoc
      | Some a -> begin 
          if a <> lev.assoc && !(entry.egram.warning_verbose) then begin 
...
let get_level entry position levs = match position with
...
and tree_check_gram entry = fun
  [ Node {node = n; brother = bro; son = son} -> begin 
    check_gram entry n;
...
let insert_tree entry gsymbols action tree =
  let rec insert symbols tree =  match symbols with
  ...
  and insert_in_tree s sl tree = match try_insert s sl tree with
  [ Some t -> t
  | None -> Node {node = s; son = insert sl DeadEnd; brother = tree} ]
  and try_insert s sl tree = match tree with
