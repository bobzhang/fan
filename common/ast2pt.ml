          ...
          let cas =
            let rec f x =
              ...
              | (`Case (_loc,p,e) : FAst.case) ->
                  (`Case
                     (_loc, p, (`Fun (_loc, (`Case (_loc, (`Uid (_loc, "()")), e))))) : 
                     FAst.case )
...
and case (x:case) = 
  ...
  | x -> errorf (unsafe_loc_of x) "case %s" (dump_case.contents x))
  cases
...
and mklabexp (x:rec_exp)  =
    ...
    List.filter_map
      (function
         ...
         | x -> errorf (unsafe_loc_of x) "mklabexp : %s" (dump_rec_exp.contents x))
      binds
