      ...
      (function
       | pairs ->
           ...
           let () =
             List.iter
               (function
                | ((k,kloc),(v,vloc)) -> Hashtbl.add tbl k (kloc, v, vloc))
               pairs in
           ((try
...
let rec token: Lexing.lexbuf -> Tokenf.t =
  ...
  | (lexbuf : Lexing.lexbuf) ->
      ...
      and __ocaml_lex_state4 = function | () -> 3
            ...
            (lexbuf.lex_last_pos <- lexbuf.lex_curr_pos;
             ...
             (match __ocaml_lex_next_char () with
              ...
              | _ ->
                  (lexbuf.lex_curr_pos <- lexbuf.lex_last_pos;
                   lexbuf.lex_last_action)))
      and __ocaml_lex_state7 = function | () -> 1
      ...
      and __ocaml_lex_state10 =
        function | () -> ((lexbuf.lex_mem).(0) <- (lexbuf.lex_mem).(1); 4) in
                             ...
                             (Gramf.mk_action
                                (function
                                 | (pairs : 'pair list) ->
                                     (function
                                      | (_loc : Locf.t) ->
                                          (compile _loc pairs : 'register)) : 
                                'pair list -> Locf.t -> 'register))
                         }]
                    } : Gramf.olevel)
