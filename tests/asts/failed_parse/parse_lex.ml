              ...
              | _loc ->
                  (function
                   | (a,b) ->
                       ...
                       Astf.ep))) _loc x)
let meta_concrete_regexp =
  function
  | _loc ->
      (function
       | (x : Translate_lex.concrete_regexp) ->
           let rec aux =
             function
             | _loc ->
                 (function
                  | (x : Translate_lex.concrete_regexp) ->
                      (match x with
                       ...
                       | Characters a ->
                           (`App
                              (_loc, (`Uid (_loc, "Characters")),
                                (meta_cset _loc a :> Astf.ep)) :> Astf.ep)
                       | Sequence (a0,a1) ->
                           (`App
                       ...
                       | Repetition a ->
                           (`App
                              (_loc, (`Uid (_loc, "Repetition")),
                                (aux _loc a :> Astf.ep)) :> Astf.ep)
                       | Bind (a,(loc,s)) ->
                           (`App
                              (_loc, (`Uid (_loc, "Bind")),
                                (`Par
            ...
            | l ->
                Compile_lex.output_entry loc
                  (Lexgen.make_single_dfa
                     { shortest; clauses = (Listf.concat l) })))
let make_lex =
  function
  | nt ->
      (function
...
let _ =
  make_lex lex
    (function
     | l -> (function | _ -> (function | loc -> make_automata loc false l)))
    (function
     | l -> (function | _ -> (function | loc -> make_automata loc true l)));
  make_lex lex_fan
    (function
     | l ->
         (function
...
let _ =
                           ...
                           | _ ->
                               (function
                                | (_loc : Locf.t) ->
                                    ...
                                    (let res =
                                       try
                                         Hashtbl.find Lex_predef.named_cases
                                           x
                                       with
                                       | Not_found  ->
                                           (Fan_warnings.emitf xloc.loc_start
                                              "Reference to unbound case name %s"
                                              x;
                                            raise UnboundCase) in
                                     res
                                       {
              ...
              };
              {
                ...
                fn =
                  (Gramf.mk_action
                     ...
                     Tokenf.txt ->
                       'char_class -> Tokenf.txt -> Locf.t -> 'regexp))
              };
              {
                symbols =
                  [Self;
              ...
              };
              {
                ...
                fn =
                  (Gramf.mk_action
                     ...
                     Tokenf.txt -> 'regexp -> Tokenf.txt -> Locf.t -> 'regexp))
              };
              {
                symbols =
                  [Token
  ...
  Gramf.extend_single
    ({
       entry = (char_class : 'char_class Gramf.t);
       olevel =
         ({
            label = None;
            lassoc = true;
              ...
              [{
                 ...
                 fn =
                   (Gramf.mk_action
                      ...
                      'char_class1 -> Tokenf.txt -> Locf.t -> 'char_class))
               };
              {
                symbols =
                  [Nterm (Gramf.obj (char_class1 : 'char_class1 Gramf.t))];
                ...
                fn =
                  (Gramf.mk_action
                     (function
                      | (r : 'char_class1) ->
                          (function | (_loc : Locf.t) -> (r : 'char_class)) : 
                     'char_class1 -> Locf.t -> 'char_class))
              }]
          } : Gramf.olevel)
     } : _ Gramf.single_extend_statement);
  Gramf.extend_single
    ({
       entry = (char_class1 : 'char_class1 Gramf.t);
       olevel =
         ({
            label = None;
            lassoc = true;
                                 ...
                                 | (__fan_0 : Tokenf.txt) ->
                                     (function
                                      | (_loc : Locf.t) ->
                                          ...
                                          (let c1 =
                                             Char.code @@ (Escape.char c1) in
                                           let c2 =
                                             Char.code @@ (Escape.char c2) in
                                           Fcset.interval c1 c2 : 'char_class1)))) : 
                      Tokenf.txt ->
                        Tokenf.txt -> Tokenf.txt -> Locf.t -> 'char_class1))
               };
              {
                symbols =
                  [Token
                      ...
                      | (__fan_0 : Tokenf.txt) ->
                          (function
                           | (_loc : Locf.t) ->
                               let c1 = __fan_0.txt in
                               (Fcset.singleton
                                  (Char.code @@ (Escape.char c1)) : 'char_class1)) : 
                     Tokenf.txt -> Locf.t -> 'char_class1))
              };
              {
                symbols = [Self; Self];
  ...
  Gramf.extend_single
    ({
       entry = (declare_regexp : 'declare_regexp Gramf.t);
       olevel =
         ({
            label = None;
            lassoc = true;
                 ...
                 fn =
                   (Gramf.mk_action
                      ...
                      'regexp ->
                        Tokenf.txt ->
                          Tokenf.txt ->
                            Tokenf.txt -> Locf.t -> 'declare_regexp))
               };
              {
                symbols = [Self; Self];
                annot = "x\n";
                fn =
                  (Gramf.mk_action
                     ...
                     'declare_regexp ->
                       'declare_regexp -> Locf.t -> 'declare_regexp))
              }]
          } : Gramf.olevel)
     } : _ Gramf.single_extend_statement)
let () =
  ...
  Ast_quotation.add_quotation ~lexer:Lex_lex.from_stream
    ...
    ~exp_filter:(function | x -> (x : Astf.ep  :> Astf.exp))
    ~pat_filter:(function | x -> (x : Astf.ep  :> Astf.pat))
