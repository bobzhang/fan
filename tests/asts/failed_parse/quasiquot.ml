           ...
           | (x : Astf.pat) ->
               (match x with
                | `Ant (_loc,x) ->
                    let meta_loc_pat =
                      function
                      | _loc -> (function | _ -> (`Any _loc :> Astf.pat)) in
                    let mloc = function | _loc -> meta_loc_pat _loc _loc in
                          ...
                          (function
                           | loc ->
                               (match !Ast_quotation.current_loc_name with
                                | Some "here" ->
                                    ((Ast_gen.meta_here _loc loc :> Astf.exp) :> 
                                    Astf.exp)
                                | x ->
                                    let x = Option.default (!Locf.name) x in
                                    (`Lid (_loc, x) :> Astf.exp))) in
                    let mloc = function | _loc -> meta_loc_exp _loc _loc in
                    let e = Tokenf.ant_expand Parsef.exp x in
                    (match ((x.kind), (x.cxt)) with
                     | (("uid"|"lid"|"flo"|"int"|"int32"|"int64"|"nativeint"
                              ...
                              (`Par
                                 (_loc,
                                   (`Com
                                      (_loc, (mloc _loc :> Astf.exp),
                                        (e :> Astf.exp)))))) :> Astf.exp)
                     | (("nativeint'"|"int'"|"int32'"|"int64'"|"chr'"|"str'"
                         |"flo'"|"bool'" as x),_) ->
                         let v =
                           match List.assoc x stringnize with
                           | Some x ->
                               let x = Fill.exp _loc x in
                               (`App (_loc, (x :> Astf.exp), (e :> Astf.exp)) :> 
                                 Astf.exp)
                           | None  -> e in
                         let s =
...
let u = expander true
let exp_filter = function | (x : ep) -> v#exp (x :> exp)
let pat_filter = function | (x : ep) -> v#pat (x :> pat)
let efilter =
  function
  | str ->
      (function
       | (e : ep) ->
           ...
           (`Subtype
              (_loc, (e :> Astf.exp),
                (`Dot (_loc, (`Uid (_loc, "Astf")), (`Lid (_loc, str))))) :> 
             Astf.exp))
let pfilter =
  function
  | str ->
...
let mp =
  object
    inherit  Metaf.meta
    method! loc = function | _loc -> (function | _ -> `Any _loc)
  end
...
let exp_filter_n = function | (x : ep) -> v#exp (x :> exp)
let pat_filter_n = function | (x : ep) -> v#pat (x :> pat)
let efilter =
  function
  | str ->
      (function
       | (e : ep) ->
           ...
           (`Subtype
              (_loc, (e :> Astf.exp),
                (`Dot (_loc, (`Uid (_loc, "Astfn")), (`Lid (_loc, str))))) :> 
             Astf.exp))
let pfilter =
  function
  | str ->
...
let _ =
  add_quotation { domain; name = "sigi-" } sigi_quot
    ~mexp:(function | loc -> (function | p -> m#sigi loc (Strip.sigi p)))
    ~mpat:(function | loc -> (function | p -> m#sigi loc (Strip.sigi p)))
    ~exp_filter:(efilter "sigi") ~pat_filter:(pfilter "sigi");
  add_quotation { domain; name = "stru-" } stru_quot
    ~mexp:(function | loc -> (function | p -> m#stru loc (Strip.stru p)))
    ~mpat:(function | loc -> (function | p -> m#stru loc (Strip.stru p)))
    ~exp_filter:(efilter "stru") ~pat_filter:(pfilter "stru");
  add_quotation { domain; name = "ctyp-" } ctyp_quot
    ~mexp:(function | loc -> (function | p -> m#ctyp loc (Strip.ctyp p)))
    ~mpat:(function | loc -> (function | p -> m#ctyp loc (Strip.ctyp p)))
    ~exp_filter:(efilter "ctyp") ~pat_filter:(pfilter "ctyp");
  add_quotation { domain; name = "pat-" } pat_quot
    ~mexp:(function | loc -> (function | p -> m#pat loc (Strip.pat p)))
    ~mpat:(function | loc -> (function | p -> m#pat loc (Strip.pat p)))
    ~exp_filter:(efilter "pat") ~pat_filter:(pfilter "pat");
  add_quotation { domain; name = "ep-" } ep
    ~mexp:(function | loc -> (function | p -> m#ep loc (Strip.ep p)))
    ~mpat:(function | loc -> (function | p -> m#ep loc (Strip.ep p)))
    ~exp_filter:(efilter "ep") ~pat_filter:(pfilter "ep");
  add_quotation { domain; name = "exp-" } exp_quot
    ~mexp:(function | loc -> (function | p -> m#exp loc (Strip.exp p)))
    ~mpat:(function | loc -> (function | p -> m#exp loc (Strip.exp p)))
    ~exp_filter:(efilter "exp") ~pat_filter:(pfilter "exp");
  add_quotation { domain; name = "mtyp-" } mtyp_quot
    ~mexp:(function | loc -> (function | p -> m#mtyp loc (Strip.mtyp p)))
    ~mpat:(function | loc -> (function | p -> m#mtyp loc (Strip.mtyp p)))
    ~exp_filter:(efilter "mtyp") ~pat_filter:(pfilter "mtyp");
  add_quotation { domain; name = "mexp-" } mexp_quot
    ~mexp:(function | loc -> (function | p -> m#mexp loc (Strip.mexp p)))
    ~mpat:(function | loc -> (function | p -> m#mexp loc (Strip.mexp p)))
    ~exp_filter:(efilter "mexp") ~pat_filter:(pfilter "mexp");
  add_quotation { domain; name = "cltyp-" } cltyp_quot
    ~mexp:(function | loc -> (function | p -> m#cltyp loc (Strip.cltyp p)))
    ~mpat:(function | loc -> (function | p -> m#cltyp loc (Strip.cltyp p)))
    ~exp_filter:(efilter "cltyp") ~pat_filter:(pfilter "cltyp");
  add_quotation { domain; name = "clexp-" } clexp_quot
    ~mexp:(function | loc -> (function | p -> m#clexp loc (Strip.clexp p)))
    ~mpat:(function | loc -> (function | p -> m#clexp loc (Strip.clexp p)))
    ~exp_filter:(efilter "clexp") ~pat_filter:(pfilter "clexp");
  add_quotation { domain; name = "clsigi-" } clsigi_quot
    ~mexp:(function | loc -> (function | p -> m#clsigi loc (Strip.clsigi p)))
    ~mpat:(function | loc -> (function | p -> m#clsigi loc (Strip.clsigi p)))
    ~exp_filter:(efilter "clsigi") ~pat_filter:(pfilter "clsigi");
  add_quotation { domain; name = "clfield-" } clfield_quot
    ~mexp:(function
           | loc -> (function | p -> m#clfield loc (Strip.clfield p)))
    ~mpat:(function
           | loc -> (function | p -> m#clfield loc (Strip.clfield p)))
    ~exp_filter:(efilter "clfield") ~pat_filter:(pfilter "clfield");
  add_quotation { domain; name = "constr-" } constr_quot
    ~mexp:(function | loc -> (function | p -> m#constr loc (Strip.constr p)))
    ~mpat:(function | loc -> (function | p -> m#constr loc (Strip.constr p)))
    ~exp_filter:(efilter "constr") ~pat_filter:(pfilter "constr");
  add_quotation { domain; name = "bind-" } bind_quot
    ~mexp:(function | loc -> (function | p -> m#bind loc (Strip.bind p)))
    ~mpat:(function | loc -> (function | p -> m#bind loc (Strip.bind p)))
    ~exp_filter:(efilter "bind") ~pat_filter:(pfilter "bind");
  add_quotation { domain; name = "rec_exp-" } rec_exp_quot
    ~mexp:(function
           | loc -> (function | p -> m#rec_exp loc (Strip.rec_exp p)))
    ~mpat:(function
           | loc -> (function | p -> m#rec_exp loc (Strip.rec_exp p)))
    ~exp_filter:(efilter "rec_exp") ~pat_filter:(pfilter "rec_exp");
  add_quotation { domain; name = "case-" } case_quot
    ~mexp:(function | loc -> (function | p -> m#case loc (Strip.case p)))
    ~mpat:(function | loc -> (function | p -> m#case loc (Strip.case p)))
    ~exp_filter:(efilter "case") ~pat_filter:(pfilter "case");
  add_quotation { domain; name = "mbind-" } mbind_quot
    ~mexp:(function | loc -> (function | p -> m#mbind loc (Strip.mbind p)))
    ~mpat:(function | loc -> (function | p -> m#mbind loc (Strip.mbind p)))
    ~exp_filter:(efilter "mbind") ~pat_filter:(pfilter "mbind");
  add_quotation { domain; name = "ident-" } ident_quot
    ~mexp:(function | loc -> (function | p -> m#ident loc (Strip.ident p)))
    ~mpat:(function | loc -> (function | p -> m#ident loc (Strip.ident p)))
    ~exp_filter:(efilter "ident") ~pat_filter:(pfilter "ident");
  add_quotation { domain; name = "or_ctyp-" } constructor_declarations
    ~mexp:(function
           | loc -> (function | p -> m#or_ctyp loc (Strip.or_ctyp p)))
    ~mpat:(function
           | loc -> (function | p -> m#or_ctyp loc (Strip.or_ctyp p)))
    ~exp_filter:(efilter "or_ctyp") ~pat_filter:(pfilter "or_ctyp");
  add_quotation { domain; name = "row_field-" } row_field
    ~mexp:(function
           | loc -> (function | p -> m#row_field loc (Strip.row_field p)))
    ~mpat:(function
           | loc -> (function | p -> m#row_field loc (Strip.row_field p)))
