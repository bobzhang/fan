...
Gram.find_level expr ~position:(`Level "simple");;
Parser.parser_of_symbol (Obj.magic expr)  (`Snterm (Obj.magic ident)) 0 (Gram.filter (Gram.lex _loc (Stream.of_string "A.B.c")));;

let u =Parser.parser_of_tree (Obj.magic expr) (0,`LA) level.lprefix (Gram.filter (Gram.lex _loc (Stream.of_string "A.B.c")));;
